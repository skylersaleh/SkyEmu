 ...
 *file and all internal symbols and functions will only be visible inside that
 *file.
 *
 ...
 *       Includes riconsdata.h header defining a set of 128 icons (binary
 *format) to be used on multiple controls and following raygui styles
 *
 *
 *   VERSIONS HISTORY:
 ...
 *reviewed GuiListView(), removed Gui*Ex() controls 2.1 (26-Dec-2018) Redesign
 *of GuiCheckBox(), GuiComboBox(), GuiDropdownBox(), GuiToggleGroup() > Use
 *combined text string Complete redesign of style system (breaking change) 2.0
 *(08-Nov-2018) Support controls guiLock and custom fonts, reviewed
 *GuiComboBox(), GuiListView()... 1.9 (09-Oct-2018) Controls review: GuiGrid(),
 ...
 *functions (removed useless ones) 1.3 (12-Jun-2017) Redesigned styles system
...
// Define functions scope to be used internally (static) or externally (extern)
// to the module including this file
#if defined(_WIN32)
...
#define RAYGUIDEF                                                              \
  __declspec(                                                                  \
      dllexport) // We are building raygui as a Win32 shared library (.dll)
#elif defined(USE_LIBTYPE_SHARED)
#define RAYGUIDEF                                                              \
  __declspec(dllimport) // We are using raygui as a Win32 shared library (.dll)
#else
#define RAYGUIDEF // We are building or using raygui as a static library
#endif
#else
#define RAYGUIDEF // We are building or using raygui as a static library (or
...
#define TEXTEDIT_CURSOR_BLINK_FRAMES                                           \
  20 // Text edit controls cursor blink timming
...
// TODO: Texture2D type is very coupled to raylib, mostly required by
// GuiImageButton() It should be redesigned to be provided by user
...
RAYGUIDEF void GuiFade(float alpha);   // Set gui controls alpha (global state),
...
RAYGUIDEF void
GuiPanel(Rectangle bounds); // Panel control, useful to group controls
RAYGUIDEF Rectangle GuiScrollPanel(Rectangle bounds, Rectangle content,
                                   Vector2 *scroll); // Scroll Panel control

// Basic controls set
RAYGUIDEF void GuiLabel(Rectangle bounds,
...
RAYGUIDEF bool GuiLabelButton(
    Rectangle bounds,
    const char *text); // Label button control, show true when clicked
RAYGUIDEF bool GuiImageButton(
    Rectangle bounds, const char *text,
    Texture2D texture); // Image button control, returns true when clicked
RAYGUIDEF bool
GuiImageButtonEx(Rectangle bounds, const char *text, Texture2D texture,
                 Rectangle texSource); // Image button extended control, returns
                                       // true when clicked
RAYGUIDEF bool
...
RAYGUIDEF bool
GuiTextBoxMulti(Rectangle bounds, char *text, int textSize,
...
RAYGUIDEF float GuiProgressBar(
...
RAYGUIDEF void GuiDummyRec(Rectangle bounds,
                           const char *text); // Dummy control for placeholders
RAYGUIDEF int GuiScrollBar(Rectangle bounds, int value, int minValue,
                           int maxValue); // Scroll Bar control
RAYGUIDEF Vector2 GuiGrid(Rectangle bounds, float spacing,
...
RAYGUIDEF int GuiListViewEx(Rectangle bounds, const char **text, int count,
...
RAYGUIDEF Color GuiColorPanel(Rectangle bounds,
                              Color color); // Color Panel control
RAYGUIDEF float GuiColorBarAlpha(Rectangle bounds,
                                 float alpha); // Color Bar Alpha control
RAYGUIDEF float GuiColorBarHue(Rectangle bounds,
...
RAYGUIDEF GuiStyle LoadGuiStyle(const char *fileName);          // Load style
from file (.rgs) RAYGUIDEF void UnloadGuiStyle(GuiStyle style); // Unload style
*/
...
RAYGUIDEF void GuiDrawIcon(int iconId, Vector2 position, int pixelSize,
                           Color color);
...
RAYGUIDEF void GuiSetIconData(int iconId,
                              unsigned int *data); // Set icon bit data

RAYGUIDEF void GuiSetIconPixel(int iconId, int x,
                               int y); // Set icon pixel value
RAYGUIDEF void GuiClearIconPixel(int iconId, int x,
                                 int y); // Clear icon pixel value
RAYGUIDEF bool GuiCheckIconPixel(int iconId, int x,
...
// NOTE: In raygui we manage a single int array with all the possible style
// properties. When a new style is loaded, it loads over the global style... but
// default gui style could always be recovered with GuiLoadStyleDefault()
static unsigned int
    guiStyle[NUM_CONTROLS * (NUM_PROPS_DEFAULT + NUM_PROPS_EXTENDED)] = {0};
...
static int
GetCharPressed(void); // -- GuiTextBox(), GuiTextBoxMulti(), GuiValueBox()
//-------------------------------------------------------------------------------
...
static void DrawTriangle(Vector2 v1, Vector2 v2, Vector2 v3,
                         Color color); // -- GuiDropdownBox(), GuiScrollBar()
static void DrawTextureRec(Texture2D texture, Rectangle sourceRec,
                           Vector2 position,
                           Color tint); // -- GuiImageButtonEx()

static void DrawTextRec(Font font, const char *text, Rectangle rec,
                        float fontSize, float spacing, bool wordWrap,
                        Color tint); // -- GuiTextBoxMulti()
//-------------------------------------------------------------------------------
...
static Vector2
MeasureTextEx(Font font, const char *text, float fontSize,
...
//----------------------------------------------------------------------------------
static int
GetTextWidth(const char *text); // Gui get text width using default font
static Rectangle
...
static const char **
GuiTextSplit(const char *text, int *count,
...
void GuiSetStyle(int control, int property, int value) {
  if (!guiStyleLoaded)
    GuiLoadStyleDefault();
  guiStyle[control * (NUM_PROPS_DEFAULT + NUM_PROPS_EXTENDED) + property] =
      value;
...
int GuiGetStyle(int control, int property) {
  if (!guiStyleLoaded)
    GuiLoadStyleDefault();
  return guiStyle[control * (NUM_PROPS_DEFAULT + NUM_PROPS_EXTENDED) +
                  property];
}
...
#define WINDOW_STATUSBAR_HEIGHT 22
  ...
  int statusBarHeight =
      WINDOW_STATUSBAR_HEIGHT + 2 * GuiGetStyle(STATUSBAR, BORDER_WIDTH);
...
#define GROUPBOX_TEXT_PADDING 10
  ...
  //--------------------------------------------------------------------
  GuiDrawRectangle(
      RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y, GROUPBOX_LINE_THICK,
                                 bounds.height},
      0, BLANK,
      Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED)
                                             ? BORDER_COLOR_DISABLED
                                             : LINE_COLOR)),
           guiAlpha));
  GuiDrawRectangle(
      RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + bounds.height - 1,
                                 bounds.width, GROUPBOX_LINE_THICK},
      0, BLANK,
      Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED)
                                             ? BORDER_COLOR_DISABLED
                                             : LINE_COLOR)),
  ...
  GuiDrawRectangle(
      RAYGUI_CLITERAL(Rectangle){bounds.x + bounds.width - 1, bounds.y,
                                 GROUPBOX_LINE_THICK, bounds.height},
      0, BLANK,
      Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED)
                                             ? BORDER_COLOR_DISABLED
                                             : LINE_COLOR)),
  ...
  GuiLine(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y, bounds.width, 1},
          text);
  //--------------------------------------------------------------------
}
...
#define LINE_TEXT_PADDING 10
  ...
  Color color = Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED)
                                                       ? BORDER_COLOR_DISABLED
                                                       : LINE_COLOR)),
                     guiAlpha);
  ...
  if (text == NULL)
    ...
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y,
                                                LINE_TEXT_PADDING - 2, 1},
                     0, BLANK, color);
    GuiLabel(textBounds, text);
    GuiDrawRectangle(
        RAYGUI_CLITERAL(Rectangle){
            bounds.x + LINE_TEXT_PADDING + textBounds.width + 4, bounds.y,
          ...
          ? content.width - bounds.width + (float)verticalScrollBarWidth +
                ...
                (((float)GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) ==
                  SCROLLBAR_LEFT_SIDE)
                     ...
                     : 0)
          : (float)-GuiGetStyle(DEFAULT, BORDER_WIDTH);
  const float verticalMin = hasVerticalScrollBar
                                ? (float)-GuiGetStyle(DEFAULT, BORDER_WIDTH)
                                : (float)-GuiGetStyle(DEFAULT, BORDER_WIDTH);
  const float verticalMax = hasVerticalScrollBar
...
void GuiLabel(Rectangle bounds, const char *text) {
  ...
  GuiDrawText(text, GetTextBounds(LABEL, bounds),
              ...
              Fade(GetColor(GuiGetStyle(LABEL, (state == GUI_STATE_DISABLED)
                                                   ? TEXT_COLOR_DISABLED
                                                   : TEXT_COLOR_NORMAL)),
                   guiAlpha));
  //--------------------------------------------------------------------
...
bool GuiImageButton(Rectangle bounds, const char *text, Texture2D texture) {
  return GuiImageButtonEx(bounds, text, texture,
                          RAYGUI_CLITERAL(Rectangle){0, 0, (float)texture.width,
                                                     (float)texture.height});
}

// Image button control, returns true when clicked
bool GuiImageButtonEx(Rectangle bounds, const char *text, Texture2D texture,
                      Rectangle texSource) {
  GuiControlState state = guiState;
  bool clicked = false;
...
bool GuiCheckBox(Rectangle bounds, const char *text, bool checked) {
  ...
  if ((state != GUI_STATE_DISABLED) && !guiLocked) {
    ...
    Rectangle totalBounds = {
        (GuiGetStyle(CHECKBOX, TEXT_ALIGNMENT) == GUI_TEXT_ALIGN_LEFT)
            ? textBounds.x
            : bounds.x,
        bounds.y,
...
bool GuiDropdownBox(Rectangle bounds, const char *text, int *active,
  ...
  boundsOpen.height =
      (itemsCount + 1) *
      (bounds.height + GuiGetStyle(DROPDOWNBOX, DROPDOWN_ITEMS_PADDING));
  ...
  DrawTriangle(
      RAYGUI_CLITERAL(Vector2){bounds.x + bounds.width -
                                   GuiGetStyle(DROPDOWNBOX, ARROW_PADDING),
                               bounds.y + bounds.height / 2 - 2},
      RAYGUI_CLITERAL(Vector2){bounds.x + bounds.width -
                                   GuiGetStyle(DROPDOWNBOX, ARROW_PADDING) + 5,
                               bounds.y + bounds.height / 2 - 2 + 5},
      RAYGUI_CLITERAL(Vector2){bounds.x + bounds.width -
                                   GuiGetStyle(DROPDOWNBOX, ARROW_PADDING) + 10,
                               bounds.y + bounds.height / 2 - 2},
      Fade(GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state * 3))), guiAlpha));

  // GuiDrawText("v", RAYGUI_CLITERAL(Rectangle){ bounds.x + bounds.width -
  // GuiGetStyle(DROPDOWNBOX, ARROW_PADDING), bounds.y + bounds.height/2 - 2,
  // 10, 10 },
  //            GUI_TEXT_ALIGN_CENTER, Fade(GetColor(GuiGetStyle(DROPDOWNBOX,
...
bool GuiTextBox(Rectangle bounds, char *text, int textSize, bool editMode) {
  ...
  Rectangle cursor = {
      ...
      bounds.y + bounds.height / 2 - GuiGetStyle(DEFAULT, TEXT_SIZE), 1,
  ...
  if ((state != GUI_STATE_DISABLED) && !guiLocked) {
    ...
    if (editMode) {
      ...
      if (keyCount < (textSize - 1)) {
        float maxWidth =
            (bounds.width - (GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING) * 2));
      ...
      if (keyCount > 0) {
        if (IsKeyPressed(KEY_BACKSPACE)) {
          ...
          if ((framesCounter > TEXTEDIT_CURSOR_BLINK_FRAMES) &&
              (framesCounter % 2) == 0)
            keyCount--;
          text[keyCount] = '\0';
          if (keyCount < 0)
      ...
      if (textAlignment == GUI_TEXT_ALIGN_CENTER)
        ...
        cursor.x =
            bounds.x + bounds.width - GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING);
    } else {
      if (CheckCollisionPointRec(mousePoint, bounds)) {
        state = GUI_STATE_FOCUSED;
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
  ...
  GuiDrawText(
      text, GetTextBounds(TEXTBOX, bounds),
...
#define VALUEBOX_MAX_CHARS 32
  ...
  GuiDrawRectangle(
      ...
      baseColor);
  GuiDrawText(
      textValue, GetTextBounds(VALUEBOX, bounds), GUI_TEXT_ALIGN_CENTER,
      Fade(GetColor(GuiGetStyle(VALUEBOX, TEXT + (state * 3))), guiAlpha));

  // Draw blinking cursor
  if ((state == GUI_STATE_PRESSED) &&
      (editMode && ((framesCounter / 20) % 2 == 0))) {
    // NOTE: ValueBox internal text is always centered
    Rectangle cursor = {
        bounds.x + GetTextWidth(textValue) / 2 + bounds.width / 2 + 2,
        bounds.y + 2 * GuiGetStyle(VALUEBOX, BORDER_WIDTH), 1,
...
bool GuiTextBoxMulti(Rectangle bounds, char *text, int textSize,
  ...
  if ((state != GUI_STATE_DISABLED) && !guiLocked) {
    ...
    if (editMode) {
      ...
      if (keyCount < (textSize - 1)) {
        ...
        if (textSize.y <
            (textAreaBounds.height - GuiGetStyle(DEFAULT, TEXT_SIZE))) {
          if (IsKeyPressed(KEY_ENTER)) {
            text[keyCount] = '\n';
            keyCount++;
          } else if (((character >= 32) &&
                      (character < 255))) // TODO: Support Unicode inputs
          {
      ...
      for (int i = 0; i < keyCount && currentLine < keyCount; i++) {
        ...
        textWidth +=
            (GetTextWidth(oneCharText) + GuiGetStyle(DEFAULT, TEXT_SPACING));
        ...
        if (text[i] == '\n' || textWidth >= textAreaBounds.width) {
          ...
          if (lastBreakingPos > 0)
            ...
            textWidth += (GetTextWidth(oneCharText) +
      ...
      cursor.x = bounds.x + GuiGetStyle(TEXTBOX, BORDER_WIDTH) +
                 GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING) + textWidth -
                 GuiGetStyle(DEFAULT, TEXT_SPACING);
      cursor.y = bounds.y + GuiGetStyle(TEXTBOX, BORDER_WIDTH) +
                 GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING) / 2 +
  ...
  DrawTextRec(
      guiFont, text, textAreaBounds, (float)GuiGetStyle(DEFAULT, TEXT_SIZE),
      (float)GuiGetStyle(DEFAULT, TEXT_SPACING), true,
      Fade(GetColor(GuiGetStyle(TEXTBOX, TEXT + (state * 3))), guiAlpha));
  //--------------------------------------------------------------------
...
float GuiSliderPro(Rectangle bounds, const char *textLeft,
  ...
  GuiDrawRectangle(
      ...
      Fade(GetColor(GuiGetStyle(SLIDER, (state != GUI_STATE_DISABLED)
                                            ? BASE_COLOR_NORMAL
                                            : BASE_COLOR_DISABLED)),
           guiAlpha));
  ...
  if (textLeft != NULL) {
    Rectangle textBounds = {0};
    textBounds.width =
        (float)GetTextWidth(textLeft); // TODO: Consider text icon
    textBounds.height = (float)GuiGetStyle(DEFAULT, TEXT_SIZE);
    textBounds.x =
    ...
    GuiDrawText(
        textLeft, textBounds, GUI_TEXT_ALIGN_RIGHT,
  ...
  if (textRight != NULL) {
    Rectangle textBounds = {0};
    textBounds.width =
        (float)GetTextWidth(textRight); // TODO: Consider text icon
    textBounds.height = (float)GuiGetStyle(DEFAULT, TEXT_SIZE);
    ...
    GuiDrawText(
        textRight, textBounds, GUI_TEXT_ALIGN_LEFT,
        Fade(GetColor(GuiGetStyle(SLIDER, TEXT + (state * 3))), guiAlpha));
  }
  //--------------------------------------------------------------------
...
void GuiStatusBar(Rectangle bounds, const char *text) {
  ...
  GuiDrawRectangle(
      ...
      Fade(GetColor(GuiGetStyle(STATUSBAR, (state != GUI_STATE_DISABLED)
                                               ? BORDER_COLOR_NORMAL
                                               : BORDER_COLOR_DISABLED)),
           guiAlpha),
      Fade(GetColor(GuiGetStyle(STATUSBAR, (state != GUI_STATE_DISABLED)
...
void GuiDummyRec(Rectangle bounds, const char *text) {
  ...
  GuiDrawRectangle(
      ...
      Fade(GetColor(GuiGetStyle(DEFAULT, (state != GUI_STATE_DISABLED)
                                             ...
                                             : BASE_COLOR_DISABLED)),
           guiAlpha));
  GuiDrawText(text, GetTextBounds(DEFAULT, bounds), GUI_TEXT_ALIGN_CENTER,
              Fade(GetColor(GuiGetStyle(BUTTON, (state != GUI_STATE_DISABLED)
                                                    ? TEXT_COLOR_NORMAL
                                                    : TEXT_COLOR_DISABLED)),
                   guiAlpha));
  //------------------------------------------------------------------
...
int GuiScrollBar(Rectangle bounds, int value, int minValue, int maxValue) {
  ...
  const Vector2 lineCoords[] = {
      ...
      {arrowDownRight.x + spinnerSize - padding,
       arrowDownRight.y + spinnerSize / 2},
      ...
      {arrowDownRight.x + padding, arrowDownRight.y + padding},
      {arrowDownRight.x + spinnerSize / 2,
       arrowDownRight.y + spinnerSize - padding},
...
#define COLORBARALPHA_CHECKED_SIZE 10
  ...
  Rectangle selector = {
      (float)bounds.x + alpha * bounds.width -
          GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
...
float GuiColorBarHue(Rectangle bounds, float hue) {
  ...
  if (state != GUI_STATE_DISABLED) {
    ...
    DrawRectangleGradientV(
        (int)bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        (int)bounds.y + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        (int)bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
        (int)bounds.height / 6,
        Fade(RAYGUI_CLITERAL(Color){255, 0, 0, 255}, guiAlpha),
        Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha));
    DrawRectangleGradientV(
        (int)bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        (int)bounds.y + (int)bounds.height / 6 +
            GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        (int)bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
        (int)bounds.height / 6,
        Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha),
    ...
    DrawRectangleGradientV(
        (int)bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        (int)bounds.y + 2 * ((int)bounds.height / 6) +
            GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        (int)bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
        (int)bounds.height / 6,
        Fade(RAYGUI_CLITERAL(Color){0, 255, 0, 255}, guiAlpha),
    ...
    DrawRectangleGradientV(
        (int)bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        (int)bounds.y + 3 * ((int)bounds.height / 6) +
            GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        (int)bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
        (int)bounds.height / 6,
        Fade(RAYGUI_CLITERAL(Color){0, 255, 255, 255}, guiAlpha),
    ...
    DrawRectangleGradientV(
        (int)bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        (int)bounds.y + 4 * ((int)bounds.height / 6) +
            GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        (int)bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
        (int)bounds.height / 6,
        Fade(RAYGUI_CLITERAL(Color){0, 0, 255, 255}, guiAlpha),
    ...
    DrawRectangleGradientV(
        (int)bounds.x + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        (int)bounds.y + 5 * ((int)bounds.height / 6) +
            GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) / 2,
        (int)bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
        (int)bounds.height / 6 -
            GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW),
...
#define MESSAGEBOX_BUTTON_PADDING 10
  ...
  buttonBounds.y = bounds.y + bounds.height - MESSAGEBOX_BUTTON_HEIGHT -
                   MESSAGEBOX_BUTTON_PADDING;
  buttonBounds.width =
      (bounds.width - MESSAGEBOX_BUTTON_PADDING * (buttonsCount + 1)) /
      buttonsCount;
  ...
  textBounds.y = bounds.y + WINDOW_STATUSBAR_HEIGHT +
                 (bounds.height - WINDOW_STATUSBAR_HEIGHT -
                  MESSAGEBOX_BUTTON_HEIGHT - MESSAGEBOX_BUTTON_PADDING) /
                     2 -
                 textSize.y / 2;
...
#define TEXTINPUTBOX_MAX_TEXT_LENGTH 256
  ...
  buttonBounds.y = bounds.y + bounds.height - TEXTINPUTBOX_BUTTON_HEIGHT -
                   TEXTINPUTBOX_BUTTON_PADDING;
  buttonBounds.width =
      (bounds.width - TEXTINPUTBOX_BUTTON_PADDING * (buttonsCount + 1)) /
      buttonsCount;
  ...
  if (message != NULL) {
    ...
    textBounds.x = bounds.x + bounds.width / 2 - textSize.x / 2;
    textBounds.y = bounds.y + WINDOW_STATUSBAR_HEIGHT + messageInputHeight / 4 -
                   textSize.y / 2;
    textBounds.width = textSize.x;
    textBounds.height = textSize.y;
  }
  ...
  textBoxBounds.y =
      bounds.y + WINDOW_STATUSBAR_HEIGHT - TEXTINPUTBOX_HEIGHT / 2;
  if (message == NULL)
  ...
  if (GuiTextBox(textBoxBounds, text, TEXTINPUTBOX_MAX_TEXT_LENGTH,
                 textEditMode))
  ...
  if (tryBinary) {
    ...
    if ((signature[0] == 'r') && (signature[1] == 'G') &&
      ...
      if (fontDataSize > 0) {
        ...
        // Load font recs data
        font.recs =
            (Rectangle *)RAYGUI_CALLOC(font.charsCount, sizeof(Rectangle));
        for (int i = 0; i < font.charsCount; i++)
          fread(&font.recs[i], 1, sizeof(Rectangle), rgsFile);

        // Load font chars info data
        font.chars =
            (CharInfo *)RAYGUI_CALLOC(font.charsCount, sizeof(CharInfo));
        for (int i = 0; i < font.charsCount; i++) {
...
char **GuiLoadIcons(const char *fileName, bool loadIconsName) {
  ...
  if (rgiFile != NULL) {
    ...
    if ((signature[0] == 'r') && (signature[1] == 'G') &&
      ...
      fread(guiIcons, iconsCount * (iconsSize * iconsSize / 32),
...
#if !defined(RAYGUI_STANDALONE)
        DrawRectangle(position.x + (k % RICON_SIZE) * pixelSize,
                      position.y + y * pixelSize, pixelSize, pixelSize, color);
#endif
      }
...
unsigned int *GuiGetIconData(int iconId) {
  ...
  if (iconId < RICON_MAX_ICONS)
    memcpy(iconData, &guiIcons[iconId * RICON_DATA_ELEMENTS],
...
void GuiSetIconData(int iconId, unsigned int *data) {
  if (iconId < RICON_MAX_ICONS)
    memcpy(&guiIcons[iconId * RICON_DATA_ELEMENTS], data,
           RICON_DATA_ELEMENTS * sizeof(unsigned int));
}
...
#define BIT_SET(a, b) ((a) |= (1 << (b)))
  ...
  // data element
  BIT_SET(guiIcons[iconId * RICON_DATA_ELEMENTS +
                   y / (sizeof(unsigned int) * 8 / RICON_SIZE)],
...
#define BIT_CLEAR(a, b) ((a) &= ~((1) << (b)))
  ...
  // For example, in case of 16x16 pixels, every 2 lines fit in one unsigned int
  // data element
  BIT_CLEAR(guiIcons[iconId * RICON_DATA_ELEMENTS +
                     y / (sizeof(unsigned int) * 8 / RICON_SIZE)],
            x + (y % (sizeof(unsigned int) * 8 / RICON_SIZE) * RICON_SIZE));
}
...
static int GetTextWidth(const char *text) {
  ...
  if ((text != NULL) && (text[0] != '\0'))
    size = MeasureTextEx(guiFont, text, (float)GuiGetStyle(DEFAULT, TEXT_SIZE),
                         (float)GuiGetStyle(DEFAULT, TEXT_SPACING));
...
static Rectangle GetTextBounds(int control, Rectangle bounds) {
  ...
  case COMBOBOX:
    bounds.width -= (GuiGetStyle(control, COMBO_BUTTON_WIDTH) +
                     GuiGetStyle(control, COMBO_BUTTON_PADDING));
    break;
  case VALUEBOX:
  ...
  // (scrollbar?) More special cases (label side): CHECKBOX, SLIDER, VALUEBOX,
  // SPINNER
...
#define ICON_TEXT_PADDING 4
    ...
    if (iconId >= 0) {
      ...
      GuiDrawIcon(iconId,
                  RAYGUI_CLITERAL(Vector2){
                      position.x, bounds.y + bounds.height / 2 -
                                      RICON_SIZE / 2 +
                                      TEXT_VALIGN_PIXEL_OFFSET(bounds.height)},
                  1, tint);
...
static void GuiDrawRectangle(Rectangle rec, int borderWidth, Color borderColor,
  ...
  // In this case all controls drawing logic should be moved to this function...
  // I don't like it...
}
...
#define TEXTSPLIT_MAX_TEXT_BUFFER_LENGTH                                       \
  1024 // Size of static buffer: TextSplit()
#define TEXTSPLIT_MAX_SUBSTRINGS_COUNT                                         \
  128 // Size of static pointers array: TextSplit()
