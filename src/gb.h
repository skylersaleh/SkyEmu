...
mmio_reg_t gb_io_reg_desc[] = {
    ...
    {SB_IO_SERIAL_CTRL,
     ...
     {
         {7, 1,
          "Transfer Start Flag (0=No transfer is in progress or requested, "
    ...
    {SB_IO_AUD3_POWER, "AUD3_POWER", {{7, 1, "Sound Channel 3 Enable"}}},
    ...
    {SB_IO_GBC_SPEED_SWITCH,
     "GBC_SPEED_SWITCH",
     {
         {7, 1, "Current Speed     (0=Normal, 1=Double) (Read Only)"},
         {0, 1, "Prepare Speed Switch (0=No, 1=Prepare) (Read/Write)"},
     }},
    {SB_IO_GBC_VBK, "GBC_VBK", {{0, 1, "VRAM Bank Sel"}}},
    {SB_IO_BIOS_BANK, "BIOS_BANK", {{0, 8, "BANK VALUE"}}},
    ...
    {SB_IO_DMA_MODE_LEN,
     "DMA_MODE_LEN",
     {{7, 1, "Mode (0: General 1: HDMA)"}, {0, 6, "Length*16B"}}},
    {SB_IO_GBC_BCPS,
     "GBC_BCPS",
    ...
    {SB_IO_GBC_SVBK, "GBC_SVBK", {{0, 3, "WRAM Bank Sel"}}},
    {SB_IO_INTER_EN,
     "INTER_EN",
     {
...
typedef struct {
  ...
  uint8_t
      color_palettes[SB_PPU_BG_COLOR_PALETTES + SB_PPU_SPRITE_COLOR_PALETTES];
...
static bool sb_load_best_effort_state(sb_gb_t *gb, uint8_t *save_state_data,
                                      uint32_t size, uint32_t bess_offset) {
  if (bess_offset + sizeof(sb_gb_bess_info_t) > size)
    return false;
  sb_gb_bess_info_t *bess =
      (sb_gb_bess_info_t *)(save_state_data + bess_offset);

  if (bess->bess_version != 1)
    return false;
  if (bess->data_seg + sizeof(gb->mem.data) > size)
    return false;
  if (bess->wram_seg + sizeof(gb->mem.wram) > size)
    return false;
  if (bess->vram_seg + sizeof(gb->lcd.vram) > size)
    return false;
  if (bess->palette_seg + sizeof(gb->lcd.color_palettes) > size)
    return false;
  ...
  memcpy(gb->lcd.color_palettes, save_state_data + bess->palette_seg,
         sizeof(gb->lcd.color_palettes));
...
typedef void (*sb_opcode_impl_t)(sb_gb_t *, int op1, int op2, int op1_enum,
                                 int op2_enum, const uint8_t *flag_mask);

// Include down here because of dependence on sb_gb_t /*TODO: refactor this*/
#include "sb_instr_tables.h"

uint32_t sb_lookup_tile(sb_gb_t *gb, int px, int py, int tile_base,
                        int data_mode);
...
static FORCE_INLINE uint8_t sb_read8_io(sb_gb_t *gb, int addr) {
  return gb->mem.data[addr];
}
static FORCE_INLINE void sb_store8_io(sb_gb_t *gb, int addr, int value) {
...
static FORCE_INLINE uint8_t sb_read8_direct(sb_gb_t *gb, int addr) {
  if (addr >= 0x0000 && addr <= 0x3fff) {
    if (addr < 256 || (addr >= 512 && addr < 2304)) {
      if (!sb_read8_io(gb, SB_IO_BIOS_BANK))
        return gb->bios[addr];
    }
    ...
    if (gb->rtc.has_rtc && gb->cart.mbc_type == SB_MBC_MBC3) {
      ...
      case 0x08:
        return gb->rtc.latched_sec;
      case 0x09:
        return gb->rtc.latched_min;
      case 0x0A:
        return gb->rtc.latched_hour;
      case 0x0B:
        return gb->rtc.latched_day & 0xff;
      case 0x0C:
        return SB_BFE(gb->rtc.latched_day, 8, 1);
      }
    }
    ...
    if (gb->cart.mbc_type == SB_MBC_MBC1) {
      ...
      if (gb->cart.bank_mode)
        ram_addr_off |= SB_BFE(gb->cart.mapped_ram_bank, 0, 2) << 13;
    }
...
bool sb_gbc_enable(sb_gb_t *gb) {
  return (gb->mem.data[SB_IO_GBC_KEY0] != 0x4 ||
          !gb->mem.data[SB_IO_BIOS_BANK]) &&
         gb->model == SB_GBC;
}
uint8_t sb_io_or_mask(sb_gb_t *gb, int addr) {
  ...
  } else if (addr == SB_IO_INTER_F)
    return 0xE0;
  else if (addr == SB_IO_LCD_STAT)
    return 0x80;
  else if (addr == SB_IO_TAC)
    return 0xf8;
  else if (addr == SB_IO_SERIAL_CTRL)
    return 0x7e;
  else if (addr == SB_IO_JOYPAD)
    return 0xc0;
  else if (addr == 0xFF03 || addr == 0xFF08 || addr == 0xFF09 ||
           addr == 0xFF0A || addr == 0xFF0B || addr == 0xFF0C ||
           addr == 0xFF0D || addr == 0xFF0E || addr == 0xFF15 ||
           addr == 0xFF1F || addr == 0xFF27 || addr == 0xFF28 ||
           addr == 0xFF29 || (addr >= 0xff4c && addr <= 0xff7f && !gbc_en) ||
  ...
  if (addr >= 0x8000 && addr <= 0x9fff) {
    ...
    if (gb->cart.ram_write_enable && gb->cart.ram_size) {
      ...
      if (gb->cart.mbc_type == SB_MBC_MBC1) {
        ...
        ram_addr_off |= gb->cart.mapped_ram_bank << 13;
      ram_addr_off %= gb->cart.ram_size;
      gb->cart.ram_data[ram_addr_off] = value;
      gb->cart.ram_is_dirty = true;
      // printf("RAM WRITE: MEM[%04x, %04x] = %02x B:%d
      // BM:%d\n",addr,ram_addr_off,value,
      // gb->cart.mapped_ram_bank,gb->cart.bank_mode);
    }
    return;
  } else if (addr >= 0xD000 && addr <= 0xDfff) {
    int bank = gb->mem.data[SB_IO_GBC_SVBK] % SB_WRAM_NUM_BANKS;
...
void sb_store8(sb_gb_t *gb, int addr, int value) {
  if (addr >= 0xff00) {
    if (!sb_gbc_enable(gb) && addr >= 0xff4C && addr <= 0xff7f &&
        addr != SB_IO_BIOS_BANK)
      return;
    if (addr == SB_IO_DMA_SRC_LO || addr == SB_IO_DMA_DST_LO) {
      ...
      if (gb->dma.active) {
        ...
        value &= 0x7f;
        uint16_t dma_src = sb_read8_io(gb, SB_IO_DMA_SRC_LO) |
                           ((int)sb_read8_io(gb, SB_IO_DMA_SRC_HI) << 8u);
        uint16_t dma_dst = sb_read8_io(gb, SB_IO_DMA_DST_LO) |
                           ((int)sb_read8_io(gb, SB_IO_DMA_DST_HI) << 8u);
      }
    }
    if (addr == SB_IO_OAM_DMA) {
      ...
      printf("%c", (char)value);
    } else if (addr >= SB_IO_AUD1_TONE_SWEEP &&
               addr < SB_IO_AUD3_WAVE_BASE + 16) {
      ...
      if (addr >= SB_IO_AUD3_WAVE_BASE && addr < SB_IO_AUD3_WAVE_BASE + 16) {
        ...
        if (wave_active) {
          ...
          addr =
              SB_IO_AUD3_WAVE_BASE + ((gb->audio.wave_sample_offset) % 32) / 2;
        }
      }
      if (addr == SB_IO_AUD1_LENGTH_DUTY || addr == SB_IO_AUD2_LENGTH_DUTY ||
        ...
        if (i == 2)
          gb->audio.sequencer.length[i] = 256 - SB_BFE(length_duty, 0, 8);
        else
          gb->audio.sequencer.length[i] = 64 - SB_BFE(length_duty, 0, 6);
        ...
        if (gb->audio.sequencer.env_period[i] == 0 &&
            !gb->audio.sequencer.env_overflow[i]) {
          gb->audio.sequencer.volume[i] =
              (gb->audio.sequencer.volume[i] + 1) & 0xf;
        }
      ...
      return;
    } else if (addr == SB_IO_BIOS_BANK) {
      value |= sb_read8_io(gb, SB_IO_BIOS_BANK);
    } else if (addr == SB_IO_GBC_KEY0) {
      if (sb_read8_io(gb, SB_IO_BIOS_BANK))
        return;
    ...
    case SB_MBC_MBC1:
      gb->cart.mapped_rom_bank = value % 32;
      if (!gb->cart.mapped_rom_bank)
        gb->cart.mapped_rom_bank = 1;
      break;
    case SB_MBC_MBC2:
      gb->cart.mapped_rom_bank = value % 16;
      if (!gb->cart.mapped_rom_bank)
        gb->cart.mapped_rom_bank = 1;
      break;
    case SB_MBC_MBC3:
      gb->cart.mapped_rom_bank = value % 256;
      if (!gb->cart.mapped_rom_bank)
        gb->cart.mapped_rom_bank = 1;
      break;
    case SB_MBC_MBC5:
    case SB_MBC_MBC7:
      if (addr >= 0x3000) {
        ...
        gb->cart.mapped_rom_bank = (gb->cart.mapped_rom_bank & 0x100) | value;
      break;
    }
    return;
  } else if (addr >= 0x4000 && addr <= 0x5fff) {
    gb->cart.rumble = false;
    if ((value & (1 << 3)) && gb->cart.has_rumble)
      gb->cart.rumble = true;
    // MBC3 rombank select
    // TODO implement other mappers
    if (gb->cart.mbc_type != SB_MBC_MBC1 && gb->cart.ram_size) {
      if (gb->cart.mbc_type != SB_MBC_MBC3)
    ...
    if (gb->cart.mbc_type == SB_MBC_MBC3 && gb->rtc.has_rtc &&
        SB_BFE(value, 0, 1)) {
...
static FORCE_INLINE void sb_update_lcd(sb_emu_state_t *emu, sb_gb_t *gb) {
  uint8_t stat = sb_read8_io(gb, SB_IO_LCD_STAT);
  ...
  if (!enable) {
    // TODO: Starting at 4 here, may indicate a CPU timing bug. But is needed to
    // pass 1-lcd_sync.gb
    ...
    if (gb->lcd.scanline_cycles < mode2_clks)
      mode = 2;
    else if (gb->lcd.scanline_cycles < mode3_clks + mode2_clks)
      mode = 3;
    else
      mode = 0;
    ...
    if (gb->lcd.render_frame) {
      if (mode == 2) {
        ...
        if (clock_num % 2) {
          ...
          int yc = (int)sb_read8_io(gb, sprite_base + 0) - 16;
          int xc = (int)sb_read8_io(gb, sprite_base + 1) - 16;
          if (yc <= y && yc + sprite_h > y &&
              gb->lcd.sprite_index < SB_SPRITES_PER_SCANLINE &&
              sprite_id < num_sprites && draw_sprite) {
            gb->lcd.render_sprites[gb->lcd.sprite_index] = sprite_id;
            for (int i = 0; i < 4; ++i) {
              gb->lcd.render_sprites_data[gb->lcd.sprite_index][i] =
                  sb_read8_io(gb, sprite_base + i);
            }
            gb->lcd.sprite_index++;
          }
        }
      ...
      if (gb->lcd.curr_scanline < SB_LCD_H) {
        ...
        // This is intenentionally not latched for: 007 - The World is Not
        // Enough's hud. Zen Intergalactic Ninja, Speedy Gonzales, and the
        // Warriors of Might and Magic are also very sensitive to the behavior
        // of this window.
        if (ly == wy && window_enable)
          gb->lcd.wy_eq_ly = true;
        if (x < SB_LCD_W && x >= 0) {
          sb_draw_pixel(emu, gb, x, gb->lcd.curr_scanline);
        }
      }
    }
    if (ly == 153 && gb->lcd.scanline_cycles >= 4) {
      ly = 0;
    }
    ...
    if (ly >= SB_LCD_H && old_ly < SB_LCD_H) {
      uint8_t inter_flag = sb_read8_io(gb, SB_IO_INTER_F);
      // V-BLANK Interrupt
      sb_store8_io(gb, SB_IO_INTER_F, inter_flag | (1 << 0));
    }
    ...
    if ((mode & 0x4) == 4 && lyc_eq_ly_interrupt)
      curr_stat_interrupt = true;
    if ((mode & 0x3) == 2 && oam_interrupt)
      curr_stat_interrupt = true;
    if ((mode & 0x3) == 0 && hblank_interrupt)
      curr_stat_interrupt = true;
    if (curr_stat_interrupt && !gb->lcd.last_stat_interrupt) {
      uint8_t inter_flag = sb_read8_io(gb, SB_IO_INTER_F);
      sb_store8_io(gb, SB_IO_INTER_F, inter_flag | (1 << 1));
    }
    gb->lcd.last_stat_interrupt = curr_stat_interrupt;
    stat = (stat & 0xf8) | mode | 0x80;
    sb_store8_io(gb, SB_IO_LCD_STAT, stat);
  }
  gb->lcd.in_hblank = (mode & 0x3) == 0;
  sb_store8_io(gb, SB_IO_LCD_LY, ly);
  ;
}
...
uint32_t sb_lookup_tile(sb_gb_t *gb, int px, int py, int tile_base,
  ...
  int tile_offset =
      (((px & 0xff) / tile_size) + ((py & 0xff) / tile_size) * tiles_per_row) &
      0x3ff;
  ...
  if (data_mode == 0) {
    byte_tile_data_off =
        0x8000 + 0x1000 + ((int)((int8_t)(tile_id))) * bytes_per_tile;
  } else {
  ...
  int color_id = (SB_BFE(data1, pixel_in_tile_x, 1) +
                  SB_BFE(data2, pixel_in_tile_x, 1) * 2);
  color_id |= (tile_bg_palette & 0x3f) << 2;
  if (bg_to_oam_priority)
    color_id |= 1 << 8;
  return color_id;
}
void sb_lookup_palette_color(sb_gb_t *gb, int color_id, int *r, int *g,
                             int *b) {
  uint8_t palette = 0;
  if (gb->model == SB_GB) {
    int pal_id = SB_BFE(color_id, 2, 6);
    if (pal_id == SB_BACKG_PALETTE)
      palette = sb_read8_io(gb, SB_IO_PPU_BGP);
    else if (pal_id == SB_OBJ1_PALETTE)
      palette = sb_read8_io(gb, SB_IO_PPU_OBP1);
    else
      palette = color_id == 0 ? 0 : sb_read8_io(gb, SB_IO_PPU_OBP0);
    ...
    *b = gb->dmg_palette[color_id * 3 + 2];
    ...
    if (!sb_gbc_enable(gb)) {
      ...
      int pal_id = SB_BFE(color_id, 2, 6);
      if (pal_id == SB_BACKG_PALETTE)
        pal_map = sb_read8_io(gb, SB_IO_PPU_BGP);
      else if (pal_id == SB_OBJ1_PALETTE)
        pal_map = sb_read8_io(gb, SB_IO_PPU_OBP1);
      else
        pal_map = color_id == 0 ? 0 : sb_read8_io(gb, SB_IO_PPU_OBP0);
...
void sb_draw_pixel(sb_emu_state_t *emu, sb_gb_t *gb, int x, int y) {
  ...
  int oam_table_offset = 0xfe00;
  int wx = sb_read8_io(gb, SB_IO_LCD_WX) - 7;
  int wy = sb_read8_io(gb, SB_IO_LCD_WY);
  ...
  gb->lcd.window_active |=
      gb->lcd.latched_window_enable && gb->lcd.wy_eq_ly && x >= wx;
  if (draw_bg_win) {
    if (gb->lcd.window_active) {
      int px = x - wx;
      if (px >= 0) {
        int py = gb->lcd.curr_window_scanline;
        color_id = sb_lookup_tile(gb, px, py, win_tile_map_base,
                                  bg_win_tile_data_mode);
      }
    } else {
      int px = x + sx;
      int py = y + sy;
      color_id =
          sb_lookup_tile(gb, px, py, bg_tile_map_base, bg_win_tile_data_mode);
    }
  }
  if (draw_sprite) {
    ...
    for (int i = 0; i < gb->lcd.sprite_index; ++i) {
      ...
      if (prior_sprite <= prior || x_sprite >= 8 || x_sprite < 0)
        continue;

      int yc = gb->lcd.render_sprites_data[i][0] - 16;
      int y_sprite = y - yc;

      int tile = gb->lcd.render_sprites_data[i][2];
      int attr = gb->lcd.render_sprites_data[i][3];
      int tile_d_vram_bank = 0;
      ...
      if (x_flip)
        x_sprite = 7 - x_sprite;
      if (y_flip)
        y_sprite = (sprite8x16 ? 15 : 7) - y_sprite;
  ...
  gb->lcd.framebuffer[p + 0] =
      r * (1.0 - ghost_coef) + gb->lcd.framebuffer[p + 0] * ghost_coef + 0.5;
  gb->lcd.framebuffer[p + 1] =
      g * (1.0 - ghost_coef) + gb->lcd.framebuffer[p + 1] * ghost_coef + 0.5;
  gb->lcd.framebuffer[p + 2] =
      b * (1.0 - ghost_coef) + gb->lcd.framebuffer[p + 2] * ghost_coef + 0.5;
}

void sb_update_timers(sb_gb_t *gb, int delta_clocks, bool double_speed) {
  uint8_t tac = sb_read8_io(gb, SB_IO_TAC);
  ...
  for (int i = 0; i < delta_clocks; ++i) {
    ...
    if (tick_tima == false && gb->timers.last_tick_tima == true) {
      uint8_t d = sb_read8_io(gb, SB_IO_TIMA);
      // Trigger timer interrupt
      if (d == 255) {
        uint8_t i_flag = sb_read8_io(gb, SB_IO_INTER_F);
        i_flag |= 1 << 2;
        sb_store8_io(gb, SB_IO_INTER_F, i_flag);
        d = sb_read8_io(gb, SB_IO_TMA);
      } else
        d += 1;
      sb_store8_io(gb, SB_IO_TIMA, d);
    }
    if (tick_seq && !gb->timers.last_tick_seq) {
      sb_tick_frame_seq(gb, &gb->audio.sequencer);
    }
    gb->timers.last_tick_seq = tick_seq;
    gb->timers.last_tick_tima = tick_tima;
  }
  sb_store8_io(gb, SB_IO_DIV, SB_BFE(gb->timers.total_clock_ticks, 8, 8));
}
int sb_update_dma(sb_gb_t *gb) {
  ...
  if (gb->dma.active) {
    ...
    dma_dst &= 0x1ff0;
    dma_dst |= 0x8000;
    uint8_t dma_mode_length = sb_read8_io(gb, SB_IO_DMA_MODE_LEN);

    int len = (SB_BFE(dma_mode_length, 0, 7));
    bool hdma_mode = gb->dma.hdma;
    if (!hdma_mode ||
        (gb->dma.in_hblank == false && gb->lcd.in_hblank == true &&
         gb->lcd.curr_scanline < SB_LCD_H - 1)) {
      while (len >= 0) {
        for (int i = 0; i < 16; ++i) {
        ...
        sb_store8_io(gb, SB_IO_DMA_SRC_LO, SB_BFE(dma_src, 0, 8));
        sb_store8_io(gb, SB_IO_DMA_SRC_HI, SB_BFE(dma_src, 8, 8));
        sb_store8_io(gb, SB_IO_DMA_DST_LO, SB_BFE(dma_dst, 0, 8));
        sb_store8_io(gb, SB_IO_DMA_DST_HI, SB_BFE(dma_dst, 8, 8));
        len--;
        if (hdma_mode)
          break;
      }
      ...
      if (!gb->dma.active)
        new_mode |= 0x80;
      sb_store8_io(gb, SB_IO_DMA_MODE_LEN, new_mode);
    }
...
static FORCE_INLINE void sb_tick_sio(sb_gb_t *gb, int delta_cycles) {
  ...
  if (active) {
    if (gb->serial.last_active == false) {
      ...
      if (fast_clock)
        gb->serial.ticks_to_complete /= 2;
    }
    bool internal_clock = SB_BFE(siocnt, 0, 1);
    if (internal_clock)
      gb->serial.ticks_to_complete -= delta_cycles;
    if (gb->serial.ticks_to_complete <= 0) {
      siocnt &= 0x7f;
      sb_store8_io(gb, SB_IO_SERIAL_CTRL, siocnt);
      sb_store8_io(gb, SB_IO_SERIAL_BYTE, 0xff);
      uint8_t i_flag = sb_read8_io(gb, SB_IO_INTER_F);
      i_flag |= (1 << 3);
      sb_store8_io(gb, SB_IO_INTER_F, i_flag);
      active = false;
...
void sb_tick(sb_emu_state_t *emu, sb_gb_t *gb, gb_scratch_t *scratch) {
  ...
  if (instructions_to_execute == 0)
    instructions_to_execute = 70224 / 2;
  ...
  for (int i = 0; i < instructions_to_execute; ++i) {
    ...
    if (dma_delta_cycles == 0) {
      ...
      if (call_interrupt == false && gb->cpu.wait_for_interrupt == false) {
        ...
        inst.impl(gb, operand1, operand2, inst.op_src1, inst.op_src2,
                  inst.flag_mask);
        if (gb->cpu.prefix_op == true)
          i--;
        ...
        cpu_delta_cycles =
            4 * ((gb->cpu.branch_taken
                      ? (inst.mcycles_branch_taken - (inst.mcycles - 1))
                      : 1));
        gb->cpu.branch_taken = false;
      } else if (call_interrupt == false &&
                 gb->cpu.wait_for_interrupt == true && request_speed_switch) {
        gb->cpu.wait_for_interrupt = false;
    ...
    int delta_cycles_after_speed =
        double_speed ? cpu_delta_cycles / 2 : cpu_delta_cycles;
    ...
    if (gb->lcd.finished_frame) {
      break;
    }
    if (total_cylces >= 70224 && emu->step_instructions == 0)
      break;
...
static bool sb_load_rom(sb_emu_state_t *emu, sb_gb_t *gb,
                        gb_scratch_t *scratch) {
  if (!sb_path_has_file_ext(emu->rom_path, ".gb") &&
      !sb_path_has_file_ext(emu->rom_path, ".gbc"))
    return false;
  if (emu->rom_size > MAX_CARTRIDGE_SIZE)
    return false;
  memset(gb, 0, sizeof(sb_gb_t));
  memset(scratch, 0, sizeof(gb_scratch_t));
  gb->cart.data = emu->rom_data;
  for (size_t i = 0; i < 32 * 1024; ++i)
    gb->mem.data[i] = gb->cart.data[i];
  // Copy Header
  for (int i = 0; i < 11; ++i) {
  ...
  gb->cart.game_boy_color = SB_BFE(gb->cart.data[0x143], 7, 1) == 1;
  gb->cart.type = gb->cart.data[0x147];

  for (int i = 0; i < sizeof(gb->lcd.color_palettes); ++i)
    gb->lcd.color_palettes[i] = 0xff;

  switch (gb->cart.type) {
  case 0:
    gb->cart.mbc_type = SB_MBC_NO_MBC;
    break;

  case 1:
  case 2:
  case 3:
    gb->cart.mbc_type = SB_MBC_MBC1;
    break;

  case 5:
  case 6:
    gb->cart.mbc_type = SB_MBC_MBC2;
    break;

  case 0x0f:
  case 0x10:
    gb->rtc.has_rtc = true;
    gb->cart.mbc_type = SB_MBC_MBC3;
    break;
  case 0x11:
  case 0x12:
  case 0x13:
  ...
  case 0x1D:
  case 0x1E:
    gb->cart.mbc_type = SB_MBC_MBC5;
    break;

  case 0x20:
    gb->cart.mbc_type = SB_MBC_MBC6;
    break;
  case 0x22:
    gb->cart.mbc_type = SB_MBC_MBC7;
    break;
  }
  ...
  case 0x1E:
  case 0x22:
    gb->cart.has_rumble = true;
    break;
  }
  switch (gb->cart.data[0x148]) {
  case 0x0:
    gb->cart.rom_size = 32 * 1024;
    break;
  case 0x1:
    gb->cart.rom_size = 64 * 1024;
    break;
  case 0x2:
    gb->cart.rom_size = 128 * 1024;
    break;
  case 0x3:
    gb->cart.rom_size = 256 * 1024;
    break;
  case 0x4:
    gb->cart.rom_size = 512 * 1024;
    break;
  case 0x5:
    gb->cart.rom_size = 1024 * 1024;
    break;
  case 0x6:
    gb->cart.rom_size = 2 * 1024 * 1024;
    break;
  case 0x7:
    gb->cart.rom_size = 4 * 1024 * 1024;
    break;
  case 0x8:
    gb->cart.rom_size = 8 * 1024 * 1024;
    break;
  case 0x52:
    gb->cart.rom_size = 1.1 * 1024 * 1024;
    break;
  case 0x53:
    gb->cart.rom_size = 1.2 * 1024 * 1024;
    break;
  case 0x54:
    gb->cart.rom_size = 1.5 * 1024 * 1024;
    break;
  default:
    gb->cart.rom_size = 32 * 1024;
    break;
  ...
  case 0x0:
    ...
    break;
  case 0x1:
    gb->cart.ram_size = 2 * 1024;
    break;
  case 0x2:
    gb->cart.ram_size = 8 * 1024;
    break;
  case 0x3:
    gb->cart.ram_size = 32 * 1024;
    break;
  case 0x4:
    gb->cart.ram_size = 128 * 1024;
    break;
  case 0x5:
    gb->cart.ram_size = 64 * 1024;
    break;
  default:
    gb->cart.ram_size = 0;
    break;
  ...
  if (data) {
    if (bytes != gb->cart.ram_size) {
      printf("Warning save file size(%zu) doesn't match size expected(%d) for "
             "the cartridge type",
  ...
  if (gb->model == SB_GB) {
    if (!emu->force_dmg_mode) {
      if (!loaded_bios)
        loaded_bios = se_load_bios_file("GBC BOOT", emu->save_file_path,
                                        "cgb_boot.bin", scratch->bios, 2304);
      if (!loaded_bios)
        loaded_bios = se_load_bios_file("GBC BOOT", emu->save_file_path,
                                        "gbc_bios.bin", scratch->bios, 2304);
      if (!loaded_bios)
        loaded_bios = se_load_bios_file("GBC BOOT", emu->save_file_path,
                                        "cgb0_boot.bin", scratch->bios, 2304);
      if (!loaded_bios)
        loaded_bios =
            se_load_bios_file("GBC BOOT", emu->save_file_path,
                              "cgb_agb_boot.bin", scratch->bios, 2304);
      if (loaded_bios) {
        gb->model = SB_GBC;
      }
    }
    if (!loaded_bios)
      loaded_bios = se_load_bios_file("DMG BOOT", emu->save_file_path,
                                      "dmg_rom.bin", scratch->bios, 256);
    if (!loaded_bios)
      loaded_bios = se_load_bios_file("DMG BOOT", emu->save_file_path,
                                      "dmg0_rom.bin", scratch->bios, 256);
    if (!loaded_bios)
      loaded_bios = se_load_bios_file("DMG BOOT", emu->save_file_path,
                                      "gb_bios.bin", scratch->bios, 256);
  } else if (gb->model == SB_GBC) {
    if (!loaded_bios)
      loaded_bios = se_load_bios_file("GBC BOOT", emu->save_file_path,
                                      "cgb_boot.bin", scratch->bios, 2304);
  ...
  if (loaded_bios) {
    ...
    sb_store8_io(gb, SB_IO_BIOS_BANK, 1);
    gb->cpu.pc = 0x100;
    gb->cpu.af = 0x01B0;
    gb->cpu.bc = 0x0013;
    gb->cpu.de = 0x00D8;
...
static uint8_t sb_read_wave_ram(sb_gb_t *gb, int index) {
  return sb_read8_io(gb, SB_IO_AUD3_WAVE_BASE + index);
}
...
static bool sb_run_ar_cheat(sb_gb_t *gb, const uint32_t *buffer,
                            uint32_t size) {
