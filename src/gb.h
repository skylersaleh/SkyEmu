...
mmio_reg_t gb_io_reg_desc[] = {
    ...
    {             SB_IO_TAC,              "TAC",  {{2, 1, "Timer Enable"}, {0, 2, "Clock Divider (0: Clk/1024 1: Clk/16 2: Clk/64 3: Clk/256)"}}},
    ...
    {        SB_IO_AUD3_VOL,         "AUD3_VOL",                                              {{5, 2, "Volume (0: Mute 1: 100% 2: 50% 3: 25%)"}}},
    ...
    {        SB_IO_GBC_KEY0,         "GBC KEY0",                                               {{0, 1, "DMG Mode (0=Normal, 1=DMG Compatible)"}}},
    ...
    {    SB_IO_DMA_MODE_LEN,     "DMA_MODE_LEN",                                     {{7, 1, "Mode (0: General 1: HDMA)"}, {0, 6, "Length*16B"}}},
    {        SB_IO_GBC_BCPS,         "GBC_BCPS", {{7, 1, "Auto Increment  (0=Disabled, 1=Increment after Writing)"}, {0, 6, "Address ($00-3F)"}}},
    {        SB_IO_GBC_BCPD,         "GBC_BCPD",                                                                                             {0}},
    {        SB_IO_GBC_OCPS,         "GBC_OCPS", {{7, 1, "Auto Increment  (0=Disabled, 1=Increment after Writing)"}, {0, 6, "Address ($00-3F)"}}},
...
static FORCE_INLINE void    sb_store8_io(sb_gb_t *gb, int addr, int value) { gb->mem.data[addr] = value; }

static FORCE_INLINE uint8_t sb_read8_direct(sb_gb_t *gb, int addr) {
  if(addr >= 0x0000 && addr <= 0x3fff) {
    if(addr < 256 || (addr >= 512 && addr < 2304)) {
      if(!sb_read8_io(gb, SB_IO_BIOS_BANK)) return gb->bios[addr];
    }
...
void sb_store8(sb_gb_t *gb, int addr, int value) {
  if(addr >= 0xff00) {
    ...
    if(addr == SB_IO_OAM_DMA) {
      ...
      if(sb_read8_io(gb, SB_IO_BIOS_BANK)) return;
    ...
    case SB_MBC_MBC1:
      ...
      if(!gb->cart.mapped_rom_bank) gb->cart.mapped_rom_bank = 1;
      break;
    case SB_MBC_MBC2:
      ...
      if(mode == 2) {
        ...
        if(clock_num % 2) {
          ...
          if(yc <= y && yc + sprite_h > y && gb->lcd.sprite_index < SB_SPRITES_PER_SCANLINE && sprite_id < num_sprites && draw_sprite) {
            ...
            for(int i = 0; i < 4; ++i) {
              gb->lcd.render_sprites_data[gb->lcd.sprite_index][i] = sb_read8_io(gb, sprite_base + i);
            }
            gb->lcd.sprite_index++;
          }
        }
