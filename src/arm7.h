...
typedef uint32_t (*arm_read32_seq_fn_t)(void *user_data, uint32_t address,
                                        bool is_sequential);
typedef uint32_t (*arm_read16_seq_fn_t)(void *user_data, uint32_t address,
                                        bool is_sequential);
typedef uint8_t (*arm_read8_fn_t)(void *user_data, uint32_t address);
typedef void (*arm_write32_fn_t)(void *user_data, uint32_t address,
                                 uint32_t data);
typedef void (*arm_write16_fn_t)(void *user_data, uint32_t address,
                                 uint16_t data);
typedef void (*arm_write8_fn_t)(void *user_data, uint32_t address,
                                uint8_t data);
typedef uint32_t (*arm_coproc_read_fn_t)(void *user_data, int coproc,
                                         int opcode, int Cn, int Cm, int Cp);
typedef void (*arm_coproc_write_fn_t)(void *user_data, int coproc, int opcode,
                                      int Cn, int Cm, int Cp, uint32_t data);
...
typedef struct {
  ...
  */
  ...
  uint32_t
      step_instructions; // Instructions to step before triggering a breakpoint
...
// This function initializes the internal state needed for the arm7 core
// emulation
static arm7_t arm7_init(void *user_data);
static void arm7_exec_instruction(arm7_t *cpu);

// Write the dissassembled opcode from mem_address into the out_disasm string up
// to out_size characters
static void arm7_get_disasm(arm7_t *cpu, uint32_t mem_address, char *out_disasm,
                            size_t out_size);
// Used to send an interrupt to the emulated CPU. The n'th set bit triggers the
// n'th interrupt
...
static FORCE_INLINE void arm9_signed_halfword_multiply(arm7_t *cpu,
                                                       uint32_t opcode);
static FORCE_INLINE void arm9_single_word_transfer(arm7_t *cpu,
                                                   uint32_t opcode);
...
static FORCE_INLINE uint32_t arm7_reg_read_r15_adj(arm7_t *cpu, unsigned reg,
                                                   int r15_off);
static FORCE_INLINE void arm7_reg_write(arm7_t *cpu, unsigned reg,
                                        uint32_t value);
static FORCE_INLINE unsigned arm7_reg_index(arm7_t *cpu, unsigned reg);
static int
arm_lookup_arm_instruction_class(const arm7_instruction_t *instruction_table,
                                 uint32_t opcode_key);
static int
arm_lookup_thumb_instruction_class(const arm7_instruction_t *instruction_table,
                                   uint32_t opcode_key);
static FORCE_INLINE uint32_t arm7_shift(arm7_t *arm, uint32_t opcode,
                                        uint64_t value, uint32_t shift_value,
                                        int *carry);
static FORCE_INLINE uint32_t arm7_load_shift_reg(arm7_t *arm, uint32_t opcode,
                                                 int *carry);
...
#define ARM7_BFE(VALUE, BITOFFSET, SIZE)                                       \
...
const static arm7_instruction_t arm7_instruction_classes[] = {
    ...
    // These duplications are to handle disambiguating bit 5 and 7 set to ones
    // for DP
...
const static arm7_instruction_t arm9_instruction_classes[] = {
    ...
    {arm9_double_word_transfer, "LDRD/STRD",
     "cccc000PUIW0nnnnddddoooo11S1oooo"},
    {arm7_half_word_transfer, "HDT(h)", "cccc000PUIWLnnnndddd00001011mmmm"},
    {arm7_half_word_transfer, "HDT(sb)", "cccc000PUIW1nnnnddddOOOO1101OOOO"},
    {arm7_half_word_transfer, "HDT(sh)", "cccc000PUIW1nnnnddddOOOO1111OOOO"},
...
static FORCE_INLINE unsigned arm7_reg_index(arm7_t *cpu, unsigned reg) {
  ...
  const static int8_t lookup[10 * 16 + 8] = {
      -1, -1, -1, -1, -1, -1, -1, -1, // 8 extra padding to remove the need to
                                      // -8 from computation
...
static FORCE_INLINE void arm9_reg_write_r15_thumb(arm7_t *cpu, unsigned reg,
                                                  uint32_t value) {
  cpu->registers[arm7_reg_index(cpu, reg)] = value;
  if (SB_UNLIKELY(reg == PC))
    arm7_set_thumb_bit(cpu, value & 1);
}
static FORCE_INLINE uint32_t arm7_reg_read(arm7_t *cpu, unsigned reg) {
...
arm_lookup_arm_instruction_class(const arm7_instruction_t *instruction_table,
  ...
  for (int c = 0; instruction_table[c].handler; ++c) {
    ...
    for (int bit = 0; bit < sizeof(key_bits) / sizeof(key_bits[0]); ++bit) {
      ...
      matches &=
          instruction_table[c].bitfield[31 - b_off] != '1' || bit_value == true;
      matches &= instruction_table[c].bitfield[31 - b_off] != '0' ||
                 bit_value == false;
      if (!matches)
        break;
    }
    if (matches) {
      if (matched_class != -1) {
        ...
        printf(
            "ARM7: Class %s and %s have ambiguous encodings for: %s %08x %d\n",
            instruction_table[c].name, instruction_table[matched_class].name,
...
arm_lookup_thumb_instruction_class(const arm7_instruction_t *instruction_table,
  ...
  for (int c = 0; instruction_table[c].handler; ++c) {
    ...
    for (int bit = 0; bit < sizeof(key_bits) / sizeof(key_bits[0]); ++bit) {
      ...
      int b_off = key_bits[bit];
      matches &=
          instruction_table[c].bitfield[15 - b_off] != '1' || bit_value == true;
      matches &= instruction_table[c].bitfield[15 - b_off] != '0' ||
                 bit_value == false;
      if (!matches)
        break;
    }
...
static arm7_t arm7_init(void *user_data) {
  ...
  for (int i = 0; i < 4096; ++i) {
    int inst_class =
        arm_lookup_arm_instruction_class(arm7_instruction_classes, i);
    arm7_lookup_table[i] =
        inst_class == -1 ? NULL : arm7_instruction_classes[inst_class].handler;
    arm7_disasm_lookup_table[i] =
        inst_class == -1 ? NULL : arm7_instruction_classes[inst_class].name;
  }
  for (int i = 0; i < 4096; ++i) {
    int inst_class =
        arm_lookup_arm_instruction_class(arm9_instruction_classes, i);
    arm9_lookup_table[i] =
        inst_class == -1 ? NULL : arm9_instruction_classes[inst_class].handler;
    arm9_disasm_lookup_table[i] =
        inst_class == -1 ? NULL : arm9_instruction_classes[inst_class].name;
  }
  // Generate Thumb Lookup Table
  for (int i = 0; i < 256; ++i) {
    int inst_class =
        arm_lookup_thumb_instruction_class(arm7t_instruction_classes, i);
    arm7t_lookup_table[i] =
        inst_class == -1 ? NULL : arm7t_instruction_classes[inst_class].handler;
    arm7t_disasm_lookup_table[i] =
        inst_class == -1 ? NULL : arm7t_instruction_classes[inst_class].name;
  }
  // Generate Thumb Lookup Table
  for (int i = 0; i < 256; ++i) {
    int inst_class =
        arm_lookup_thumb_instruction_class(arm9t_instruction_classes, i);
    arm9t_lookup_table[i] =
        inst_class == -1 ? NULL : arm9t_instruction_classes[inst_class].handler;
    arm9t_disasm_lookup_table[i] =
        inst_class == -1 ? NULL : arm9t_instruction_classes[inst_class].name;
  }
...
static FORCE_INLINE void arm7_process_interrupts(arm7_t *cpu,
  ...
  if (I == 0 && cpu->phased_op_id == 0) {
    if (SB_UNLIKELY(cpu->log_cmp_file || interrupts == 0))
      return; // Log drives interrupts when enabled
...
static void arm9_get_disasm(arm7_t *cpu, uint32_t mem_address, char *out_disasm,
                            size_t out_size) {
  out_disasm[0] = '\0';
  ...
  if (arm7_get_thumb_bit(cpu) == false) {
    ...
    const char *cond_code_table[16] = {"EQ", "NE", "CS", "CC", "MI", "PL",
                                       "VS", "VC", "HI", "LS", "GE", "LT",
                                       "GT", "LE", "",   "INV"};
    ...
    int instr_class =
        arm_lookup_arm_instruction_class(arm9_instruction_classes, key);
    name = instr_class == -1 ? "INVALID"
                             : arm9_instruction_classes[instr_class].name;
    key_str =
        instr_class == -1 ? "" : arm9_instruction_classes[instr_class].bitfield;
    // Get more information for the DP class instruction
    if (strcmp(name, "DP") == 0) {
      const char *op_name[] = {"AND", "EOR", "SUB", "RSB", "ADD", "ADC",
    ...
    int instr_class =
        arm_lookup_thumb_instruction_class(arm9t_instruction_classes, key);
    name = instr_class == -1 ? "INVALID"
                             : arm9t_instruction_classes[instr_class].name;
    key_str = instr_class == -1
                  ? ""
                  : arm9t_instruction_classes[instr_class].bitfield;
  }
  ...
  while (key_str[key_off]) {
    ...
    if (letter_handled[letter] == false) {
      ...
      offset += snprintf(out_disasm + offset, out_size - offset, "%c:%d ",
                         letter, value);
    }
    ++key_off;
  }
...
static FORCE_INLINE bool arm7_check_cond_code(arm7_t *cpu, uint32_t opcode) {
  ...
  case 0x1:
    return !Z; // NE: !Equal
  case 0x2:
    return C; // CS: Unsigned >=
  case 0x3:
    return !C; // CC: Unsigned <
  case 0x4:
...
static void arm_check_log_file(arm7_t *cpu) {
  ...
  if (fread(cmp_regs, 18 * 4, 1, cpu->log_cmp_file) == 1) {
    ...
    if (!matches) {
      ...
      char *rnames[18] = {"R0",  "R1",  "R2",  "R3",  "R4",   "R5",
                          "R6",  "R7",  "R8",  "R9",  "R10",  "R11",
                          "R12", "R13", "R14", "R15", "CPSR", "SPSR"};
      ...
      for (int i = 0; i < 18; ++i) {
        if (regs[i] != cmp_regs[i])
          printf("%s %d (%08x) Log Value = %d (%08x) Prev Value =%d (%08x)\n",
                 rnames[i], regs[i], regs[i], cmp_regs[i], cmp_regs[i],
                 prev_regs[i], prev_regs[i]);
        else
          printf("Matches %s %d (%08x) Prev Value =%d (%08x)\n", rnames[i],
                 regs[i], regs[i], prev_regs[i], prev_regs[i]);
      }
      uint32_t log_cpsr = cmp_regs[16];
      uint32_t prev_cpsr = prev_regs[16];

      printf("N:%d LogN:%d PrevN:%d Z:%d LogZ:%d PrevZ:%d C:%d LogC:%d "
             "PrevC:%d V:%d LogV:%d PrevV:%d\n",
             ARM7_BFE(cpsr, 31, 1), ARM7_BFE(log_cpsr, 31, 1),
             ARM7_BFE(prev_cpsr, 31, 1), ARM7_BFE(cpsr, 30, 1),
             ARM7_BFE(log_cpsr, 30, 1), ARM7_BFE(prev_cpsr, 30, 1),
             ARM7_BFE(cpsr, 29, 1), ARM7_BFE(log_cpsr, 29, 1),
             ARM7_BFE(prev_cpsr, 29, 1), ARM7_BFE(cpsr, 28, 1),
             ARM7_BFE(log_cpsr, 28, 1), ARM7_BFE(prev_cpsr, 28, 1));
      // Set CPSR first
      ...
      if (thumb) {
        cpu->registers[PC] &= ~1;
        cpu->prefetch_opcode[0] =
            cpu->read16_seq(cpu->user_data, cpu->registers[PC] + 0, false);
        cpu->prefetch_opcode[1] =
            cpu->read16_seq(cpu->user_data, cpu->registers[PC] + 2, true);
        cpu->prefetch_opcode[2] =
            cpu->read16_seq(cpu->user_data, cpu->registers[PC] + 4, true);
      } else {
        cpu->registers[PC] &= ~3;
        cpu->prefetch_opcode[0] =
            cpu->read32_seq(cpu->user_data, cpu->registers[PC] + 0, false);
        cpu->prefetch_opcode[1] =
            cpu->read32_seq(cpu->user_data, cpu->registers[PC] + 4, true);
        cpu->prefetch_opcode[2] =
            cpu->read32_seq(cpu->user_data, cpu->registers[PC] + 8, true);
      }
    }
  } else {
    printf("Log finished!\n");
  ...
  if (thumb == false) {
    uint32_t key = ((opcode >> 4) & 0xf) | ((opcode >> 16) & 0xff0);
    printf("ARM OP: %08x PC: %08x %s Binary: ", opcode, cpu->registers[PC],
           disasm);
    arm7_print_binary(opcode, 32);
    printf("\n");
  } else {
    uint32_t key = ((opcode >> 8) & 0xff);
    printf("THUMB OP: %04x PC: %08x %s Binary: ", opcode, cpu->registers[PC],
           disasm);
...
static FORCE_INLINE void arm7_fill_pipeline(arm7_t *cpu) {
  ...
  if (thumb) {
    cpu->registers[PC] &= ~1;
    cpu->prefetch_opcode[cpu->phase] = cpu->read16_seq(
        cpu->user_data, cpu->registers[PC] + 2 * cpu->phase, cpu->phase != 0);
  } else {
    cpu->registers[PC] &= ~3;
    cpu->prefetch_opcode[cpu->phase] = cpu->read32_seq(
        cpu->user_data, cpu->registers[PC] + 4 * cpu->phase, cpu->phase != 0);
  }
  ++cpu->phase;
  if (cpu->phase != 2)
    return;
  cpu->debug_branch_ring[(cpu->debug_branch_ring_offset++) %
                         ARM_DEBUG_BRANCH_RING_SIZE] = cpu->registers[PC];
  cpu->phase = 0;
  cpu->phased_op_id = 0;
...
static FORCE_INLINE void arm7_exec_instruction(arm7_t *cpu) {
  ...
  if (thumb == false) {
    if (SB_LIKELY(cpu->prefetch_pc == cpu->registers[PC]))
      cpu->prefetch_opcode[2] = cpu->read32_seq(
          cpu->user_data, cpu->registers[PC] + 8, cpu->next_fetch_sequential);
    else
      cpu->phased_op_id = ARM_PHASED_FILL_PIPE;
  } else {
    if (SB_LIKELY(cpu->prefetch_pc == cpu->registers[PC]))
      cpu->prefetch_opcode[2] = cpu->read16_seq(
          cpu->user_data, cpu->registers[PC] + 4, cpu->next_fetch_sequential);
    else
      cpu->phased_op_id = ARM_PHASED_FILL_PIPE;
  }
...
static FORCE_INLINE uint32_t arm7_rotr(uint32_t value, uint32_t rotate) {
  return ((uint64_t)value >> (rotate & 31)) |
         ((uint64_t)value << (32 - (rotate & 31)));
}
static FORCE_INLINE uint32_t arm7_load_shift_reg(arm7_t *arm, uint32_t opcode,
                                                 int *carry) {
...
static FORCE_INLINE uint32_t arm7_shift(arm7_t *arm, uint32_t opcode,
  ...
  // If this byte is zero, the unchanged contents of Rm will be used as the
  // second operand, and the old value of the CPSR C flag will be passed on as
  // the shifter carry output.
  ...
  case 3:
    if (shift_value == 0) {
      ...
      // Rotate Extended (RRX)
      *carry = ARM7_BFE(value, 0, 1);
      value = (value >> 1) | (C << 31);

    } else {
      // Rotate
      value = arm7_rotr(value, shift_value);
      *carry = ARM7_BFE(value, 31, 1);
    }
    break;
  }
  return value;
...
static FORCE_INLINE void arm7_data_processing(arm7_t *cpu, uint32_t opcode) {
  // If it's used as anything but the shift amount in an operation with a
  // register-specified shift, r15 will be PC + 12 I.e. add r0, r15, r15, lsl
  // r15 would set r0 to PC + 12 + ((PC + 12) << (PC + 8))
  ...
  if (opcode & ((1 << 25) | (0xff0))) {
    ...
    if (I) {
      ...
      if (ARM7_BFE(opcode, 4, 1)) {
        ...
        shift_value = ARM7_BFE(opcode, 7, 5);
      uint32_t value =
          arm7_reg_read_r15_adj(cpu, ARM7_BFE(opcode, 0, 4), r15_off);
  ...
  switch (op) {
  /*AND*/ case 0:
    arm7_reg_write(cpu, Rd, result = Rn & Rm);
    break;
  /*EOR*/ case 1:
    arm7_reg_write(cpu, Rd, result = Rn ^ Rm);
    break;
  /*SUB*/ case 2:
    ...
    break;
  /*RSB*/ case 3:
    arm7_reg_write(cpu, Rd, result = Rm - Rn);
    break;
  /*ADD*/ case 4:
    arm7_reg_write(cpu, Rd, result = Rn + Rm);
    break;
  /*ADC*/ case 5:
    arm7_reg_write(cpu, Rd,
                   result = Rn + Rm + ARM7_BFE(cpu->registers[CPSR], 29, 1));
    break;
  /*SBC*/ case 6:
    arm7_reg_write(
        cpu, Rd, result = Rn - Rm + ARM7_BFE(cpu->registers[CPSR], 29, 1) - 1);
    break;
  /*RSC*/ case 7:
    arm7_reg_write(
        cpu, Rd, result = Rm - Rn + ARM7_BFE(cpu->registers[CPSR], 29, 1) - 1);
    break;
  /*TST*/ case 8:
    result = Rn & Rm;
    break;
  ...
  /*ORR*/ case 12:
    ...
    break;
  /*MOV*/ case 13:
    arm7_reg_write(cpu, Rd, result = Rm);
    break;
  /*BIC*/ case 14:
    arm7_reg_write(cpu, Rd, result = Rn & ~Rm);
    break;
  ...
  if (S) {
    // Rd is not valid for TST, TEQ, CMP, or CMN
    {
      ...
      /*CMP*/ case 10:
        ...
        // if (Rn has a different sign as Rm and result has a differnt sign to
        // Rn)
      ...
      /*RSC*/ case 7:
        C = !ARM7_BFE(result, 32, 1);
        // if (Rm has a different sign as Rn and result has a differnt sign to
        // Rm)
    ...
    if (Rd == 15) {
      // When Rd is R15 and the S flag is set the result of the operation is
      // placed in R15 and the SPSR corresponding to the current mode is moved
      // to the CPSR. This allows state changes which atomically restore both PC
      // and CPSR. This form of instruction should not be used in User mode.
...
static FORCE_INLINE void arm7_single_data_swap(arm7_t *cpu, uint32_t opcode) {
  ...
  // Load
  uint32_t read_data =
      B ? cpu->read8(cpu->user_data, addr)
        : arm7_rotr(cpu->read32(cpu->user_data, addr), (addr & 0x3) * 8);
...
static FORCE_INLINE void arm9_branch_link_exchange(arm7_t *cpu,
                                                   uint32_t opcode) {
  int v = arm7_reg_read_r15_adj(cpu, ARM7_BFE(opcode, 0, 4), 4);
...
static FORCE_INLINE void arm7_half_word_transfer(arm7_t *cpu, uint32_t opcode) {
  ...
  int offset = I == 0 ? arm7_reg_read(cpu, ARM7_BFE(opcode, 0, 4))
  ...
  if (L == 1) { // Load
    uint32_t data =
        H ? arm7_rotr(cpu->read16(cpu->user_data, addr), (addr & 0x1) * 8)
          : cpu->read8(cpu->user_data, addr);
    if (S) {
      data &= 0xffff;
...
static FORCE_INLINE void arm7_single_word_transfer(arm7_t *cpu,
                                                   uint32_t opcode) {
...
static FORCE_INLINE void arm9_double_word_transfer(arm7_t *cpu,
                                                   uint32_t opcode) {
...
static FORCE_INLINE void arm7_block_transfer(arm7_t *cpu, uint32_t opcode) {
  ...
  if (cpu->phase == 0) {
    ...
    int base_addr = addr;
    if (reglist == 0) {
      // Handle Empty Rlist case: R15 loaded/stored (ARMv4 only), and
      // Rb=Rb+/-40h (ARMv4-v5).
      reglist = 1 << 15;
      num_regs = 16;
    }
  ...
  for (int i = cpu->phase; i < 16; ++i) {
    ...
    // Inexplicablly SRAM accesses are not DWORD aligned. GBA suite memory test
    // can be used to verify this.
    if ((a & 0xfe000000) != 0x0e000000)
      a &= ~3;
    if (!L)
      cpu->write32(cpu->user_data, a,
                   cpu->registers[reg_index] +
                       (i == 15 ? cpu->block.r15_off : 0));
    ...
    if (L) {
      ...
      cpu->registers[reg_index] =
          cpu->read32_seq(cpu->user_data, a, bank == cpu->block.last_bank);
...
static FORCE_INLINE void arm9_block_transfer(arm7_t *cpu, uint32_t opcode) {
  ...
  // Writeback happens on second cycle
  if ((reglist >= (1 << (Rn + 1)) || cpu->block.num_regs <= 1 ||
       reglist == 0) &&
      w) {
    arm7_reg_write(cpu, Rn, cpu->block.base_addr);
  }
...
static FORCE_INLINE void arm7_coproc_data_transfer(arm7_t *cpu,
                                                   uint32_t opcode) {
...
static FORCE_INLINE void arm7_coproc_reg_transfer(arm7_t *cpu,
                                                  uint32_t opcode) {
  ...
  if (coprocessor_read) {
    if (!cpu->coprocessor_read) {
      printf("Coprocessor Read Issued without bound coprocessor_read handler: "
             "%x\n",
             opcode);
      return;
    }
    uint32_t data = cpu->coprocessor_read(cpu->user_data, Pn,
                                          coprocessor_opcode, Cn, Cm, Cp);
    arm7_reg_write(cpu, Rd, data);
  } else {
    if (!cpu->coprocessor_write) {
      printf("Coprocessor Write Issued without bound coprocessor_write "
             "handler: %x\n",
             opcode);
      return;
    }
    uint32_t data = arm7_reg_read_r15_adj(cpu, Rd, 8);
    cpu->coprocessor_write(cpu->user_data, Pn, coprocessor_opcode, Cn, Cm, Cp,
                           data);
  }
}
static FORCE_INLINE void arm7_software_interrupt(arm7_t *cpu, uint32_t opcode) {
  ...
  int id = -1;
  for (int i = 0; i < ARM_DEBUG_SWI_RING_SIZE && i < cpu->debug_swi_ring_offset;
       ++i) {
...
static FORCE_INLINE void arm7t_mov_shift_reg(arm7_t *cpu, uint32_t opcode) {
  ...
  opcode =
      (0xD << 21) | (1 << 20) | (Rd << 12) | (offset << 7) | (op << 5) | (Rs);
...
static FORCE_INLINE void arm7t_add_sub(arm7_t *cpu, uint32_t opcode) {
  ...
  uint32_t arm_op =
      (I << 25) | (op << 21) | (1 << 20) | (Rs << 16) | (Rd << 12) | (Rn);
  arm7_data_processing(cpu, arm_op);
}
static FORCE_INLINE void arm7t_mov_cmp_add_sub_imm(arm7_t *cpu,
                                                   uint32_t opcode) {
  ...
  uint32_t arm_op =
      (1 << 25) | (op << 21) | (1 << 20) | (Rd << 16) | (Rd << 12) | (imm);
  arm7_data_processing(cpu, arm_op);
}
static FORCE_INLINE void arm7t_alu_op(arm7_t *cpu, uint32_t opcode) {
  ...
  if (op == 13) {
    uint32_t arm_op =
        (0xE << 28) | (1 << 20) | (Rd << 16) | (Rd << 8) | (9 << 4) | (Rs);
    arm7_multiply(cpu, arm_op);
    cpu->registers[CPSR] &= ~(1 << 29);
    return;
  }

  int op_mapping[16] = {/*AND{S}*/ 0,
                        /*EOR{S}*/ 1,
                        /*LSL{S}*/ 13,
                        /*LSR{S}*/ 13,
                        /*ASR{S}*/ 13,
  ...
  uint32_t arm_op = (0xEu << 28) | (alu_op << 21) | (1 << 20) | (Rn << 16) |
                    (Rd << 12) | (shift_op << 5);
...
static FORCE_INLINE void arm7t_hi_reg_op(arm7_t *cpu, uint32_t opcode) {
  ...
  if (op == 3) {
    ...
    uint32_t arm_op = (op << 21) | (S << 20) | (op == 13 ? 0 : (Rd << 16)) |
                      (Rd << 12) | (Rs << 0);
    arm7_data_processing(cpu, arm_op);
  }
}
...
static FORCE_INLINE void arm7t_ldst_bh(arm7_t *cpu, uint32_t opcode) {
  ...
  case 3: // Load Sign Extended Half
    data =
        arm7_rotr(cpu->read16(cpu->user_data, addr), (addr & 0x1) * 8) & 0xffff;
    cpu->i_cycles++;
...
static FORCE_INLINE void arm7t_push_pop_reg(arm7_t *cpu, uint32_t opcode) {
  ...
  uint32_t arm_op = (0xe << 28) | (4 << 25) | (P << 24) | (U << 23) |
                    (W << 21) | (push_or_pop << 20) | (13 << 16) | r_list;
  if (include_pc_lr)
...
static FORCE_INLINE void arm7t_mult_ldst(arm7_t *cpu, uint32_t opcode) {
  ...
  uint32_t arm_op = (0xe << 28) | (4 << 25) | (P << 24) | (U << 23) |
                    (W << 21) | (write_or_read << 20) | (Rb << 16) | r_list;
  arm7_block_transfer(cpu, arm_op);
}
static FORCE_INLINE void arm9t_mult_ldst(arm7_t *cpu, uint32_t opcode) {
  bool write_or_read = ARM7_BFE(opcode, 11, 1);
