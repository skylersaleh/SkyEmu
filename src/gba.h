...
void gba_tick(sb_emu_state_t* emu, gba_t* gba,gba_scratch_t *scratch){
  ...
  // solar_sensor is a normalized [0,1] brightness (the settings slider, the
  // hotkeys, or the device light sensor). se_solar_float_to_byte() maps it onto
  // the game's "calibrated" 0..140 window and then to the inverted 8-bit trip
  // count; see se_solar_sensor.h. This corrects both endpoints: 0.0 now empties
  // the gauge, and 1.0 lands exactly on the clamp instead of saturating at ~0.77.
