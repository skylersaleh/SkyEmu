...
#include <string.h>
#include <time.h>
// Should be power of 2 for perf, 8192 samples gives ~85ms maximal latency for
// 48kHz
#define LR 14
#define PC 15
#define CPSR 16
...
// Sound Registers
#define GBA_SOUND1CNT_L                                                        \
  0x4000060 /* R/W   Channel 1 Sweep register       (NR10) */
#define GBA_SOUND1CNT_H                                                        \
  0x4000062 /* R/W   Channel 1 Duty/Length/Envelope (NR11, NR12) */
#define GBA_SOUND1CNT_X                                                        \
  0x4000064 /* R/W   Channel 1 Frequency/Control    (NR13, NR14) */
#define GBA_SOUND2CNT_L                                                        \
  0x4000068 /* R/W   Channel 2 Duty/Length/Envelope (NR21, NR22) */
#define GBA_SOUND2CNT_H                                                        \
  0x400006C /* R/W   Channel 2 Frequency/Control    (NR23, NR24) */
#define GBA_SOUND3CNT_L                                                        \
  0x4000070 /* R/W   Channel 3 Stop/Wave RAM select (NR30) */
#define GBA_SOUND3CNT_H                                                        \
  0x4000072 /* R/W   Channel 3 Length/Volume        (NR31, NR32) */
#define GBA_SOUND3CNT_X                                                        \
  0x4000074 /* R/W   Channel 3 Frequency/Control    (NR33, NR34) */
#define GBA_SOUND4CNT_L                                                        \
  0x4000078 /* R/W   Channel 4 Length/Envelope      (NR41, NR42) */
#define GBA_SOUND4CNT_H                                                        \
  0x400007C /* R/W   Channel 4 Frequency/Control    (NR43, NR44) */
#define GBA_SOUNDCNT_L                                                         \
  0x4000080 /* R/W   Control Stereo/Volume/Enable   (NR50, NR51) */
#define GBA_SOUNDCNT_H 0x4000082 /* R/W   Control Mixing/DMA Control */
#define GBA_SOUNDCNT_X                                                         \
  0x4000084 /* R/W   Control Sound on/off           (NR52) */
...
#define GBA_IR                                                                 \
  0x4000136 /* -    Ancient - Infrared Register (Prototypes only) */
...
#define GBA_IF                                                                 \
  0x4000202 /* R/W  IF        Interrupt Request Flags / IRQ Acknowledge */
...
#define GBA_POSTFLG 0x4000300 /* R/W  POSTFLG   Undocumented - Post Boot Flag  \
                               */
#define GBA_HALTCNT                                                            \
  0x4000301 /* W    HALTCNT   Undocumented - Power Down Control */
// #define GBA_?       0x4000410      /* ?    ?         Undocumented - Purpose
// Unknown / Bug ??? 0FFh */ #define GBA_?       0x4000800      /* R/W  ?
// Undocumented - Internal Memory Control (R/W) */ #define GBA_?       0x4xx0800
// /* R/W  ?         Mirrors of 4000800h (repeated each 64K) */ #define
// GBA_(3DS)   0x4700000      /* W    (3DS)     Disable ARM7 bootrom overlay
// (3DS only) */
...
mmio_reg_t gba_io_reg_desc[] = {
    ...
    {GBA_WAITCNT,
     "WAITCNT",
     {
         ...
         {11, 2,
          "PHI Terminal Output (0..3 = Disable, 4.19MHz, 8.38MHz, 16.78MHz)"},
    ...
    {GBA_POSTFLG,
     "POSTFLG",
     {0}}, /* R/W  POSTFLG   Undocumented - Post Boot Flag */
    ...
    {GBA_WAVE_RAM,
     "WAVE_RAM",
     {0}}, /* R/W Channel 3 Wave Pattern RAM (2 banks!!) */
    ...
    {GBA_DMA0CNT_H,
     "DMA0CNT_H",
     {
         ...
         {12, 2,
          "DMA Start Timing (0=Immediately, 1=VBlank, 2=HBlank, 3=Prohibited)"},
    ...
    {GBA_DMA1CNT_H,
     "DMA1CNT_H",
     {
         ...
         {10, 1, "DMA Transfer Type (0=16bit, 1=32bit)"},
         {12, 2,
          "DMA Start Timing (0=Immediately, 1=VBlank, 2=HBlank, 3=Sound)"},
         {14, 1, "IRQ upon end of Word Count (0=Disable, 1=Enable)"},
         {15, 1, "DMA Enable (0=Off, 1=On)"},
     }},                               /* R/W  DMA 1 Control */
    ...
    {GBA_IR,
     "IR",
     {0}}, /* -    Ancient - Infrared Register (Prototypes only) */
...
typedef struct {
  ...
  // Tracks the place of the squashable pipeline bubble that enters the pipeline
  // after a single cycle data read Each bit represents one clock cycle in time
  // with bit 0 meaning the current cycle of the last stage of the CPU pipeline
  // and larger bits indicating future cycles.

  // This is shifted to the right every clock cycle to mimic the bubble
  // traveling through the pipeline. Some complexity is introduced because this
  // bubble may be squashed by upstream push back. This is modelled by extra
  // shift rights for the multicycling of the various stages.
...
static uint8_t gba_audio_process_byte_write(gba_t *gba, uint32_t addr,
                                            uint8_t value);
static bool gba_run_ar_cheat(gba_t *gba, const uint32_t *buffer, uint32_t size);
static FORCE_INLINE void gba_recompute_waitstate_table(gba_t *gba,
                                                       uint16_t waitcnt);
...
static uint32_t gba_save_best_effort_state(gba_t *gba) {
  ...
  gba->bess.cart_backup_seg =
      ((uint8_t *)gba->mem.cart_backup) - (uint8_t *)gba;
  for (int i = 0; i < 4; ++i)
    gba->bess.timer_reload_values[i] = gba->timers[i].reload_value;
  return ((uint8_t *)&gba->bess) - (uint8_t *)(gba);
}
static bool gba_load_best_effort_state(gba_t *gba, uint8_t *save_state_data,
                                       uint32_t size, uint32_t bess_offset) {
  if (bess_offset + sizeof(gba_bess_info_t) > size)
    return false;
  gba_bess_info_t *bess = (gba_bess_info_t *)(save_state_data + bess_offset);
  if (bess->bess_version != 1)
    return false;
  if (bess->wram0_seg + sizeof(gba->mem.wram0) > size)
    return false;
  if (bess->wram1_seg + sizeof(gba->mem.wram1) > size)
    return false;
  if (bess->io_seg + sizeof(gba->mem.io) > size)
    return false;
  if (bess->palette_seg + sizeof(gba->mem.palette) > size)
    return false;
  if (bess->vram_seg + sizeof(gba->mem.vram) > size)
    return false;
  if (bess->oam_seg + sizeof(gba->mem.oam) > size)
    return false;
  if (bess->cart_backup_seg + sizeof(gba->mem.cart_backup) > size)
    return false;
  for (int i = 0; i < 37; ++i)
    gba->cpu.registers[i] = bess->cpu_registers[i];
  memcpy(gba->mem.wram0, save_state_data + bess->wram0_seg,
         sizeof(gba->mem.wram0));
  memcpy(gba->mem.wram1, save_state_data + bess->wram1_seg,
         sizeof(gba->mem.wram1));
  memcpy(gba->mem.io, save_state_data + bess->io_seg, sizeof(gba->mem.io));
  memcpy(gba->mem.palette, save_state_data + bess->palette_seg,
         sizeof(gba->mem.palette));
  memcpy(gba->mem.vram, save_state_data + bess->vram_seg,
         sizeof(gba->mem.vram));
  memcpy(gba->mem.oam, save_state_data + bess->oam_seg, sizeof(gba->mem.oam));
  memcpy(gba->mem.cart_backup, save_state_data + bess->cart_backup_seg,
         sizeof(gba->mem.cart_backup));
  for (int i = 0; i < 4; ++i)
    gba->timers[i].pending_reload_value = gba->timers[i].reload_value =
        bess->timer_reload_values[i];

  gba_recompute_waitstate_table(gba, gba_read32(gba, GBA_WAITCNT));
  return true;
}

static FORCE_INLINE sb_debug_mmio_access_t
gba_debug_mmio_access(gba_t *gba, unsigned baddr, int trigger_breakpoint) {
  baddr &= 0xffff;
  baddr /= 4;
...
static FORCE_INLINE uint32_t *gba_dword_lookup(gba_t *gba, unsigned baddr,
                                               int req_type);
static FORCE_INLINE uint32_t gba_read32(gba_t *gba, unsigned baddr) {
...
static FORCE_INLINE uint8_t gba_read8_debug(gba_t *gba, unsigned baddr) {
  uint32_t *val =
      gba_dword_lookup(gba, baddr, GBA_REQ_READ | GBA_REQ_1B | GBA_REQ_DEBUG);
  int offset = SB_BFE(baddr, 0, 2);
  return ((uint8_t *)val)[offset];
}
static FORCE_INLINE void
gba_process_flash_state_machine(gba_t *gba, unsigned baddr, uint8_t data) {
...
#define FLASH_SET_BANK 0xB0
  ...
  case FLASH_DEFAULT:
    if (baddr == 0x5555 && data == 0xAA)
      gba->cart.flash_state = FLASH_RECV_AA;
    break;
  case FLASH_RECV_AA:
    if (baddr == 0x2AAA && data == 0x55)
      gba->cart.flash_state = FLASH_RECV_55;
    break;
  case FLASH_RECV_55:
    if (baddr == 0x5555) {
      // Process command
      switch (data) {
      case FLASH_ENTER_CHIP_ID:
        gba->cart.in_chip_id_mode = true;
        break;
      case FLASH_EXIT_CHIP_ID:
        gba->cart.in_chip_id_mode = false;
        break;
      case FLASH_PREP_ERASE:
        gba->cart.flash_state = FLASH_PREP_ERASE;
        break;
      case FLASH_WRITE_BYTE:
        gba->cart.flash_state = FLASH_WRITE_BYTE;
        break;
      case FLASH_SET_BANK:
        gba->cart.flash_state = FLASH_SET_BANK;
        break;
      default:
  ...
  case FLASH_PREP_ERASE:
    if (baddr == 0x5555 && data == 0xAA)
      gba->cart.flash_state = FLASH_ERASE_RECV_AA;
    break;
  case FLASH_ERASE_RECV_AA:
    if (baddr == 0x2AAA && data == 0x55)
      gba->cart.flash_state = FLASH_ERASE_RECV_55;
    break;
  case FLASH_ERASE_RECV_55:
    if (baddr == 0x5555 || data == FLASH_ERASE_4KB) {
      int size = gba->cart.backup_type == GBA_BACKUP_FLASH_64K ? 64 * 1024
                                                               : 128 * 1024;
      int erase_4k_off =
          gba->cart.flash_bank * 64 * 1024 + SB_BFE(baddr, 12, 4) * 4096;
      // Process command
      switch (data) {
...
#define GBA_RTC_UNUSED3 7
  ...
  if (cs == 0) {
    ...
    time_t time_secs = gba->rtc.initial_rtc_time +
                       (gba->rtc.total_clocks_ticked) / (16 * 1024 * 1024);
    ...
    if (new_bit) {
      ...
      if (gba->rtc.serial_bits_clocked == 8) {
        // Check whether the command should be interpreted MSB-first or
        // LSB-first.
        gba->rtc.input_register &= 0xff;
        if ((gba->rtc.input_register >> 4) == 6) {
          gba->rtc.input_register =
              (gba->rtc.input_register << 4) | (gba->rtc.input_register >> 4);
          gba->rtc.input_register &= 0xff;
          gba->rtc.input_register = ((gba->rtc.input_register & 0x33) << 2) |
                                    ((gba->rtc.input_register & 0xCC) >> 2);
          gba->rtc.input_register &= 0xff;
          gba->rtc.input_register = ((gba->rtc.input_register & 0x55) << 1) |
                                    ((gba->rtc.input_register & 0xAA) >> 1);
          gba->rtc.input_register &= 0xff;
        }
      ...
      case GBA_RTC_STATUS: {
        if (gba->rtc.serial_bits_clocked == 8)
          gba->rtc.output_register = gba->rtc.status_register;
      ...
      case GBA_RTC_TIME: {
        if (gba->rtc.serial_bits_clocked == 8)
          gba->rtc.output_register = ((uint64_t)(hour & 0xff) << (0 * 8)) |
...
static FORCE_INLINE void gba_process_backup_write(gba_t *gba, unsigned baddr,
                                                  uint32_t data) {
...
static void gba_process_solar_sensor(gba_t *gba) {
  ...
  if (!cs) {
    ...
    // printf("DAC Value: %d clk: %d rst: %d dir:%d
...
static FORCE_INLINE void gba_store32(gba_t *gba, unsigned baddr,
                                     uint32_t data) {
  if (baddr >= 0x08000000) {
    ...
    if (baddr >= 0x080000C4 && baddr < 0x080000C8) {
      ...
      gba->mem.cart_rom[0x0000C4] =
          gba->cart.gpio_data & ~SB_BFE(gba->cart.gpio_data, 16, 16);
...
static FORCE_INLINE void gba_store16(gba_t *gba, unsigned baddr,
                                     uint32_t data) {
  if (baddr >= 0x08000000) {
    // Mask is 0xfe to catch the sram mirror at 0x0f and 0x0e
    if ((baddr & 0xfe000000) == 0xE000000) {
    ...
    if (baddr >= 0x080000C4 && baddr < 0x080000C8) {
      ...
      if (addr == 0x080000c4)
        gba->cart.gpio_data =
            (gba->cart.gpio_data & 0xffff0000) | (data & 0xffff);
      if (addr == 0x080000c6)
        gba->cart.gpio_data =
            (gba->cart.gpio_data & 0x0000ffff) | ((data & 0xffff) << 16);
      gba->mem.cart_rom[0x0000C4] =
          gba->cart.gpio_data & ~SB_BFE(gba->cart.gpio_data, 16, 16);
      gba_process_solar_sensor(gba);
      gba_process_rtc_state_machine(gba);
...
static FORCE_INLINE void gba_store8(gba_t *gba, unsigned baddr, uint32_t data) {
  if (baddr >= 0x05000000) {
    ...
    if (((baddr & 0xff000000) == 0x06000000) &&
        ((baddr & 0x1ffff) <= 0x0013FFF)) {
      gba_store16(gba, baddr & ~1, (data & 0xff) * 0x0101);
      return;
    }
    // Mask is 0xfe to catch the sram mirror at 0x0f and 0x0e
...
static FORCE_INLINE void gba_store8_debug(gba_t *gba, unsigned baddr,
                                          uint32_t data) {
  if (baddr >= 0x05000000) {
    // 8 bit stores to palette mirror across 8 bit halves
    if ((baddr & 0xff000000) == 0x5000000) {
  ...
  uint32_t *val =
      gba_dword_lookup(gba, baddr, GBA_REQ_WRITE | GBA_REQ_1B | GBA_REQ_DEBUG);
  int offset = SB_BFE(baddr, 0, 2);
  ((uint8_t *)val)[offset] = data;
}
static FORCE_INLINE void gba_io_store8(gba_t *gba, unsigned baddr,
                                       uint8_t data) {
  gba->mem.io[baddr & 0xfff] = data;
}
static FORCE_INLINE void gba_io_store16(gba_t *gba, unsigned baddr,
                                        uint16_t data) {
  *(uint16_t *)(gba->mem.io + (baddr & 0xfff)) = data;
}
static FORCE_INLINE void gba_io_store32(gba_t *gba, unsigned baddr,
                                        uint32_t data) {
  *(uint32_t *)(gba->mem.io + (baddr & 0xfff)) = data;
}

static FORCE_INLINE uint8_t gba_io_read8(gba_t *gba, unsigned baddr) {
  return gba->mem.io[baddr & 0xfff];
}
static FORCE_INLINE uint16_t gba_io_read16(gba_t *gba, unsigned baddr) {
  return *(uint16_t *)(gba->mem.io + (baddr & 0xfff));
}
static FORCE_INLINE uint32_t gba_io_read32(gba_t *gba, unsigned baddr) {
  return *(uint32_t *)(gba->mem.io + (baddr & 0xfff));
}
static FORCE_INLINE void gba_recompute_waitstate_table(gba_t *gba,
                                                       uint16_t waitcnt) {
...
static FORCE_INLINE void
gba_compute_access_cycles(gba_t *gba, uint32_t address,
                          int request_size /*0: 1B,1: 2B,3: 4B*/) {
  ...
  if (SB_LIKELY(prefetch_en)) {
    ...
    if (bank >= 0x08 && bank <= 0x0D) {
      if (SB_UNLIKELY(request_size & 1)) {
        ...
        if (pc >= 0x08000000) {
          // Check if the bubble made it to the execute stage before being
          // squashed, and apply the bubble cycle if it was not squashed. Note,
          // only a single pipeline bubble is tracked using this infrastructure.
          ...
          if (gba->mem.prefetch_size > gba->cpu.i_cycles &&
              prefetch_phase == prefetch_cycles - 1)
            wait += 1;
        }
...
static FORCE_INLINE uint32_t gba_compute_access_cycles_dma(
    gba_t *gba, uint32_t address, int request_size /*0: 1B,1: 2B,3: 4B*/) {
  int bank = SB_BFE(address, 24, 4);
  uint32_t wait = gba->mem.wait_state_table[bank * 4 + request_size];
  return wait;
}
...
static FORCE_INLINE uint32_t arm7_read32_seq(void *user_data, uint32_t address,
                                             bool seq) {
  gba_compute_access_cycles((gba_t *)user_data, address, seq ? 2 : 3);
  return gba_read32((gba_t *)user_data, address);
}
static FORCE_INLINE uint32_t arm7_read16_seq(void *user_data, uint32_t address,
                                             bool seq) {
...
static bool gba_process_mmio_write(gba_t *gba, uint32_t address, uint32_t data,
                                   int req_size_bytes);
...
static FORCE_INLINE void gba_dma_write32(gba_t *gba, uint32_t address,
                                         uint32_t data) {
...
static FORCE_INLINE void gba_dma_write16(gba_t *gba, uint32_t address,
                                         uint16_t data) {
  if ((address & 0xfffffC00) == 0x04000000) {
...
static FORCE_INLINE void arm7_write32(void *user_data, uint32_t address,
                                      uint32_t data) {
  gba_compute_access_cycles((gba_t *)user_data, address, 3);
  gba_dma_write32((gba_t *)user_data, address, data);
}
static FORCE_INLINE void arm7_write16(void *user_data, uint32_t address,
                                      uint16_t data) {
  gba_compute_access_cycles((gba_t *)user_data, address, 1);
  gba_dma_write16((gba_t *)user_data, address, data);
}
static FORCE_INLINE void arm7_write8(void *user_data, uint32_t address,
                                     uint8_t data) {
  gba_compute_access_cycles((gba_t *)user_data, address, 1);
...
static FORCE_INLINE uint32_t *gba_dword_lookup(gba_t *gba, unsigned addr,
                                               int req_type) {
  ...
  case 0x0:
    if (addr < 0x4000) {
      if (gba->cpu.registers[15] < 0x4000)
        gba->mem.bios_word = *(uint32_t *)(gba->mem.bios + (addr & ~3));
    ...
    if (SB_LIKELY(addr <= 0x40003FF)) {
      if (req_type & GBA_REQ_READ) {
        ...
        if (SB_LIKELY(gba->mem.mmio_reg_valid_lookup[io_reg])) {
          ...
          gba->mem.mmio_word = (*(uint32_t *)(gba->mem.io + (addr & 0x3fc))) &
                               gba->mem.mmio_data_mask_lookup[io_reg];
          ret = &gba->mem.mmio_word;
        }
      } else
        ret = (uint32_t *)(gba->mem.io + (addr & 0x3fc));
      if (!(req_type & GBA_REQ_DEBUG)) {
        gba->mem.mmio_debug_access_buffer[(addr & 0xffff) / 4] |=
            (req_type & GBA_REQ_WRITE) ? 0x70 : 0xf;
  ...
  case 0x6:
    if (addr & 0x10000) {
      ...
      if (addr & 0x08000) {
        ...
        // Don't allow writes to mirrored VRAM in bitmap mode. See also
        // vram-mirror.gba Needed for Acrobat Kid. Still requires testing to
  ...
  case 0xD: {
    ...
    if (SB_UNLIKELY(maddr >= gba->cart.rom_size)) {
      gba->mem.openbus_word =
          ((maddr / 2) & 0xffff) | (((maddr / 2 + 1) & 0xffff) << 16);
  ...
  case 0xF:
    if (gba->cart.backup_type == GBA_BACKUP_SRAM) {
      ...
      if (gba->cart.in_chip_id_mode && addr <= 0xE000001) {
        ...
        gba->mem.sram_word =
            gba->mem.cart_backup[(addr & 0xffff) +
                                 gba->cart.flash_bank * 64 * 1024] *
            0x01010101;
        ret = &gba->mem.sram_word;
      }
    }
...
static FORCE_INLINE void gba_audio_fifo_push(gba_t *gba, int fifo,
                                             int8_t data) {
  int size =
      (gba->audio.fifo[fifo].write_ptr - gba->audio.fifo[fifo].read_ptr) & 0x1f;
  if (size < 28) {
    gba->audio.fifo[fifo].write_ptr =
        (gba->audio.fifo[fifo].write_ptr + 1) & 0x1f;
...
static void gba_recompute_mmio_mask_table(gba_t *gba) {
  for (int io_reg = 0; io_reg < 256; io_reg++) {
    ...
    if (dword_address == 0x4000008)
    ...
    else if (dword_address == 0x4000064 || dword_address == 0x400006C ||
             dword_address == 0x4000074)
      data_mask &= 0x4000;
      ...
      data_mask &= 0x008F;
    else if (dword_address == 0x4000088 || dword_address == 0x4000134 ||
             dword_address == 0x4000140 || dword_address == 0x4000158 ||
             dword_address == 0x4000204 || dword_address == 0x4000208)
      data_mask = 0x0000ffff;
    else if (dword_address == 0x40000B8 || dword_address == 0x40000C4 ||
             dword_address == 0x40000D0)
...
static bool gba_process_mmio_write(gba_t *gba, uint32_t address, uint32_t data,
  ...
  if (address_u32 == GBA_IE) {
  ...
  } else if (address_u32 == GBA_TM0CNT_L || address_u32 == GBA_TM1CNT_L ||
             address_u32 == GBA_TM2CNT_L || address_u32 == GBA_TM3CNT_L) {
    gba_compute_timers(gba);
    int timer_off = (address_u32 - GBA_TM0CNT_L) / 4;
    if (word_mask & 0xffff) {
      gba->timers[timer_off + 0].pending_reload_value =
          word_data & (word_mask & 0xffff);
    }
    if (word_mask & 0xffff0000) {
      gba_store16(gba, address_u32 + 2, (word_data >> 16) & 0xffff);
      gba->timers[timer_off + 0].reload_value =
          gba->timers[timer_off + 0].pending_reload_value;
    }
    ...
    gba_audio_fifo_push(gba,0,gba->audio.fifo[0].data[(gba->audio.fifo[0].write_ptr)&0x1f]);
      }
    }else if(address_u32==GBA_FIFO_B){
      for(int i=0;i<4;++i){
        if(word_mask&(0xff<<(8*i)))gba_audio_fifo_push(gba,1,SB_BFE(word_data,8*i,8));
        else
    gba_audio_fifo_push(gba,1,gba->audio.fifo[1].data[(gba->audio.fifo[1].write_ptr)&0x1f]);
      }
    */
    ...
    for (int i = 0; i < 4; ++i) {
      if (word_mask & (0xff << (i * 8))) {
        uint8_t data = gba_audio_process_byte_write(
            gba, address_u32 + i, SB_BFE(word_data, (i * 8), 8));
...
int gba_search_rom_for_backup_string(gba_t *gba) {
  ...
  for (int b = 0; b < gba->cart.rom_size; ++b) {
    const char *strings[] = {"EEPROM_", "SRAM_", "FLASH_", "FLASH512_",
                             "FLASH1M_"};
    int backup_type[] = {GBA_BACKUP_EEPROM, GBA_BACKUP_SRAM,
                         GBA_BACKUP_FLASH_64K, GBA_BACKUP_FLASH_64K,
                         GBA_BACKUP_FLASH_128K};
...
void gba_unload(gba_t *gba, gba_scratch_t *scratch) {
  ...
  if (scratch->log_cmp_file)
    fclose(scratch->log_cmp_file);
...
bool gba_load_rom(sb_emu_state_t *emu, gba_t *gba, gba_scratch_t *scratch) {
  ...
  bool loaded_bios =
      se_load_bios_file("GBA BIOS", emu->save_file_path, "gba_bios.bin",
                        scratch->bios, 16 * 1024);
  if (!loaded_bios) {
  ...
  // Setup flash chip id (this is not used if the cartridge does not have flash
  // backup storage)
  if (gba->cart.backup_type == GBA_BACKUP_FLASH_64K) {
...
static FORCE_INLINE int gba_ppu_compute_max_fast_forward(gba_t *gba,
                                                         bool render) {
  int scanline_clock = (gba->ppu.scan_clock) % 1232;
  // If inside hblank, can fastforward to outside of hblank
  if (scanline_clock >= GBA_LCD_HBLANK_START * 4 &&
      scanline_clock <= GBA_LCD_HBLANK_END * 4)
  ...
  if (not_visible &&
      (scanline_clock >= 1 && scanline_clock <= GBA_LCD_HBLANK_START * 4))
...
static FORCE_INLINE void gba_tick_ppu(gba_t *gba, bool render) {
  ...
  gba->ppu.scan_clock++;
  gba->ppu.fast_forward_ticks =
      gba_ppu_compute_max_fast_forward(gba, render) + 1;
  gba->ppu.scan_clock += gba->ppu.fast_forward_ticks;
  if (lcd_x == 0 || lcd_x == GBA_LCD_HBLANK_START ||
  ...
  bool reload_ref_points =
      lcd_x == GBA_LCD_HBLANK_END || (lcd_y == 0 && lcd_x == 0);
  if (reload_ref_points) {
    // Latch BGX and BGY registers
    for (int aff = 0; aff < 2; ++aff) {
      if (gba->ppu.aff[aff].wrote_bgx || (lcd_y == 0 && lcd_x == 0)) {
        gba->ppu.aff[aff].render_bgx =
            gba_io_read32(gba, GBA_BG2X + (aff)*0x10);
        gba->ppu.aff[aff].render_bgx =
            SB_BFE(gba->ppu.aff[aff].render_bgx, 0, 28);
        gba->ppu.aff[aff].render_bgx =
            ((int32_t)(gba->ppu.aff[aff].render_bgx << 4)) >> 4;
        gba->ppu.aff[aff].wrote_bgx = false;
      }
      if (gba->ppu.aff[aff].wrote_bgy || (lcd_y == 0 && lcd_x == 0)) {
        gba->ppu.aff[aff].render_bgy =
            gba_io_read32(gba, GBA_BG2Y + (aff)*0x10);
        gba->ppu.aff[aff].render_bgy =
            SB_BFE(gba->ppu.aff[aff].render_bgy, 0, 28);
        gba->ppu.aff[aff].render_bgy =
            ((int32_t)(gba->ppu.aff[aff].render_bgy << 4)) >> 4;
  ...
  if ((lcd_y < 159 || lcd_y == 227) && lcd_x == GBA_LCD_HBLANK_START) {
    ...
    // Render sprites over scanline when it completes
    uint8_t default_window_control = 0x3f; // bitfield [0-3:bg0-bg3 enable 4:obj
                                           // enable, 5: special effect enable]
    ...
    if (display_obj) {
      for (int o = 0; o < 128; ++o) {
        ...
        int obj_mode = SB_BFE(
            attr0, 10,
            2); //(0=Normal, 1=Semi-Transparent, 2=OBJ Window, 3=Prohibited)
        ...
        if (((sprite_lcd_y - y_coord) & 0xff) <
            y_size * (double_size ? 2 : 1)) {
          ...
          for (int x = x_start; x < x_end; ++x) {
            ...
            int y_tile_stride = obj_vram_map_2d
                                    ? 32
                                    : x_size / 8 * (colors_or_palettes ? 2 : 1);
            int tile = tile_base + (((sx / 8)) * (colors_or_palettes ? 2 : 1)) +
                       (sy / 8) * y_tile_stride;
            // Don't allow the column indices to overflow into the row indices
            // in 2D mode. See: https://github.com/skylersaleh/SkyEmu/issues/13
            if (obj_vram_map_2d) {
              tile = (tile_base + (((sx / 8)) * (colors_or_palettes ? 2 : 1))) &
                     31;
              tile |= (tile_base + (sy / 8) * y_tile_stride) & ~31;
            }
            // Tiles >511 are not rendered in bg_mode3-5 since that memory is
            // used to store the bitmap graphics.
            ...
            if (colors_or_palettes == false) {
              palette_id =
                  gba->mem.vram[obj_tile_base + tile * 8 * 4 + tx / 2 + ty * 4];
              palette_id = (palette_id >> ((tx & 1) * 4)) & 0xf;
              transparent = palette_id == 0;
              palette_id += palette * 16;
            } else {
              palette_id =
                  gba->mem.vram[obj_tile_base + tile * 8 * 4 + tx + ty * 8];
              transparent = palette_id == 0;
            }

            uint32_t col = *(uint16_t *)(gba->mem.palette + GBA_OBJ_PALETTE +
                                         palette_id * 2);
            // Handle window objects(not displayed but control the windowing of
            // other things)
            if (obj_mode == 2 && !transparent) {
              ...
              if ((col >> 17) > (gba->first_target_buffer[x] >> 17)) {
                if (transparent) {
                  // Update priority for transparent pixels (needed for golden
                  // sun)
                  if (SB_BFE(gba->first_target_buffer[x], 17, 3) != 5)
                    gba->first_target_buffer[x] =
                        (gba->first_target_buffer[x] & (0x0fffffff)) |
                        (col & 0xf0000000);
                } else
    ...
    if (enabled_windows) {
      ...
      uint32_t backdrop_col =
          (*(uint16_t *)(gba->mem.palette + GBA_BG_PALETTE + 0 * 2)) |
          (backdrop_type << 17);
      for (int x = 0; x < 240; ++x) {
  ...
  if (visible) {
    ...
    if (bg_mode == 6 || bg_mode == 7) {
      ...
      for (int bg = 3; bg >= 0; --bg) {
        ...
        bool bg_en = SB_BFE(dispcnt, 8 + bg, 1) &&
                     SB_BFE(gba->ppu.dispcnt_pipeline[0], 8 + bg, 1);
        ...
        if (bg_mode == 3) {
          ...
          if (pallete_id == 0)
            continue;
          col =
              *(uint16_t *)(gba->mem.palette + GBA_BG_PALETTE + pallete_id * 2);
        } else if (bg_mode == 5) {
          int p = bg_x + bg_y * 160;
          int frame_sel = SB_BFE(dispcnt, 4, 1);
          ...
          if (rot_scale)
            ...
            tile_data =
                *(uint16_t *)(gba->mem.vram + screen_base_addr + tile_off * 2);
          ...
          if (colors == false) {
            ...
            // There is an undocumented GBA quirk where tiles over 64KB are not
            // loaded https://github.com/skylersaleh/SkyEmu/issues/292
    ...
    gba->framebuffer[p + 0] = r * 8 * (1.0 - screen_blend_factor) +
                              gba->framebuffer[p + 0] * screen_blend_factor;
    gba->framebuffer[p + 2] = b * 8 * (1.0 - screen_blend_factor) +
                              gba->framebuffer[p + 2] * screen_blend_factor;

    if (green_swap & 1) {
      if (p & 4)
        gba->framebuffer[p + 1 - 4] =
            g * 8 * (1.0 - screen_blend_factor) +
            gba->framebuffer[p + 1 - 4] * screen_blend_factor;
      else
        gba->framebuffer[p + 1 + 4] =
            g * 8 * (1.0 - screen_blend_factor) +
            gba->framebuffer[p + 1 + 4] * screen_blend_factor;
    } else {
      gba->framebuffer[p + 1] = g * 8 * (1.0 - screen_blend_factor) +
                                gba->framebuffer[p + 1] * screen_blend_factor;
    }
  }
}
static void gba_tick_keypad(sb_joy_t *joy, gba_t *gba) {
...
uint64_t gba_read_eeprom_bitstream(gba_t *gba, uint32_t source_address,
                                   int offset, int size, int elem_size,
                                   int dir) {
  uint64_t data = 0;
  for (int i = 0; i < size; ++i) {
    data |= ((uint64_t)(gba_read16(gba, source_address +
                                            (i + offset) * elem_size * dir) &
                        1))
            << (size - i - 1);
  }
  return data;
}
void gba_store_eeprom_bitstream(gba_t *gba, uint32_t source_address, int offset,
                                int size, int elem_size, int dir,
                                uint64_t data) {
  for (int i = 0; i < size; ++i) {
    gba_store16(gba, source_address + (i + offset) * elem_size * dir,
                data >> (size - i - 1) & 1);
  }
}
    ...
    if (enable) {
      ...
      if (gba->dma[i].current_transaction == 0) {
        ...
        if (gba->dma[i].source_addr >= 0x08000000 &&
            gba->dma[i].dest_addr >= 0x08000000) {
          ...
          if (gba->dma[i].dest_addr >= 0x08000000) {
            ...
            if (!gba->mem.prefetch_size && gba->mem.prefetch_en)
              ticks += gba_compute_access_cycles_dma(gba, gba->dma[i].dest_addr,
                                                     2) > 4;
          }
        }
        if (gba->dma[i].source_addr >= 0x08000000) {
          if (gba->mem.prefetch_en)
            ticks += gba_compute_access_cycles_dma(gba, gba->dma[i].source_addr,
                                                   2) <= 4;
        }
        ...
        static const uint32_t src_mask[] = {0x07FFFFFF, 0x0FFFFFFF, 0x0FFFFFFF,
                                            0x0FFFFFFF};
        static const uint32_t dst_mask[] = {0x07FFFFFF, 0x07FFFFFF, 0x07FFFFFF,
                                            0x0FFFFFFF};
        gba->dma[i].source_addr &= src_mask[i];
        gba->dma[i].dest_addr &= dst_mask[i];
        gba_io_store16(gba, GBA_DMA0CNT_L + 12 * i, cnt);

        if (src_addr_ctl == 0 && (dst_addr_ctl == 0 || dst_addr_ctl == 3) &&
            cnt > 2) {
          ...
          uint8_t *source_start =
              (uint8_t *)gba_dword_lookup(gba, src_addr,
                                          transfer_bytes | GBA_REQ_READ) +
              (src_addr & 2);
          uint8_t *dest_start =
              (uint8_t *)gba_dword_lookup(gba, dst_addr,
                                          transfer_bytes | GBA_REQ_WRITE) +
              (dst_addr & 2);
          uint8_t *source_end =
              (uint8_t *)gba_dword_lookup(gba, src_addr + bytes,
                                          transfer_bytes | GBA_REQ_READ) +
              (src_addr & 2);
          uint8_t *dest_end =
              (uint8_t *)gba_dword_lookup(gba, dst_addr + bytes,
                                          transfer_bytes | GBA_REQ_WRITE) +
              (dst_addr & 2);
          if (source_end - source_start == bytes &&
              dest_end - dest_start == bytes) {
            bool overlaps_io =
                src_addr <= 0x04000000 && src_addr + bytes >= 0x04000000;
            overlaps_io |=
                dst_addr <= 0x05000000 && dst_addr + bytes >= 0x04000000;
            if ((src_addr < 0x08000000) && (src_addr >= 0x02000000) &&
                !overlaps_io) {
              // Restrict the amount of cycles that can be spent on a fast DMA
              // to avoid missing events for very large DMAs.
              if (fast_dma_count > 128)
                fast_dma_count = 128;
              ...
              ticks += gba_compute_access_cycles_dma(
                  gba, gba->dma[i].dest_addr,
                  trans_type + (force_first_write_sequential ? 0 : 1));
              ticks +=
                  gba_compute_access_cycles_dma(gba, src_addr, trans_type + 1);
              // Remaining sequential fetches
              ticks += gba_compute_access_cycles_dma(gba, gba->dma[i].dest_addr,
                                                     trans_type) *
                       (fast_dma_count - 1);
              ticks +=
                  gba_compute_access_cycles_dma(gba, src_addr, trans_type) *
                  (fast_dma_count - 1);
            }
          }
        }
      ...
      if (i == 3 && gba->cart.backup_type == GBA_BACKUP_EEPROM) {
        int src_in_eeprom = (src & 0x1ffffff) >= gba->cart.rom_size ||
                            (src & 0x1ffffff) >= 0x01ffff00;
        int dst_in_eeprom = (dst & 0x1ffffff) >= gba->cart.rom_size ||
                            (dst & 0x1ffffff) >= 0x01ffff00;
        ...
        if (dst_in_eeprom) {
          if (cnt == 73) {
            ...
            uint32_t addr = gba_read_eeprom_bitstream(gba, src, 2, 6,
                                                      type ? 4 : 2, src_dir);
            uint64_t data = gba_read_eeprom_bitstream(gba, src, 2 + 6, 64,
                                                      type ? 4 : 2, src_dir);
            ((uint64_t *)gba->mem.cart_backup)[addr] = data;
            gba->cart.backup_is_dirty = true;
          } else if (cnt == 81) {
            // Write data 14 bit address
            uint32_t addr = gba_read_eeprom_bitstream(gba, src, 2, 14,
                                                      type ? 4 : 2, src_dir) &
                            0x3ff;
            uint64_t data = gba_read_eeprom_bitstream(gba, src, 2 + 14, 64,
                                                      type ? 4 : 2, src_dir);
            ((uint64_t *)gba->mem.cart_backup)[addr] = data;
            gba->cart.backup_is_dirty = true;
          } else if (cnt == 9) {
            ...
            // 1 bit "0"
            // Write data 6 bit address
            gba->mem.eeprom_addr = gba_read_eeprom_bitstream(
                gba, src, 2, 6, type ? 4 : 2, src_dir);
          } else if (cnt == 17) {
            // 2 bits "11" (Read Request)
        ...
        if (src_in_eeprom) {
          if (cnt == 68) {
            uint64_t data =
                ((uint64_t *)gba->mem.cart_backup)[gba->mem.eeprom_addr];
            gba_store_eeprom_bitstream(gba, dst, 4, 64, type ? 4 : 2, dst_dir,
                                       data);
          } else {
      ...
      if (audio_dma) {
        ...
        for (int x = 0; x < 4; ++x) {
          ...
          ticks += gba_compute_access_cycles_dma(
              gba, dst, x != 0 || force_first_write_sequential ? 2 : 3);
        }
        dst_addr_ctl = 2;
        transfer_bytes = 4;
        cnt = 4;
        ...
        if (gba->dma[i].current_transaction < cnt) {
          ...
          if (type) {
            if (src_addr >= 0x02000000) {
              ...
              ticks +=
                  gba_compute_access_cycles_dma(gba, src_addr, x != 0 ? 2 : 3);
            }
            gba_dma_write32(gba, dst_addr, gba->dma[i].latched_transfer);
            ticks += gba_compute_access_cycles_dma(
                gba, dst_addr, x != 0 || force_first_write_sequential ? 2 : 3);
          } else {
            int v = 0;
            if (src_addr >= 0x02000000) {
              v = gba->dma[i].latched_transfer =
                  (gba_read16(gba, src_addr)) & 0xffff;
              gba->dma[i].latched_transfer |= gba->dma[i].latched_transfer
                                              << 16;
              ticks +=
                  gba_compute_access_cycles_dma(gba, src_addr, x != 0 ? 0 : 1);
            } else
              v = gba->dma[i].latched_transfer >> (((dst_addr)&0x3) * 8);
            gba_dma_write16(gba, dst_addr, v & 0xffff);
            ticks += gba_compute_access_cycles_dma(
                gba, dst_addr, x != 0 || force_first_write_sequential ? 0 : 1);
          }
        }
      }
  ...
  for (int t = 0; t < 4; ++t) {
    ...
    if (enable) {
      ...
      if (count_up) {
        ...
        int increment = (gba->global_timer >> prescale_duty) -
                        (old_global_timer >> prescale_duty);
      ...
      if (last_timer_overflow) {
        ...
        if (t < 2) {
          for (int i = 0; i < 2; ++i) {
            ...
            int size =
                (gba->audio.fifo[i].write_ptr - gba->audio.fifo[i].read_ptr) &
                0x1f;
            while (samples_to_pop-- && size) {
              gba->audio.fifo[i].read_ptr =
                  (gba->audio.fifo[i].read_ptr + 1) & 0x1f;
              --size;
            }
...
static FORCE_INLINE float gba_compute_vol_env_slope(int length_of_step,
                                                    int dir) {
...
static FORCE_INLINE float gba_bandlimited_square(float t, float duty_cycle,
                                                 float dt) {
...
bool gba_run_ar_cheat(gba_t *gba, const uint32_t *buffer, uint32_t size) {
  ...
  for (int i = 0; i < size; i += 2) {
    uint64_t code =
        gba_decrypt_arv3(buffer[i + 1] | ((uint64_t)buffer[i] << 32));
    ...
    if (left != 0) {
      ...
      case 0x1E: {
        ...
        uint64_t decrypted =
            gba_decrypt_arv3(buffer[i + 3] | ((uint64_t)buffer[i + 2] << 32));
...
static int gba_lookup_gb_reg(int gb_reg) {
  switch (gb_reg) {
  case SB_IO_AUD1_TONE_SWEEP:
    return GBA_SOUND1CNT_L;
  case SB_IO_AUD1_LENGTH_DUTY:
    return GBA_SOUND1CNT_H;
  case SB_IO_AUD1_VOL_ENV:
    return GBA_SOUND1CNT_H + 1;
  case SB_IO_AUD1_FREQ:
    return GBA_SOUND1CNT_X;
  case SB_IO_AUD1_FREQ_HI:
    return GBA_SOUND1CNT_X + 1;
  case SB_IO_AUD2_LENGTH_DUTY:
    return GBA_SOUND2CNT_L;
  case SB_IO_AUD2_VOL_ENV:
    return GBA_SOUND2CNT_L + 1;
  case SB_IO_AUD2_FREQ:
    return GBA_SOUND2CNT_H;
  case SB_IO_AUD2_FREQ_HI:
    return GBA_SOUND2CNT_H + 1;
  case SB_IO_AUD3_POWER:
    return GBA_SOUND3CNT_L;
  case SB_IO_AUD3_LENGTH:
    return GBA_SOUND3CNT_H;
  case SB_IO_AUD3_VOL:
    return GBA_SOUND3CNT_H + 1;
  case SB_IO_AUD3_FREQ:
    return GBA_SOUND3CNT_X;
  case SB_IO_AUD3_FREQ_HI:
    return GBA_SOUND3CNT_X + 1;
  case SB_IO_AUD4_LENGTH:
    return GBA_SOUND4CNT_L;
  case SB_IO_AUD4_VOL_ENV:
    return GBA_SOUND4CNT_L + 1;
  case SB_IO_AUD4_POLY:
    return GBA_SOUND4CNT_H;
  case SB_IO_AUD4_COUNTER:
    return GBA_SOUND4CNT_H + 1;
  case SB_IO_MASTER_VOLUME:
    return GBA_SOUNDCNT_L;
  case SB_IO_SOUND_OUTPUT_SEL:
    return GBA_SOUNDCNT_L + 1;
  case SB_IO_SOUND_ON_OFF:
    return GBA_SOUNDCNT_X;
  }
...
static int gba_inverse_lookup_gb_reg(int gb_reg) {
  switch (gb_reg) {
  case GBA_SOUND1CNT_L:
    return SB_IO_AUD1_TONE_SWEEP;
  case GBA_SOUND1CNT_H:
    return SB_IO_AUD1_LENGTH_DUTY;
  case GBA_SOUND1CNT_H + 1:
    return SB_IO_AUD1_VOL_ENV;
  case GBA_SOUND1CNT_X:
    return SB_IO_AUD1_FREQ;
  case GBA_SOUND1CNT_X + 1:
    return SB_IO_AUD1_FREQ_HI;
  case GBA_SOUND2CNT_L:
    return SB_IO_AUD2_LENGTH_DUTY;
  case GBA_SOUND2CNT_L + 1:
    return SB_IO_AUD2_VOL_ENV;
  case GBA_SOUND2CNT_H:
    return SB_IO_AUD2_FREQ;
  case GBA_SOUND2CNT_H + 1:
    return SB_IO_AUD2_FREQ_HI;
  case GBA_SOUND3CNT_L:
    return SB_IO_AUD3_POWER;
  case GBA_SOUND3CNT_H:
    return SB_IO_AUD3_LENGTH;
  case GBA_SOUND3CNT_H + 1:
    return SB_IO_AUD3_VOL;
  case GBA_SOUND3CNT_X:
    return SB_IO_AUD3_FREQ;
  case GBA_SOUND3CNT_X + 1:
    return SB_IO_AUD3_FREQ_HI;
  case GBA_SOUND4CNT_L:
    return SB_IO_AUD4_LENGTH;
  case GBA_SOUND4CNT_L + 1:
    return SB_IO_AUD4_VOL_ENV;
  case GBA_SOUND4CNT_H:
    return SB_IO_AUD4_POLY;
  case GBA_SOUND4CNT_H + 1:
    return SB_IO_AUD4_COUNTER;
  case GBA_SOUNDCNT_L:
    return SB_IO_MASTER_VOLUME;
  case GBA_SOUNDCNT_L + 1:
    return SB_IO_SOUND_OUTPUT_SEL;
  case GBA_SOUNDCNT_X:
    return SB_IO_SOUND_ON_OFF;
  }
  return 0;
}
...
static uint8_t gba_audio_process_byte_write(gba_t *gba, uint32_t addr,
  ...
  if (addr == SB_IO_AUD1_LENGTH_DUTY || addr == SB_IO_AUD2_LENGTH_DUTY ||
      addr == SB_IO_AUD3_LENGTH || addr == SB_IO_AUD4_LENGTH) {
    uint8_t length_duty = value;
    if (i == 2)
      seq->length[i] = 256 - SB_BFE(length_duty, 0, 8);
    else
      seq->length[i] = 64 - SB_BFE(length_duty, 0, 6);
  } else if (addr == SB_IO_AUD1_VOL_ENV || addr == SB_IO_AUD2_VOL_ENV ||
             addr == SB_IO_AUD4_VOL_ENV) {
...
static FORCE_INLINE void sb_process_audio(sb_gb_t *gb, sb_emu_state_t *emu,
  ...
  uint32_t frame_cycles =
      (audio->audio_clock - (prev_audio_clock & ~32767)) / 32768;
  while (frame_cycles--)
  ...
  while (audio->current_sample_generated_time < audio->current_sim_time) {
    ...
    float channels[6];
    channels[0] = sb_bandlimited_square(seq->chan_t[0], duty1,
                                        sample_delta_t * freq_hz[0]) *
                  v[0];
    channels[1] = sb_bandlimited_square(seq->chan_t[1], duty2,
                                        sample_delta_t * freq_hz[1]) *
                  v[1];
    ...
    for (int i = 0; i < 2; ++i)
      channels[4 + i] =
          audio->fifo[i].data[audio->fifo[i].read_ptr & 0x1f] / 128.;
#else
    for (int i = 0; i < 2; ++i)
      channels[4 + i] = 0;
#endif
    ...
    emu->mix_l_volume = emu->mix_l_volume * lowpass_coef +
                        fabs(sample_volume_l) * (1.0 - lowpass_coef);
    emu->mix_r_volume = emu->mix_r_volume * lowpass_coef +
                        fabs(sample_volume_r) * (1.0 - lowpass_coef);

    for (int i = 0; i < 6; ++i) {
      emu->audio_channel_output[i] =
          emu->audio_channel_output[i] * lowpass_coef +
...
void gba_tick(sb_emu_state_t *emu, gba_t *gba, gba_scratch_t *scratch) {
  ...
  while (true) {
    ...
    int timer_fast_forward =
        gba->timer_ticks_before_event - gba->deferred_timer_ticks;
    ...
    if (SB_UNLIKELY(gba->ppu.has_hit_vblank || gba->stop_mode))
      break;
  }
