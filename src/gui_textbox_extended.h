...
typedef struct GuiTextBoxState {
  ...
  int index;  // Text start index (index inside the text of `start` always in
              // sync)
...
RAYGUIDEF void
GuiTextBoxSetCursor(int cursor); // Set cursor position of active textbox
RAYGUIDEF int
GuiTextBoxGetCursor(void); // Get cursor position of active textbox

RAYGUIDEF void
GuiTextBoxSetSelection(int start,
                       int length); // Set selection of active textbox
RAYGUIDEF Vector2
GuiTextBoxGetSelection(void); // Get selection of active textbox (x - selection
                              // start  y - selection length)

RAYGUIDEF bool GuiTextBoxIsActive(
    Rectangle bounds); // Returns true if a textbox control with specified
                       // `bounds` is the active textbox
RAYGUIDEF GuiTextBoxState
GuiTextBoxGetState(void); // Get state for the active textbox
RAYGUIDEF void GuiTextBoxSetState(
    GuiTextBoxState state); // Set state for the active textbox (state must be
                            // valid else things will break)

RAYGUIDEF void GuiTextBoxSelectAll(
    const char *text); // Select all characters in the active textbox (same as
                       // pressing `CTRL` + `A`)
RAYGUIDEF void GuiTextBoxCopy(
    const char *text); // Copy selected text to clipboard from the active
                       // textbox (same as pressing `CTRL` + `C`)
RAYGUIDEF void
GuiTextBoxPaste(char *text,
                int textSize); // Paste text from clipboard into the textbox
                               // (same as pressing `CTRL` + `V`)
RAYGUIDEF void
GuiTextBoxCut(char *text); // Cut selected text in the active textbox and copy
                           // it to clipboard (same as pressing `CTRL` + `X`)
RAYGUIDEF int GuiTextBoxDelete(
    char *text, int length,
    bool before); // Deletes a character or selection before from the active
                  // textbox (depending on `before`). Returns bytes deleted.
RAYGUIDEF int GuiTextBoxGetByteIndex(
    const char *text, int start, int from,
    int to); // Get the byte index for a character starting at position `from`
             // with index `start` until position `to`.

RAYGUIDEF bool GuiTextBoxEx(Rectangle bounds, char *text, int textSize,
                            bool editMode);
...
static int GuiMeasureTextBox(const char *text, int length, Rectangle rec,
                             int *pos, int mode);
static int GuiMeasureTextBoxRev(
    const char *text, int length, Rectangle rec,
    int *pos); // Highly synchronized with calculations in DrawTextRecEx()

static inline int GuiTextBoxGetCursorCoordinates(
    const char *text, int length, Rectangle rec,
    int pos); // Calculate cursor coordinates based on the cursor position `pos`
              // inside the `text`.
static inline int
GuiTextBoxGetCursorFromMouse(const char *text, int length, Rectangle rec,
                             int *pos); // Calculate cursor position in textbox
                                        // based on mouse coordinates.
static inline int
GuiTextBoxMaxCharacters(const char *text, int length,
                        Rectangle rec); // Calculates how many characters is the
                                        // textbox able to draw inside rec
static inline unsigned int GuiCountCodepointsUntilNewline(
    const char *text); // Returns total number of characters(codepoints) in a
                       // UTF8 encoded `text` until `\0` or a `\n` is found.

static inline void MoveTextBoxCursorRight(const char *text, int length,
                                          Rectangle textRec);
...
RAYGUIDEF void GuiTextBoxSetActive(Rectangle bounds) {
  ...
  guiTextBoxState =
      (GuiTextBoxState){.cursor = -1, .start = 0, .index = 0, .select = -1};
}
...
RAYGUIDEF Vector2 GuiTextBoxGetSelection(void) {
  if (guiTextBoxState.select == -1 ||
    ...
    return RAYGUI_CLITERAL(Vector2){guiTextBoxState.select,
                                    guiTextBoxState.cursor -
                                        guiTextBoxState.select};

  return RAYGUI_CLITERAL(Vector2){
      guiTextBoxState.cursor, guiTextBoxState.select - guiTextBoxState.cursor};
}

// Returns true if a textbox control with specified `bounds` is the active
// textbox
RAYGUIDEF bool GuiTextBoxIsActive(Rectangle bounds) {
  return (bounds.x == guiTextBoxActive.x && bounds.y == guiTextBoxActive.y &&
          bounds.width == guiTextBoxActive.width &&
          bounds.height == guiTextBoxActive.height);
}
...
RAYGUIDEF int GuiTextBoxGetByteIndex(const char *text, int start, int from,
                                     int to) {
  int i = start, k = from;
        ...
        if (guiTextBoxState.cursor != 0) {
          ...
          if (guiTextBoxState.cursor < guiTextBoxState.start)
            guiTextBoxState.start =
                -1; // Force to recalculate on the next frame
        }
      } else {
        // Delete character after cursor
        if (guiTextBoxState.cursor + 1 <=
            GuiCountCodepointsUntilNewline(text)) {
          startIdx = GuiTextBoxGetByteIndex(text, 0, 0, guiTextBoxState.cursor);
          endIdx =
              GuiTextBoxGetByteIndex(text, 0, 0, guiTextBoxState.cursor + 1);
        }
      }
    }
...
RAYGUIDEF void GuiTextBoxCopy(const char *text) {
  if ((text != NULL) && (guiTextBoxState.select != -1) &&
    ...
    // FIXME: `TextSubtext()` only lets use copy TEXTSPLIT_MAX_TEXT_LENGTH
    // (1024) bytes maybe modify `SetClipboardText()` so we can use it only on
    // part of a string
...
// `text` is the pointer to the buffer used by the textbox while `textSize` is
// the text buffer max size
RAYGUIDEF void GuiTextBoxPaste(char *text, int textSize) {
  const char *clipText =
      GetClipboardText(); // GLFW guaratees this should be UTF8 encoded!
  ...
  if ((text != NULL) && (clipText != NULL) && (guiTextBoxState.cursor != -1)) {
    ...
    // Calculate how many bytes can we copy from clipboard text before we run
    // out of space
...
// A text box control supporting text selection, cursor positioning and commonly
// used keyboard shortcuts. NOTE 1: Requires static variables: framesCounter
// NOTE 2: Returns if KEY_ENTER pressed (useful for data validation)
RAYGUIDEF bool GuiTextBoxEx(Rectangle bounds, char *text, int textSize,
                            bool editMode) {
// Define the cursor movement/selection speed when movement keys are
// held/pressed
#define TEXTBOX_CURSOR_COOLDOWN 5
  ...
  // Make sure length doesn't exceed `textSize`. `textSize` is actually the max
  // amount of characters the textbox can handle.
  int length = strlen(text);
  if (length > textSize) {
    text[textSize] = '\0';
    length = textSize;
  ...
  if ((bounds.width - 2 * GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING)) <
      GuiGetStyle(DEFAULT, TEXT_SIZE)) {
    bounds.width = GuiGetStyle(DEFAULT, TEXT_SIZE) +
                   2 * GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING);
  }
  ...
  Vector2 cursorPos = {
      textRec.x,
      textRec.y}; // This holds the coordinates inside textRec of the cursor at
                  // current position and will be recalculated later
  bool active = GuiTextBoxIsActive(
      bounds); // Check if this textbox is the global active textbox
    ...
    if (editMode) {
      ...
      if (!active) {
        if (CheckCollisionPointRec(mousePoint, bounds) &&
          ...
          GuiTextBoxSetActive(bounds);
                 ...
                 IsMouseButtonPressed(MOUSE_RIGHT_BUTTON)) {
        // When active and the right mouse is clicked outside the textbox we
        // should deactivate it NOTE: We set a dummy rect as the active textbox
        // bounds
      ...
      if (active) {
        ...
        if (guiTextBoxState.cursor == -1) {
          ...
          guiTextBoxState.cursor =
              GuiTextBoxMaxCharacters(text, length, textRec);
        }
        ...
        if (guiTextBoxState.start == -1) {
          ...
          // start will hold the starting character position from where the text
          // will be drawn while index will hold the byte index inside the text
          // for that character

          if (guiTextBoxState.cursor == 0) {
            guiTextBoxState.start = guiTextBoxState.index =
                0; // No need to recalculate
          } else {
            int pos = 0;
            int len =
                GuiTextBoxGetByteIndex(text, 0, 0, guiTextBoxState.cursor);
            guiTextBoxState.index =
                GuiMeasureTextBoxRev(text, len, textRec, &pos);
        ...
        // HANDLE KEY INPUT
        // -----------------
        // * -> | LSHIFT + -> move cursor to the right | increase selection by
        // one
        // * <- | LSHIFT + <- move cursor to the left | decrease selection by
        // one
        // * HOME | LSHIFT + HOME moves cursor to start of text | selects text
        // from cursor to start of text
        // * END | LSHIFT + END move cursor to end of text | selects text from
        // cursor until end of text
        ...
        // TODO: Add more shortcuts (insert mode, select word, moveto/select
        // prev/next word ...)
        if (IsKeyPressed(KEY_RIGHT) ||
            (IsKeyDown(KEY_RIGHT) &&
             (framesCounter % TEXTBOX_CURSOR_COOLDOWN == 0))) {
          if (IsKeyDown(KEY_LEFT_SHIFT)) {
            // Selecting
            if (guiTextBoxState.select == -1)
              guiTextBoxState.select =
                  guiTextBoxState.cursor; // Mark selection start
            ...
            if ((guiTextBoxState.select != -1) &&
              ...
              if (guiTextBoxState.cursor > guiTextBoxState.select) {
                ...
                if (guiTextBoxState.start > guiTextBoxState.cursor) {
                  ...
                  guiTextBoxState.index = GuiTextBoxGetByteIndex(
                      text, 0, 0,
                      guiTextBoxState.start); // Recalculate byte index
                }
              }
            } else {
          ...
          GuiTextBoxSelectAll(text); // CTRL + A > Select all
        else if (IsKeyDown(KEY_LEFT_CONTROL) && IsKeyPressed(KEY_C))
          GuiTextBoxCopy(text); // CTRL + C > Copy selected text to clipboard
        else if (IsKeyDown(KEY_LEFT_CONTROL) && IsKeyPressed(KEY_X))
          GuiTextBoxCut(text); // CTRL + X > Cut selected text
        else if (IsKeyDown(KEY_LEFT_CONTROL) && IsKeyPressed(KEY_V))
          GuiTextBoxPaste(text, textSize); // CTRL + V > Paste clipboard text
          ...
          if ((key >= 32) && ((guiTextBoxState.cursor + 1) < textSize)) {
            ...
            if (sz != 0) {
              ...
              if (endIdx <= textSize && length < textSize - 1) {
                ...
                if (guiTextBoxState.start != -1) {
                  const int max = GuiTextBoxMaxCharacters(
                      &text[guiTextBoxState.index],
                      length - guiTextBoxState.index, textRec);
        ...
        if (CheckCollisionPointRec(mousePoint, bounds)) {
          if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
            if (CheckCollisionPointRec(mousePoint, textRec)) {
              GuiTextBoxGetCursorFromMouse(&text[guiTextBoxState.index],
                                           length - guiTextBoxState.index,
                                           textRec, &guiTextBoxState.cursor);
              guiTextBoxState.cursor += guiTextBoxState.start;
              guiTextBoxState.select = -1;
            } else {
              ...
              if (mousePoint.x <= bounds.x + bounds.width / 2)
                ...
                guiTextBoxState.cursor =
                    guiTextBoxState.start +
                    GuiTextBoxMaxCharacters(&text[guiTextBoxState.index],
                                            length - guiTextBoxState.index,
                                            textRec);
              guiTextBoxState.select = -1;
            }
            ...
            if (CheckCollisionPointRec(mousePoint, textRec)) {
              ...
              // Clicked outside the `textRec` but still inside bounds, this
              // means that we must move the text
              move = true;
              if (mousePoint.x > bounds.x + bounds.width / 2) {
                cursor = GuiTextBoxMaxCharacters(&text[guiTextBoxState.index],
                                                 length - guiTextBoxState.index,
                                                 textRec);
              }
            }
            ...
            if ((framesCounter % TEXTBOX_CURSOR_COOLDOWN) == 0 && move) {
              if (cursor == 0)
                MoveTextBoxCursorLeft(text);
              else if (cursor == GuiTextBoxMaxCharacters(
                                     &text[guiTextBoxState.index],
                                     length - guiTextBoxState.index, textRec)) {
                MoveTextBoxCursorRight(text, length, textRec);
              }
        ...
        cursorPos.x = GuiTextBoxGetCursorCoordinates(
            &text[guiTextBoxState.index], length - guiTextBoxState.index,
            textRec, guiTextBoxState.cursor - guiTextBoxState.start);
        ...
        // We aren't drawing all of the text so make sure `DrawTextRecEx()` is
  ...
  if (state == GUI_STATE_PRESSED) {
    ...
    if (editMode && active &&
        ((framesCounter / TEXTEDIT_CURSOR_BLINK_FRAMES) % 2 == 0) &&
        selLength == 0) {
      DrawRectangle(
          cursorPos.x, cursorPos.y, 1, GuiGetStyle(DEFAULT, TEXT_SIZE) * 2,
  ...
  DrawTextRecEx(
      guiFont, &text[textStartIndex], textRec, GuiGetStyle(DEFAULT, TEXT_SIZE),
      GuiGetStyle(DEFAULT, TEXT_SPACING), false,
      Fade(GetColor(GuiGetStyle(TEXTBOX, TEXT + (state * 3))), guiAlpha),
      selStart, selLength, GetColor(GuiGetStyle(TEXTBOX, COLOR_SELECTED_FG)),
      GetColor(GuiGetStyle(TEXTBOX, COLOR_SELECTED_BG)));
...
// Returns total number of characters(codepoints) in a UTF8 encoded `text` until
// `\0` or a `\n` is found. NOTE: If a invalid UTF8 sequence is encountered a
// `?`(0x3f) codepoint is counted instead.
  ...
  for (i = 0; i < length; i++, k++) {
    ...
    if (letter != '\n') {
      ...
      if ((mode == GUI_MEASURE_MODE_CURSOR_POS) && (*pos == k))
        ...
        Rectangle grec = {rec.x + textOffsetX - 1, rec.y, glyphWidth,
                          (font.baseSize + font.baseSize / 2) * scaleFactor -
                              1};
        Vector2 mouse = GetMousePosition();

        if (CheckCollisionPointRec(mouse, grec)) {
          // Smooth selection by dividing the glyph rectangle into 2 equal parts
          // and checking where the mouse resides
...
// Highly synchronized with calculations in DrawTextRecEx()
static int GuiMeasureTextBoxRev(const char *text, int length, Rectangle rec,
                                int *pos) {
  // Get gui font properties
  const Font font = guiFont;
  const float fontSize = GuiGetStyle(DEFAULT, TEXT_SIZE);
...
// Calculate cursor coordinates based on the cursor position `pos` inside the
// `text`.
static inline int GuiTextBoxGetCursorCoordinates(const char *text, int length,
                                                 Rectangle rec, int pos) {
  return GuiMeasureTextBox(text, length, rec, &pos,
                           GUI_MEASURE_MODE_CURSOR_POS);
}

// Calculate cursor position in textbox based on mouse coordinates.
static inline int GuiTextBoxGetCursorFromMouse(const char *text, int length,
                                               Rectangle rec, int *pos) {
  return GuiMeasureTextBox(text, length, rec, pos,
                           GUI_MEASURE_MODE_CURSOR_COORDS);
}
...
static inline int GuiTextBoxMaxCharacters(const char *text, int length,
                                          Rectangle rec) {
  int pos = -1;
...
static inline void MoveTextBoxCursorRight(const char *text, int length,
                                          Rectangle textRec) {
  // FIXME: Counting codepoints each time we press the key is expensive, find
  // another way
  ...
  const int max = GuiTextBoxMaxCharacters(
      &text[guiTextBoxState.index], length - guiTextBoxState.index, textRec);

  if ((guiTextBoxState.cursor - guiTextBoxState.start) > max) {
    const int cidx =
        GuiTextBoxGetByteIndex(text, guiTextBoxState.index,
                               guiTextBoxState.start, guiTextBoxState.cursor);
    int pos = 0;
...
static inline void MoveTextBoxCursorLeft(const char *text) {
  ...
  if (guiTextBoxState.cursor < guiTextBoxState.start) {
    int prev = 0;
    int letter =
        GetPrevCodepoint(&text[guiTextBoxState.index - 1], text, &prev);
    if (letter == 0x3f)
      prev = 1;
    guiTextBoxState.start--;
