...
// Reserve space for extra keybinds/analog binds so that adding them in new
// versions don't break a users settings.
...
typedef struct {
  int bind_being_set;
  double rebind_start_time; // The time that the rebind button was pressed (used
                            // for the timer to cancel keybinding)
  int last_bind_activitiy; // ID of binding with latest activity only within the
                           // current frame. -1 if no keys pressed/movement
                           // during frame.
...
typedef struct {
  ...
  // Be very careful to keep alignment and ordering the same otherwise you will
  // break the settings.
  ...
  uint32_t screen_rotation; // 0: No rotation, 1: Rotate Left, 2: Rotate Right,
                            // 3: Upside Down
...
_Static_assert(sizeof(persistent_settings_t) == 1024,
               "persistent_settings_t must be exactly 1024 bytes");
...
_Static_assert(sizeof(se_search_paths_t) == SB_FILE_PATH_SIZE * 8,
               "se_search_paths_t must contain 8 paths");
...
typedef struct {
  ...
  // Utilize a watchdog channel to detect if the audio context has encountered
  // an error and restart it if a problem occurred.
  ...
  uint8_t font_cache_page_valid[(SE_MAX_UNICODE_CODE_POINT + 1) /
                                SE_FONT_CACHE_PAGE_SIZE];
...
typedef union {
  ...
  uint64_t raw_data[SE_MAX_CONST(SE_MAX_CONST(sizeof(gba_t), sizeof(nds_t)),
                                 sizeof(sb_gb_t)) /
                        SE_REWIND_SEGMENT_SIZE +
                    1];
} se_core_state_t;
typedef union {
  gba_scratch_t gba;
...
typedef struct {
  ...
  int32_t system;
  int32_t valid; // 0: invalid, 1: Valid (Perfect Save State) 2: Valid (BESS
                 // restore)
...
typedef struct {
  ...
  uint32_t bess_offset; // Number of bytes after the se_emu_id where the save
                        // state descriptor is located.
...
void se_draw_image(uint8_t *data, int im_width, int im_height, int x, int y,
                   int render_width, int render_height, bool has_alpha);
void se_draw_lcd(uint8_t *data, int im_width, int im_height, int x, int y,
                 int render_width, int render_height, float rotation,
                 bool is_touch);
void se_load_rom_overlay(bool visible);
void sb_draw_onscreen_controller(sb_emu_state_t *state, int controller_h,
                                 int controller_y_pad, bool preview);
...
static bool se_load_best_effort_state(se_core_state_t *state,
                                      uint8_t *save_state_data, uint32_t size,
                                      uint32_t bess_offset);
static size_t se_get_core_size();
uint8_t *se_hcs_callback(const char *cmd, const char **params,
                         uint64_t *result_size, const char **mime_type);
void se_open_file_browser(bool clicked, float x, float y, float w, float h,
                          void (*file_open_fn)(const char *dir),
                          const char **file_types, char *output_path);
...
static bool se_draw_theme_region(int region, float x, float y, float w,
                                 float h);
static bool se_draw_theme_region_tint(int region, float x, float y, float w,
                                      float h, uint32_t tint);
static bool se_draw_theme_region_tint_partial(int region, float x, float y,
                                              float w, float h, float w_ratio,
                                              float h_ratio, uint32_t tint);
static const char *se_get_pref_path() {
#if defined(EMSCRIPTEN)
  return "/offline/";
#elif defined(USE_SDL)
  static const char *cached_pref_path = NULL;
  if (cached_pref_path == NULL)
    cached_pref_path = SDL_GetPrefPath("Sky", "SkyEmu");
  return cached_pref_path;
#elif defined(PLATFORM_ANDROID)
  ANativeActivity *activity =
      (ANativeActivity *)sapp_android_get_native_activity();
...
static float se_dpi_scale() {
  if (gui_state.dpi_override)
    return gui_state.dpi_override / 120.;
...
char *se_replace_fake_path(char *new_path) {
  ...
  if (gui_state.fake_paths) {
    ...
    if (gui_state.ui_type == SE_UI_ANDROID)
      new_base = "/storage/emulated/0/Android/data/com.sky.SkyEmu/";
...
static bool se_combo_str(const char *label, int *current_item,
  ...
  return igComboStr(se_localize_and_cache(label), current_item,
                    se_localize_and_cache(items_separated_by_zeros),
                    popup_max_height_in_items);
}
static int se_slider_float(const char *label, float *v, float v_min,
                           float v_max, const char *format) {
  return igSliderFloat(se_localize_and_cache(label), v, v_min, v_max,
                       se_localize_and_cache(format),
                       ImGuiSliderFlags_AlwaysClamp);
}
static bool se_input_int(const char *label, int *v, int step, int step_fast,
                         ImGuiInputTextFlags flags) {
  return igInputInt(se_localize_and_cache(label), v, step, step_fast, flags);
}
static bool se_input_uint32(const char *label, uint32_t *v, int step,
...
static bool se_input_int32(const char *label, int32_t *v, int step,
                           int step_fast, ImGuiInputTextFlags flags) {
  int val = *v;
  bool ret = se_input_int(label, &val, step, step_fast, flags);
  *v = val;
...
static bool se_button_themed(int region, const char *label, ImVec2 size,
                             bool always_draw_label) {
  label = se_localize_and_cache(label);
  ImVec2 label_size;
  igCalcTextSize(&label_size, label, NULL, true, -1.0);
  ImGuiStyle *style = igGetStyle();

  igCalcItemSize(&size, size, label_size.x + style->FramePadding.x * 2.0f,
  ...
  if (gui_state.settings.theme == SE_THEME_CUSTOM &&
      gui_state.theme.regions[region].active) {
    for (int i = 0; i < ImGuiCol_COUNT; ++i)
      style->Colors[i].w = 0.;
    if (always_draw_label) {
      style->Colors[ImGuiCol_Text] = restore_style.Colors[ImGuiCol_Text];
      style->Colors[ImGuiCol_TextDisabled] =
          restore_style.Colors[ImGuiCol_TextDisabled];
    }
  }
  bool hover = igIsMouseHoveringRect(
      (ImVec2){pos.x, pos.y}, (ImVec2){pos.x + size.x, pos.y + size.y}, true);
  float alpha = 1.0;
  if (hover)
    alpha = 0.75;
...
bool se_slider_float_themed(const char *label, float *p_data, float p_min,
                            float p_max, const char *format) {
  if (igGetCurrentWindow()->SkipItems)
    return false;
  ...
  if (gui_state.settings.theme == SE_THEME_CUSTOM &&
      gui_state.theme.regions[SE_REGION_VOL_EMPTY].active) {
    for (int i = 0; i < ImGuiCol_COUNT; ++i)
      style->Colors[i].w = 0.;
    style->Colors[ImGuiCol_Text] = restore_style.Colors[ImGuiCol_Text];
    style->Colors[ImGuiCol_TextDisabled] =
  ...
  bool hover = igIsMouseHoveringRect(
      (ImVec2){pos.x, pos.y},
      (ImVec2){pos.x + frame_size.x, pos.y + frame_size.y}, true);
  float alpha = 1.0;
  if (hover)
    alpha = 0.75;
  uint32_t tint = 0x00ffffff | ((uint32_t)(alpha * 255) << 24u);
  se_draw_theme_region_tint(SE_REGION_VOL_EMPTY, pos.x, pos.y, frame_size.x,
                            frame_size.y, tint);
  ...
  float knob_x =
      orig_pos.x + bar_value * (orig_size.x - knob_size) + knob_size * 0.5;
  bar_value = (knob_x - pos.x) / frame_size.x;
  se_draw_theme_region_tint_partial(SE_REGION_VOL_FULL, pos.x, pos.y,
                                    frame_size.x, frame_size.y, bar_value, 1.0,
                                    tint);
  float render_knob_size = 20.0;
  se_draw_theme_region_tint(SE_REGION_VOL_KNOB, knob_x - render_knob_size * 0.5,
...
bool se_slider_int_themed(const char *label, int *v, float v_min, float v_max,
                          const char *format) {
  float vf = *v;
...
static bool se_button(const char *label, ImVec2 size) {
  return se_button_themed(SE_REGION_BLANK, se_localize_and_cache(label), size,
                          true);
}
static bool se_input_path(const char *label, char *new_path,
                          ImGuiInputTextFlags flags) {
  ...
  if (!read_only)
    igPushItemWidth(has_path ? -button_w * 2 : -button_w);
  else
    igPushItemWidth(-1);

  new_path = se_replace_fake_path(new_path);
  bool b = igInputText("##", new_path, SB_FILE_PATH_SIZE,
                       flags | ImGuiInputTextFlags_ReadOnly, NULL, NULL);
  igPopItemWidth();
  if (!read_only) {
    if (has_path) {
      igSameLine(0, 1);
    ...
    if (igIsItemVisible()) {
      ...
      se_open_file_browser(clicked, min.x, min.y, max.x - min.x, max.y - min.y,
                           NULL, types, new_path);
    }
  }
...
static bool se_input_file_callback(const char *label, char *new_path,
                                   const char **types,
                                   void (*file_open_fn)(const char *),
                                   ImGuiInputTextFlags flags) {
  int win_w = igGetWindowWidth();
  se_text(label);
  igSameLine(SE_FIELD_INDENT, 0);
...
static bool se_input_file(const char *label, char *new_path, const char **types,
                          ImGuiInputTextFlags flags) {
...
void se_bios_file_open_fn(const char *dir) {
  // Make use of the fact that the accept function is called before the output
  // path is updated.
  ...
  if (strncmp(dir, se_bios_file_open_tmp_path, SB_FILE_PATH_SIZE) != 0) {
    if (sb_file_exists(se_bios_file_open_tmp_path) ||
        strncmp(dir, "", SB_FILE_PATH_SIZE) == 0)
      remove(se_bios_file_open_tmp_path);
    if (strncmp(dir, "", SB_FILE_PATH_SIZE) != 0)
...
bool se_more_rewind_deltas(se_core_rewind_buffer_t *rewind, uint32_t index) {
  return (rewind->deltas[index % SE_REWIND_BUFFER_SIZE].offset &
          SE_LAST_DELTA_IN_TX) == 0;
}
void se_push_rewind_state(se_core_state_t *core,
                          se_core_rewind_buffer_t *rewind) {
  ...
  for (int s = 0; s < total_segments; ++s) {
    ...
    for (int s_off = 0; s_off < SE_REWIND_SEGMENT_SIZE / 8; ++s_off) {
      if (new_data[base_off + s_off] != old_data[base_off + s_off]) {
        delta = true;
        break;
      }
    }
  ...
  if (rewind->size >= SE_REWIND_BUFFER_SIZE)
    rewind->size = SE_REWIND_BUFFER_SIZE - 1;
  rewind->index = rewind->index % SE_REWIND_BUFFER_SIZE;
  // Discard partial transactions remaing
  while (rewind->size > 0 &&
         se_more_rewind_deltas(rewind, rewind->index - rewind->size))
    rewind->size--;
}
void se_rewind_state_single_tick(se_core_state_t *core,
                                 se_core_rewind_buffer_t *rewind) {
  uint64_t *old_data = (uint64_t *)&rewind->last_core;
...
uint8_t *se_save_state_to_image(se_save_state_t *save_state, uint32_t *width,
                                uint32_t *height) {
  ...
  int screenshot_size =
      save_state->screenshot_width * save_state->screenshot_height;
  ...
  for (int y = 0; y < save_state->screenshot_height * scale; ++y) {
    for (int x = 0; x < save_state->screenshot_width * scale; ++x) {
      ...
      uint8_t data = 0;
      if (p_out < sizeof(emu_id))
        data = emu_id_dat[p_out];
      else if (p_out - sizeof(emu_id) < save_state_size)
        data = save_state_dat[p_out - sizeof(emu_id)];
...
bool se_bess_state_restore(uint8_t *state_data, size_t data_size,
                           const se_emu_id emu_id, se_save_state_t *state) {
  state->state = core;
  printf("Attempting BESS Restore\n");
  if (sizeof(emu_id) > data_size)
    return false;
  size_t save_state_size = data_size - sizeof(emu_id);
  uint8_t *data = state_data + sizeof(emu_id);
  bool valid = se_load_best_effort_state(&(state->state), data, save_state_size,
                                         emu_id.bess_offset);
  printf("Valid:%d\n", valid);
  if (!valid) {
...
bool se_load_state_from_disk(se_save_state_t *save_state,
                             const char *filename) {
  ...
  int downscale = 1;
  while ((im_w / downscale) * (im_h / downscale) * 4 >=
         sizeof(save_state->screenshot))
    downscale++;
  ...
  for (int y = 0; y < im_h; y += downscale) {
    for (int x = 0; x < im_w; x += downscale) {
      ...
      for (int i = 0; i < 3; ++i)
        save_state->screenshot[p2 * 4 + i] = imdata[p1 * 4 + i];
  ...
  if (sizeof(se_emu_id) < data_size) {
    ...
    if (!bess && se_get_core_size() + sizeof(se_emu_id) <= data_size) {
      memcpy(&(save_state->state), data + sizeof(se_emu_id),
             se_get_core_size());
      save_state->valid = 1;
    ...
    if (save_state->valid)
      ...
      printf("Failed to load state from file:%s\n", filename);
  }
  free(data);
...
static void se_tooltip(const char *tooltip) {
  if (igGetCurrentContext()->HoveredIdTimer < 1.5 ||
...
void se_load_search_paths() {
  ...
  snprintf(settings_path, SB_FILE_PATH_SIZE, "%ssearch_paths.bin",
           se_get_pref_path());
  if (!sb_load_file_data_into_buffer(settings_path, (void *)&gui_state.paths,
                                     sizeof(gui_state.paths)))
    memset(&gui_state.paths, 0, sizeof(gui_state.paths));
  char *paths[] = {gui_state.paths.save, gui_state.paths.bios,
  ...
  for (int i = 0; i < sizeof(paths) / sizeof(paths[0]); ++i) {
    ...
    if (len == 0) {
      ...
      if ((last_c != '/' && last_c != '\\') && len < SB_FILE_PATH_SIZE)
        paths[i][len] = '/';
    }
  }
}
void se_save_search_paths() {
  ...
  snprintf(settings_path, SB_FILE_PATH_SIZE, "%ssearch_paths.bin",
           se_get_pref_path());
  sb_save_file_data(settings_path, (uint8_t *)&gui_state.paths,
                    sizeof(gui_state.paths));
  se_emscripten_flush_fs();
}
...
bool se_load_bios_file(const char *name, const char *base_path,
                       const char *file_name, uint8_t *data, size_t data_size) {
  ...
  if (bios_data) {
    if (bios_bytes == data_size) {
      ...
      printf("%s file at %s is incorrectly sized. Expected %zu bytes, got %zu "
             "bytes",
             name, file_name, data_size, bios_bytes);
    }
  }
  if (!loaded_bios) {
    se_join_path(bios_path, SB_FILE_PATH_SIZE, gui_state.paths.bios, file_name,
                 NULL);
    size_t bios_bytes = 0;
    if (gui_state.settings.save_to_path)
      strncpy(bios_create_path, bios_path, SB_FILE_PATH_SIZE);
    ...
    if (bios_data) {
      if (bios_bytes == data_size) {
        ...
        printf("%s file at %s is incorrectly sized. Expected %zu bytes, got "
               "%zu bytes",
               name, file_name, data_size, bios_bytes);
      }
    }
...
static int se_game_info_alpha_comparator(const void *a, const void *b) {
  const int ga = *(int *)a;
  const int gb = *(int *)b;
  return strcmp(gui_state.recently_loaded_games[ga].path,
                gui_state.recently_loaded_games[gb].path);
}
static int se_game_info_rev_alpha_comparator(const void *a, const void *b) {
  const int ga = *(int *)a;
  const int gb = *(int *)b;
  return strcmp(gui_state.recently_loaded_games[gb].path,
                gui_state.recently_loaded_games[ga].path);
}
static void se_sort_recent_games_list() {
  int size = 0;
  for (int i = 0; i < SE_NUM_RECENT_PATHS; ++i) {
    if (gui_state.recently_loaded_games[i].path[0] == '\0')
      gui_state.sorted_recently_loaded_games[i] = -1;
    else {
      gui_state.sorted_recently_loaded_games[i] = i;
      size++;
    }
  }
  if (gui_state.recent_games_sort_type == SE_SORT_ALPHA_ASC)
    qsort(gui_state.sorted_recently_loaded_games, size, sizeof(int),
          se_game_info_alpha_comparator);
  else if (gui_state.recent_games_sort_type == SE_SORT_ALPHA_DESC)
    qsort(gui_state.sorted_recently_loaded_games, size, sizeof(int),
          se_game_info_rev_alpha_comparator);
}
static void se_save_recent_games_list() {
  gui_state_t *gui = &gui_state;
  char pref_path[SB_FILE_PATH_SIZE];
  snprintf(pref_path, SB_FILE_PATH_SIZE, "%s/%s", se_get_pref_path(),
           "recent_games.txt");
  ...
  for (int i = 0; i < SE_NUM_RECENT_PATHS; ++i) {
    if (strcmp("", gui->recently_loaded_games[i].path) == 0)
      break;
...
bool se_key_is_pressed(int keycode) {
  if (keycode > SAPP_MAX_KEYCODES || keycode == -1)
    return false;
  // Don't let keyboard input reach emulator when ImGUI is capturing it.
  // Allow inputs if the touch screen is being pressed as the screen is an ImGUI
  // object that registers drag events.
  if (igGetIO()->WantCaptureKeyboard && !emu_state.joy.inputs[SE_KEY_PEN_DOWN])
    return false;
...
static void se_write16(emu_byte_write_t write, uint64_t address,
                       uint16_t data) {
  write(address, SB_BFE(data, 0, 8));
  write(address + 1, SB_BFE(data, 8, 8));
}
static void se_write32(emu_byte_write_t write, uint64_t address,
                       uint32_t data) {
  write(address, SB_BFE(data, 0, 8));
  write(address + 1, SB_BFE(data, 8, 8));
...
void se_record_emulation_frame_stats(se_emulator_stats_t *stats,
                                     int frames_emulated) {
  ...
  for (int i = 0; i < SE_STATS_GRAPH_DATA - abs_frames; ++i) {
    stats->waveform_fps_emulation[i] =
        stats->waveform_fps_emulation[i + abs_frames];
  }
...
void se_draw_emu_stats() {
  ...
  for (int i = 0; i < SE_STATS_GRAPH_DATA; ++i) {
    float l = emu_state.audio_ring_buff
                  .data[(emu_state.audio_ring_buff.write_ptr - i * 2 - 2) %
                        SB_AUDIO_RING_BUFFER_SIZE] /
              32768.;
    float r = emu_state.audio_ring_buff
                  .data[(emu_state.audio_ring_buff.write_ptr - i * 2 - 1) %
                        SB_AUDIO_RING_BUFFER_SIZE] /
              32768.;
  ...
  snprintf(label_tmp, 128, se_localize_and_cache("Display FPS: %2.1f\n"),
           render_avg);
  igPlotLinesFloatPtr("", stats->waveform_fps_render, SE_STATS_GRAPH_DATA, 0,
                      label_tmp, 0, render_max * 1.3,
                      (ImVec2){content_width, 80}, 4);

  snprintf(label_tmp, 128, se_localize_and_cache("Emulation FPS: %2.1f\n"),
           emulate_avg);
  igPlotLinesFloatPtr("", stats->waveform_fps_emulation, SE_STATS_GRAPH_DATA, 0,
  ...
  igPlotLinesFloatPtr("", stats->waveform_l, SE_STATS_GRAPH_DATA, 0,
                      se_localize_and_cache("Left Audio Channel"), -1, 1,
                      (ImVec2){content_width, 80}, 4);
  igPlotLinesFloatPtr("", stats->waveform_r, SE_STATS_GRAPH_DATA, 0,
                      se_localize_and_cache("Right Audio Channel"), -1, 1,
                      (ImVec2){content_width, 80}, 4);

  const char *null_names[] = {NULL};
  const char **channel_names = null_names;
  if (emu_state.system == SYSTEM_GB) {
    static const char *names[] = {"Channel 1 (Square)", "Channel 2 (Square)",
                                  "Channel 3 (Wave)", "Channel 4 (Noise)",
                                  NULL};
    channel_names = names;
  } else if (emu_state.system == SYSTEM_GBA) {
    static const char *names[] = {"Channel 1 (Square)",
                                  "Channel 2 (Square)",
                                  "Channel 3 (Wave)",
                                  "Channel 4 (Noise)",
                                  "Channel A (FIFO)",
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    igProgressBar(emu_state.audio_channel_output[i],
                  (ImVec2){content_width * 0.6, 0}, "");
  }
  float audio_buff_size = sb_ring_buffer_size(&emu_state.audio_ring_buff) /
                          (float)SB_AUDIO_RING_BUFFER_SIZE;
  snprintf(label_tmp, 128,
           se_localize_and_cache("Audio Ring (Samples Available: %d)"),
           sb_ring_buffer_size(&emu_state.audio_ring_buff));
  se_text(label_tmp);
  igProgressBar(audio_buff_size, (ImVec2){content_width, 0}, "");
  snprintf(label_tmp, 128,
           se_localize_and_cache("Audio Watchdog Triggered %d Times"),
  ...
  se_text("Commit Hash:");
  igPushItemWidth(-1);
  igInputText("##COMMIT_HASH", GIT_COMMIT_HASH, sizeof(GIT_COMMIT_HASH),
...
void se_psg_debugger() {
  ...
  sb_frame_sequencer_t *seq =
      emu_state.system == SYSTEM_GB
          ? &core.gb.audio.sequencer
          : (sb_frame_sequencer_t *)&core.gba.audio.sequencer;
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    se_input_uint32("Frequency", &seq->frequency[i], 1, 10,
                    ImGuiInputTextFlags_None);

    if (i == 0) {
      se_checkbox("Sweep Enable", &seq->sweep_enable);
      se_input_int32("Sweep Dir.", &seq->sweep_direction, 1, 10,
                     ImGuiInputTextFlags_None);
      se_input_uint32("Sweep Timer", &seq->sweep_timer, 1, 10,
                      ImGuiInputTextFlags_None);
      se_input_uint32("Sweep Period", &seq->sweep_period, 1, 10,
                      ImGuiInputTextFlags_None);
      se_input_uint32("Sweep Shift", &seq->sweep_shift, 1, 10,
                      ImGuiInputTextFlags_None);
    }
    se_input_int32("Env Dir.", &seq->env_direction[i], 1, 10,
                   ImGuiInputTextFlags_None);
    se_input_uint32("Env Period", &seq->env_period[i], 1, 10,
                    ImGuiInputTextFlags_None);
    se_input_uint32("Env Timer", &seq->env_period_timer[i], 1, 10,
                    ImGuiInputTextFlags_None);
...
void se_draw_arm_state(const char *label, arm7_t *arm, emu_byte_read_t read) {
  ...
  while (reg_names[r]) {
    ...
    if (se_input_int(reg_names[r], &value, 0, 0,
  ...
  for (int i = 0; i < buffer_size; ++i)
    buffer[i] = read(pc - off + i);
  se_text(ICON_FK_LIST_OL " Disassembly");
  igSeparator();
  csh handle;
  if (cs_open(CS_ARCH_ARM, thumb ? CS_MODE_THUMB : CS_MODE_ARM, &handle) ==
      CS_ERR_OK) {
  ...
  igSeparator();
  igBeginChildStr(("##BranchLoc"), (ImVec2){0, 150}, true,
                  ImGuiWindowFlags_None);
  for (int i = 0;
       i < ARM_DEBUG_BRANCH_RING_SIZE && i < arm->debug_branch_ring_offset;
       ++i) {
  ...
  igEndChild();
  if (clear_step_data)
    arm->debug_branch_ring_offset = 0;
  se_text("SWI");
  igSeparator();
  igBeginChildStr(("##SWI"), (ImVec2){0, 150}, true, ImGuiWindowFlags_None);
  for (int i = 0; i < ARM_DEBUG_SWI_RING_SIZE && i < arm->debug_swi_ring_offset;
       ++i) {
    uint32_t ind = (arm->debug_swi_ring_offset - i - 1);
    ind %= ARM_DEBUG_SWI_RING_SIZE;
    se_text("%d", i + 1);
    igSameLine(60, 0);
    if (arm->debug_swi_ring_times[ind] >= 2)
      se_text("SWI 0x%02x (%dx)", arm->debug_swi_ring[ind],
              arm->debug_swi_ring_times[ind]);
    else
      se_text("SWI 0x%02x", arm->debug_swi_ring[ind]);
  }
  igEndChild();
  if (clear_step_data)
    arm->debug_swi_ring_offset = 0;
}
...
void gb_cpu_debugger() {
  ...
  const char *flag_names[] = {"Z",          "N",      "H",          "C",
                              "Inter. En.", "Prefix", "Wait Inter", NULL};

  bool flag_values[] = {SB_BFE(cpu_state->af, SB_Z_BIT, 1), // Z
  ...
  while (register_names_16b[r]) {
    ...
    if (r % 2) {
      ...
      igSetNextItemWidth((w - 100) * 0.5);
    se_input_int(register_names_16b[r], &value, 0, 0,
                 ImGuiInputTextFlags_CharsHexadecimal);
    ++r;
  }
  r = 0;
  ...
  while (register_names_8b[r]) {
    ...
    if (r % 2) {
      ...
      igSetNextItemWidth((w - 100) * 0.5);
    se_input_int(register_names_8b[r], &value, 0, 0,
                 ImGuiInputTextFlags_CharsHexadecimal);
    ++r;
  }
  r = 0;
...
void se_draw_mem_debug_state(const char *label, gui_state_t *gui,
                             emu_byte_read_t read, emu_byte_write_t write) {
  se_text(ICON_FK_EXCHANGE " Read/Write Memory Address");
  igSeparator();
  se_input_int("address", &gui->mem_view_address, 1, 5,
               ImGuiInputTextFlags_CharsHexadecimal);
  igSeparator();
  int v = se_read32(read, gui->mem_view_address);
  if (se_input_int("data (32 bit)", &v, 1, 5,
                   ImGuiInputTextFlags_CharsHexadecimal)) {
    se_write32(write, gui->mem_view_address, v);
  }
  v = se_read16(read, gui->mem_view_address);
  if (se_input_int("data (16 bit)", &v, 1, 5,
                   ImGuiInputTextFlags_CharsHexadecimal)) {
    se_write16(write, gui->mem_view_address, v);
  }
  v = (*read)(gui->mem_view_address);
  if (se_input_int("data (8 bit)", &v, 1, 5,
                   ImGuiInputTextFlags_CharsHexadecimal)) {
  ...
  igSeparator();
  se_input_int("Start Address", &gui->mem_dump_start_address, 1, 5,
               ImGuiInputTextFlags_CharsHexadecimal);
...
void gb_tile_map_debugger() {
  ...
  for (int tile_map = 0; tile_map < 2; ++tile_map) {
    ...
    int scanline = tile_map == 0 ? gb->lcd.curr_scanline + sy
                                 : gb->lcd.curr_window_scanline;
      ...
      for (int xt = 0; xt < 32; ++xt)
        for (int py = 0; py < 8; ++py)
          for (int px = 0; px < 8; ++px) {
            ...
            int y = yt * 8 + py;
            int color_id =
                sb_lookup_tile(gb, x, y, tile_map_base, bg_win_tile_data_mode);
            int p = (xt * 10 + (px) + 1) + (yt * 10 + py + 1) * image_width;
            int r = 0, g = 0, b = 0;
            sb_lookup_palette_color(gb, color_id, &r, &g, &b);
            if (((x == (box_x1 % 256) || x == (box_x2 % 256)) &&
                 (((y - box_y1) & 0xff) >= 0 &&
                  ((box_y2 - y) & 0xff) <= box_y2 - box_y1)) ||
                ((y == (box_y1 % 256) || y == (box_y2 % 256)) &&
                 (((x - box_x1) & 0xff) >= 0 &&
                  ((box_x2 - x) & 0xff) <= box_x2 - box_x1))) {
              r = 255;
              g = b = 0;
            }
            if (y == (scanline & 0xff) &&
    ...
    se_draw_image(tmp_image, image_width, image_height, x * se_dpi_scale(),
                  y * se_dpi_scale(), w * se_dpi_scale(), h * se_dpi_scale(),
                  false);
    igDummy((ImVec2){w, h});

    const char *name = tile_map == 0 ? "Background" : "Window";
    ImVec2 mouse_pos = {gui_state.mouse_pos[0] / se_dpi_scale(),
                        gui_state.mouse_pos[1] / se_dpi_scale()};
    mouse_pos.x -= x;
    mouse_pos.y -= y;
    if (mouse_pos.x < w && mouse_pos.y < h && mouse_pos.x >= 0 &&
        mouse_pos.y >= 0) {
      ...
      se_text("Tile (%d, %d) Index=0x%02x Attr=0x%02x)", tx, ty, tile_data0,
              tile_data1);
    } else
...
void gb_tile_data_debugger() {
  ...
  for (int tile_data_bank = 0; tile_data_bank < SB_VRAM_NUM_BANKS;
       ++tile_data_bank) {
...
void se_draw_io_state(const char *label, mmio_reg_t *mmios, int mmios_size,
                      emu_byte_read_t read, emu_byte_write_t write,
                      emu_mmio_access_type access_type) {
  for (int i = 0; i < mmios_size; ++i) {
    ...
    snprintf(lab, 80, "0x%08x: %s %s%s", addr, mmios[i].name,
             access.write_in_tick ? ICON_FK_PENCIL_SQUARE_O : "",
             access.read_in_tick ? ICON_FK_SEARCH : "");
    if (igTreeNodeStrStr(mmios[i].name, lab)) {
      for (int f = 0; f < sizeof(mmios[i].bits) / sizeof(mmios[i].bits[0]);
           ++f) {
        igPushIDInt(f);
        ...
        if (size) {
          ...
          igSameLine(0, 2);
          if (size > 1)
            se_text("%s (Bits [%d:%d])", mmios[i].bits[f].name, start,
                    start + size - 1);
          else
            se_text("%s (Bit %d)", mmios[i].bits[f].name, start);
        }
        igPopID();
      }
      if (!has_fields) {
...
static const char *valid_rom_file_types[] = {"*.gb",  "*.gba", "*.gbc",
                                             "*.nds", "*.zip", NULL};
...
void se_load_rom(const char *filename) {
  ...
  // Compute Save File Path
  {
    ...
    if (sb_path_has_file_ext(filename, ".sav") ||
        sb_path_has_file_ext(filename, ".code")) {
      if (strncmp(filename, gui_state.recently_loaded_games[0].path,
                  SB_FILE_PATH_SIZE) != 0) {
        return se_load_rom(gui_state.recently_loaded_games[0].path);
      }
      return;
    }
    snprintf(emu_state.save_data_base_path, SB_FILE_PATH_SIZE, "/offline/%s",
             c);
#else
    se_join_path(emu_state.save_data_base_path, SB_FILE_PATH_SIZE, base, c,
                 NULL);
#endif
    snprintf(save_file, SB_FILE_PATH_SIZE, "%s.sav",
             emu_state.save_data_base_path);
    if (!sb_file_exists(save_file)) {
      const char *base, *c, *ext;
      sb_breakup_path(filename, &base, &c, &ext);
      char tmp_path[SB_FILE_PATH_SIZE];
      se_join_path(tmp_path, SB_FILE_PATH_SIZE, gui_state.paths.save, c,
                   ".sav");

      if (sb_file_exists(tmp_path) || gui_state.settings.save_to_path) {
        se_join_path(emu_state.save_data_base_path, SB_FILE_PATH_SIZE,
                     gui_state.paths.save, c, NULL);
  ...
  // Compute Cheat Code File Path
  {
    ...
    const char *base, *c, *ext;
    sb_breakup_path(filename, &base, &c, &ext);
    snprintf(cheat_path, SB_FILE_PATH_SIZE, "%s.code",
             emu_state.save_data_base_path);
    if (!sb_file_exists(cheat_path)) {
      const char *base, *c, *ext;
      sb_breakup_path(filename, &base, &c, &ext);
      char tmp_path[SB_FILE_PATH_SIZE];
      se_join_path(tmp_path, SB_FILE_PATH_SIZE, gui_state.paths.cheat_codes, c,
                   ".code");
      if (sb_file_exists(tmp_path) || gui_state.settings.save_to_path) {
  ...
  if (emu_state.rom_loaded) {
    if (emu_state.system == SYSTEM_NDS)
      nds_unload(&core.nds, &scratch.nds);
    else if (emu_state.system == SYSTEM_GBA)
      gba_unload(&core.gba, &scratch.gba);
  }
  ...
  if (sb_path_has_file_ext(filename, ".zip")) {
    ...
    if (mz_zip_reader_init_file(&zip, filename, 0)) {
      ...
      for (size_t i = 0; i < total_files; ++i) {
        ...
        emu_state.rom_data = NULL;
        emu_state.rom_size = 0;
        snprintf(emu_state.rom_path, sizeof(emu_state.rom_path), "%s/%s",
                 filename, file_name_buff);
        if (success) {
          emu_state.rom_size = stat.m_uncomp_size;
          emu_state.rom_data = (uint8_t *)malloc(emu_state.rom_size);
          success &= mz_zip_reader_extract_to_mem(&zip, i, emu_state.rom_data,
                                                  emu_state.rom_size, 0);
          if (!success)
            free(emu_state.rom_data);
        }
      ...
      printf("Failed to load zip: %s\n", filename);

  } else {
    emu_state.rom_data =
        sb_load_file_data(emu_state.rom_path, &emu_state.rom_size);
    se_load_rom_from_emu_state(&emu_state);
  }
  if (emu_state.rom_loaded == false) {
    ...
    if (strncmp(filename, recent_games[0].path, SB_FILE_PATH_SIZE) != 0) {
      ...
      for (int i = 0; i < SE_NUM_RECENT_PATHS; ++i) {
        ...
        recent_games[i] = g;
        if (strncmp(temp_filename, g2.path, SB_FILE_PATH_SIZE) == 0 ||
            strncmp("", g2.path, SB_FILE_PATH_SIZE) == 0)
          break;
        g = g2;
      }
    }
  ...
  for (int i = 0; i < SE_NUM_SAVE_STATES; ++i) {
    ...
    snprintf(save_state_path, SB_FILE_PATH_SIZE, "%s.slot%d.state.png",
             emu_state.save_data_base_path, i);
    ...
    if (!save_states[i].valid) {
      ...
      snprintf(save_state_path, SB_FILE_PATH_SIZE, "%s%s.slot%d.state.png",
               gui_state.paths.save, file, i);
      se_load_state_from_disk(save_states + i, save_state_path);
    }
  }
  return;
...
static bool se_write_save_to_disk(const char *path) {
  ...
  if (emu_state.system == SYSTEM_GB) {
    if (core.gb.cart.ram_is_dirty) {
      saved = true;
      if (sb_save_file_data(path, core.gb.cart.ram_data,
                            core.gb.cart.ram_size)) {
      } else
        printf("Failed to write out save file: %s\n", path);
    ...
    if (core.gba.cart.backup_is_dirty) {
      ...
      case GBA_BACKUP_EEPROM:
        size = 8 * 1024;
        break;
      case GBA_BACKUP_EEPROM_512B:
        size = 512;
        break;
      case GBA_BACKUP_EEPROM_8KB:
        size = 8 * 1024;
        break;
      case GBA_BACKUP_SRAM:
        size = 32 * 1024;
        break;
      case GBA_BACKUP_FLASH_64K:
        size = 64 * 1024;
        break;
      case GBA_BACKUP_FLASH_128K:
        size = 128 * 1024;
        break;
      }
      if (size) {
...
static bool se_sync_save_to_disk() {
  return se_write_save_to_disk(emu_state.save_file_path);
}
// Returns offset into savestate where bess info can be found
static uint32_t se_save_best_effort_state(se_core_state_t *state) {
  if (emu_state.system == SYSTEM_GB)
    return sb_save_best_effort_state(&state->gb);
  if (emu_state.system == SYSTEM_GBA)
    return gba_save_best_effort_state(&state->gba);
  if (emu_state.system == SYSTEM_NDS)
    return nds_save_best_effort_state(&state->nds);
  return -1;
}
static bool se_load_best_effort_state(se_core_state_t *state,
                                      uint8_t *save_state_data, uint32_t size,
                                      uint32_t bess_offset) {
  if (emu_state.system == SYSTEM_GB)
    return sb_load_best_effort_state(&state->gb, save_state_data, size,
                                     bess_offset);
  if (emu_state.system == SYSTEM_GBA)
    return gba_load_best_effort_state(&state->gba, save_state_data, size,
                                      bess_offset);
  if (emu_state.system == SYSTEM_NDS)
    return nds_load_best_effort_state(&state->nds, save_state_data, size,
                                      bess_offset);
  return false;
}
...
se_lcd_info_t se_get_lcd_info() {
  if (emu_state.system == SYSTEM_GB) {
    if (core.gb.model == SB_GBC) {
      ...
      return (se_lcd_info_t){.red_color = {1, 0, 0},
                             .green_color = {0, 1, 0},
                             .blue_color = {0, 0, 1},
                             .gamma = 2.2,
                             .is_grayscale = true};
    ...
    if (gui_state.settings.gba_color_correction_mode == GBA_HIGAN_CORRECTION) {
      return (se_lcd_info_t){.red_color = {1, 0.039, 0.196},
                             .green_color = {0.196, 0.901, 0.039},
                             .blue_color = {0, 0.117, 0.862},
                             .gamma = 4.0};
    }
    return (se_lcd_info_t){.red_color = {1, 0.05, 0.0},
...
static void se_emulate_single_frame() {
  if (emu_state.system == SYSTEM_GB) {
    if (gui_state.test_runner_mode) {
      uint8_t palette[4 * 3] = {0xff, 0xff, 0xff, 0xAA, 0xAA, 0xAA,
    ...
    gba_tick(&emu_state, &core.gba, &scratch.gba);
  else if (emu_state.system == SYSTEM_NDS)
    nds_tick(&emu_state, &core.nds, &scratch.nds);

  se_run_all_ar_cheats();
}
static void se_screenshot(uint8_t *output_buffer, int *out_width,
                          int *out_height) {
  *out_height = *out_width = 0;
  ...
  if (emu_state.system == SYSTEM_GBA) {
    ...
    *out_height = NDS_LCD_H * 2;
    memcpy(output_buffer, scratch.nds.framebuffer_top,
           NDS_LCD_W * NDS_LCD_H * 4);
    memcpy(output_buffer + NDS_LCD_W * NDS_LCD_H * 4,
           scratch.nds.framebuffer_bottom, NDS_LCD_W * NDS_LCD_H * 4);
...
void se_draw_lcd_callback(const ImDrawList *parent_list, const ImDrawCmd *cmd) {
  if (cmd->UserCallbackData == NULL)
    return;
  se_draw_lcd_callback_t *call =
      (se_draw_lcd_callback_t *)cmd->UserCallbackData;
  se_draw_lcd(call->data, call->im_width, call->im_height, call->x, call->y,
              call->render_width, call->render_height, call->rotation,
              call->is_touch);
  free(call);
}
void se_draw_lcd_defer(uint8_t *data, int im_width, int im_height, int x, int y,
                       int render_width, int render_height, float rotation,
                       bool is_touch) {
  se_draw_lcd_callback_t *call =
      (se_draw_lcd_callback_t *)malloc(sizeof(se_draw_lcd_callback_t));
  call->data = data;
...
static void se_draw_emulated_system_screen(bool preview) {
  ...
  bool touch_controller_active =
      gui_state.last_touch_time >= 0 ||
      gui_state.settings.auto_hide_touch_controls == false;
  if (emu_state.system == SYSTEM_GBA) {
    native_w = GBA_LCD_W;
    native_h = GBA_LCD_H;
  } else if (emu_state.system == SYSTEM_NDS) {
    native_w = NDS_LCD_W;
    native_h = NDS_LCD_H * 2;
  ...
  if (gui_state.screen_height / dpi_scale - SE_MENU_BAR_HEIGHT >
          gui_state.screen_width / dpi_scale * render_aspect &&
      !preview) {
  ...
  if (touch_controller_active) {
    if (emu_state.system == SYSTEM_NDS &&
      ...
      } else if (lcd_render_w / scr_w >=
                 1.0 - 0.5 * gui_state.settings.touch_controls_scale) {
        if (gui_state.settings.avoid_overlaping_touchscreen) {
          controller_h = height * 0.5;
          controller_y_pad =
              height * 0.5 +
              (1.0 - gui_state.settings.touch_controls_scale) * height * 0.25;
        }
      } else {
  ...
  if (gui_state.settings.stretch_to_fit) {
    if (gui_state.last_touch_time >= 0) {
      if (scr_w * render_aspect <
          scr_h - (controller_h + controller_y_pad * 2)) {
        lcd_render_h = scr_h - (controller_h + controller_y_pad * 2);
        lcd_render_y =
            scr_h - (controller_h + controller_y_pad * 2 + lcd_render_h * 0.5) -
            scr_h * 0.5;
      }
    } else {
      lcd_render_h = scr_h;
    ...
    if (gui_state.settings.screen_rotation == 1 ||
        gui_state.settings.screen_rotation == 3) {
      int t = lcd_render_w;
  ...
  if (!se_draw_theme_region(portrait ? SE_REGION_BEZEL_PORTRAIT
                                     : SE_REGION_BEZEL_LANDSCAPE,
                            win_pos.x, win_pos.y, scr_w / se_dpi_scale(),
                            scr_h / se_dpi_scale())) {
    ImVec2 v;
    igGetWindowPos(&v);
    ...
    if (preview && emu_state.rom_loaded == false) {
      ImVec2 min = {(lcd_render_x - lcd_render_w * 0.5) / se_dpi_scale(),
                    (lcd_render_y - lcd_render_h * 0.5) / se_dpi_scale()};
      ImVec2 max = {(lcd_render_x + lcd_render_w * 0.5) / se_dpi_scale(),
                    (lcd_render_y + lcd_render_h * 0.5) / se_dpi_scale()};
      ...
      if (emu_state.system == SYSTEM_GBA) {
        se_draw_lcd_defer(core.gba.framebuffer, GBA_LCD_W, GBA_LCD_H,
                          lcd_render_x, lcd_render_y, lcd_render_w,
                          lcd_render_h, rotation, false);
      } else if (emu_state.system == SYSTEM_NDS) {
        if (hybrid_nds) {
          float p[6] = {
          ...
          se_draw_lcd_defer(core.nds.framebuffer_top, NDS_LCD_W, NDS_LCD_H,
                            lcd_render_x + p[0], lcd_render_y + p[1],
                            lcd_render_w / 3, lcd_render_h * 0.5, rotation,
                            false);
          se_draw_lcd_defer(core.nds.framebuffer_bottom, NDS_LCD_W, NDS_LCD_H,
                            lcd_render_x + p[2], lcd_render_y + p[3],
                            lcd_render_w / 3, lcd_render_h * 0.5, rotation,
                            true);
          se_draw_lcd_defer(core.nds.framebuffer_top, NDS_LCD_W, NDS_LCD_H,
                            lcd_render_x + p[4], lcd_render_y + p[5],
                            lcd_render_w * 2 / 3, lcd_render_h, rotation,
                            false);
        } else {
          float p[4] = {0, -lcd_render_h * 0.25, 0, lcd_render_h * 0.25};
          for (int i = 0; i < 2; ++i) {
            float x = p[i * 2 + 0];
            float y = p[i * 2 + 1];
            p[i * 2 + 0] = x * cos(-rotation) + y * sin(-rotation);
          ...
          se_draw_lcd_defer(core.nds.framebuffer_top, NDS_LCD_W, NDS_LCD_H,
                            lcd_render_x + p[0], lcd_render_y + p[1],
                            lcd_render_w, lcd_render_h * 0.5, rotation, false);
          se_draw_lcd_defer(core.nds.framebuffer_bottom, NDS_LCD_W, NDS_LCD_H,
                            lcd_render_x + p[2], lcd_render_y + p[3],
                            lcd_render_w, lcd_render_h * 0.5, rotation, true);
        }
      } else if (emu_state.system == SYSTEM_GB) {
        se_draw_lcd_defer(core.gb.lcd.framebuffer, SB_LCD_W, SB_LCD_H,
                          lcd_render_x, lcd_render_y, lcd_render_w,
                          lcd_render_h, rotation, false);
      }
    }
  }
  if (!gui_state.block_touchscreen || preview)
    sb_draw_onscreen_controller(&emu_state, controller_h, controller_y_pad,
                                preview);
}
static uint8_t gba_byte_read(uint64_t address) {
  return gba_read8_debug(&core.gba, address);
}
static void gba_byte_write(uint64_t address, uint8_t data) {
  gba_store8_debug(&core.gba, address, data);
}

static uint8_t gb_byte_read(uint64_t address) {
  return sb_read8(&core.gb, address);
}
static void gb_byte_write(uint64_t address, uint8_t data) {
  sb_store8(&core.gb, address, data);
}

static uint8_t nds9_byte_read(uint64_t address) {
  return nds9_debug_read8(&core.nds, address);
}
static void nds9_byte_write(uint64_t address, uint8_t data) {
  nds9_debug_write8(&core.nds, address, data);
}
static uint8_t nds7_byte_read(uint64_t address) {
  return nds7_debug_read8(&core.nds, address);
}
static void nds7_byte_write(uint64_t address, uint8_t data) {
  nds7_debug_write8(&core.nds, address, data);
}
...
sb_debug_mmio_access_t gba_mmio_access_type(uint64_t address,
                                            int trigger_breakpoint) {
  return gba_debug_mmio_access(&core.gba, address, trigger_breakpoint);
}
void gba_memory_debugger() {
  se_draw_mem_debug_state("GBA MEM", &gui_state, &gba_byte_read,
                          &gba_byte_write);
}
void gba_cpu_debugger() {
  se_draw_arm_state("CPU", &core.gba.cpu, &gba_byte_read);
}
void gba_mmio_debugger() {
  se_draw_io_state("GBA MMIO", gba_io_reg_desc,
                   sizeof(gba_io_reg_desc) / sizeof(mmio_reg_t), &gba_byte_read,
                   &gba_byte_write, &gba_mmio_access_type);
}

void gb_mmio_debugger() {
  se_draw_io_state("GB MMIO", gb_io_reg_desc,
                   sizeof(gb_io_reg_desc) / sizeof(mmio_reg_t), &gb_byte_read,
                   &gb_byte_write, NULL);
}
void gb_memory_debugger() {
  se_draw_mem_debug_state("GB MEM", &gui_state, &gb_byte_read, &gb_byte_write);
}

sb_debug_mmio_access_t nds7_mmio_access_type(uint64_t address,
                                             int trigger_breakpoint) {
  return nds_debug_mmio_access(&core.nds, NDS_ARM7, address,
                               trigger_breakpoint);
}
sb_debug_mmio_access_t nds9_mmio_access_type(uint64_t address,
                                             int trigger_breakpoint) {
  return nds_debug_mmio_access(&core.nds, NDS_ARM9, address,
                               trigger_breakpoint);
}
void nds7_mmio_debugger() {
  se_draw_io_state("NDS7 MMIO", nds7_io_reg_desc,
                   sizeof(nds7_io_reg_desc) / sizeof(mmio_reg_t),
                   &nds7_byte_read, &nds7_byte_write, &nds7_mmio_access_type);
}
void nds9_mmio_debugger() {
  se_draw_io_state("NDS9 MMIO", nds9_io_reg_desc,
                   sizeof(nds9_io_reg_desc) / sizeof(mmio_reg_t),
                   &nds9_byte_read, &nds9_byte_write, &nds9_mmio_access_type);
}
void nds7_mem_debugger() {
  se_draw_mem_debug_state("NDS9 MEM", &gui_state, &nds9_byte_read,
                          &nds9_byte_write);
}
void nds9_mem_debugger() {
  se_draw_mem_debug_state("NDS7_MEM", &gui_state, &nds7_byte_read,
                          &nds7_byte_write);
}
void nds7_cpu_debugger() {
  se_draw_arm_state("ARM7", &core.nds.arm7, &nds7_byte_read);
}
void nds9_cpu_debugger() {
  se_draw_arm_state("ARM9", &core.nds.arm9, &nds9_byte_read);
}
void nds_io_debugger() {
  nds_t *nds = &core.nds;
  for (int cpu = 0; cpu < 2; ++cpu) {
    se_text(cpu ? ICON_FK_EXCHANGE " ARM9 IPC FIFO"
                : ICON_FK_EXCHANGE " ARM7 IPC FIFO");
    igSeparator();
...
se_debug_tool_desc_t gba_debug_tools[] = {
    ...
    {ICON_FK_PENCIL_SQUARE_O, ICON_FK_PENCIL_SQUARE_O " Memory",
     gba_memory_debugger},
    {ICON_FK_VOLUME_UP, ICON_FK_VOLUME_UP " PSG", se_psg_debugger},
    {ICON_FK_AREA_CHART, ICON_FK_AREA_CHART " Emulator Stats",
     se_draw_emu_stats, .allow_hardcore = true},
    {NULL, NULL, NULL}};
...
se_debug_tool_desc_t nds_debug_tools[] = {
    {ICON_FK_TELEVISION " 7", ICON_FK_TELEVISION " ARM7 CPU",
     nds7_cpu_debugger},
    {ICON_FK_TELEVISION " 9", ICON_FK_TELEVISION " ARM9 CPU",
     nds9_cpu_debugger},
    ...
    {ICON_FK_PENCIL_SQUARE_O " 7", ICON_FK_PENCIL_SQUARE_O " ARM7 Memory",
     nds7_mem_debugger},
    {ICON_FK_PENCIL_SQUARE_O " 9", ICON_FK_PENCIL_SQUARE_O " ARM9 Memory",
     nds9_mem_debugger},
...
static se_debug_tool_desc_t *se_get_debug_description() {
  ...
  if (emu_state.system == SYSTEM_GBA)
    desc = gba_debug_tools;
  if (emu_state.system == SYSTEM_GB)
    desc = gb_debug_tools;
  if (emu_state.system == SYSTEM_NDS)
    desc = nds_debug_tools;
  return desc;
}
emu_byte_read_t se_read_byte_func(int addr_map) {
  if (emu_state.system == SYSTEM_GBA)
    return gba_byte_read;
  if (emu_state.system == SYSTEM_GB)
    return gb_byte_read;
  if (emu_state.system == SYSTEM_NDS)
    return addr_map == 7 ? nds7_byte_read : nds9_byte_read;
  return null_byte_read;
}
emu_byte_write_t se_write_byte_func(int addr_map) {
  if (emu_state.system == SYSTEM_GBA)
    return gba_byte_write;
  if (emu_state.system == SYSTEM_GB)
    return gb_byte_write;
  if (emu_state.system == SYSTEM_NDS)
    return addr_map == 7 ? nds7_byte_write : nds9_byte_write;
...
void se_capture_state(se_core_state_t *core, se_save_state_t *save_state) {
  ...
  se_screenshot(save_state->screenshot, &save_state->screenshot_width,
                &save_state->screenshot_height);
}
void se_restore_state(se_core_state_t *core, se_save_state_t *save_state) {
  if (!save_state->valid || save_state->system != emu_state.system ||
...
void se_load_cheats(const char *filename) {
  ...
  for (size_t i = 0; i < data_size; ++i) {
    ...
    if (state == 1) {
      if (cheat_name_size < SE_MAX_CHEAT_NAME_SIZE)
        ch->name[cheat_name_size++] = c;
    }
    if (state == 3) {
      if (cheat_name_size < SE_MAX_CHEAT_CODE_SIZE * 8)
        cheat_buffer[cheat_code_size++] = c;
    }
  }
  free(data);
}
static void se_draw_debug_menu() {
  ...
  if (gui_state.screen_width * 0.5 / se_dpi_scale() - SE_TOGGLE_WIDTH * 2.5 <
      (SE_MENU_BAR_BUTTON_WIDTH + 1) * 9) {
    igSetNextItemWidth(SE_MENU_BAR_BUTTON_WIDTH);
    if (igBeginCombo("##debug combo", "  " ICON_FK_BUG,
                     ImGuiComboFlags_NoArrowButton |
                         ImGuiComboFlags_HeightLarge)) {
      while (desc->label) {
        bool is_selected =
            desc->visible; // You can store your selection however you want,
                           // outside or inside your objects
        if (igSelectableBool(se_localize_and_cache(desc->label), is_selected,
                             ImGuiSelectableFlags_None, (ImVec2){0, 30})) {
    ...
    while (desc->label) {
      ...
      if (desc->visible) {
        ...
        if (se_button_themed(
                SE_REGION_BLANK_ACTIVE, desc->short_label,
                (ImVec2){SE_MENU_BAR_BUTTON_WIDTH, SE_MENU_BAR_HEIGHT}, true)) {
          desc->visible = !desc->visible;
        }
        igPopStyleColor(1);
      } else {
        if (se_button_themed(
                SE_REGION_BLANK, desc->short_label,
                (ImVec2){SE_MENU_BAR_BUTTON_WIDTH, SE_MENU_BAR_HEIGHT}, true)) {
          desc->visible = !desc->visible;
        }
      }
...
static float se_draw_debug_panels(float screen_x, float sidebar_w, float y,
                                  float height) {
  se_debug_tool_desc_t *desc = se_get_debug_description();
  if (!desc)
    return screen_x;
  while (desc->label) {
    if (desc->visible) {
      ...
      igSetNextWindowPos((ImVec2){screen_x, y}, ImGuiCond_Always,
                         (ImVec2){0, 0});
      igSetNextWindowSize((ImVec2){w, height}, ImGuiCond_Always);
      igBegin(se_localize_and_cache(desc->label), &desc->visible,
...
void se_draw_image_opacity(uint8_t *data, int im_width, int im_height, int x,
                           int y, int render_width, int render_height,
                           bool has_alpha, float opacity) {
  ...
  *image = sg_make_image(&desc);
  ...
  tint |= 0xff000000;
  ImDrawList_AddImage(
      igGetWindowDrawList(), (ImTextureID)(uintptr_t)image->id,
...
void se_draw_lcd(uint8_t *data, int im_width, int im_height, int x, int y,
  ...
  for(int i=0;i<im_width*im_height;++i){
    ...
    uint16_t color =
  SB_BFE(rgba8_data[i*4+0]-last_value[i*4+0],3,5)|(SB_BFE(rgba8_data[i*4+1]-last_value[i*4+1],2,6)<<5)|(SB_BFE(rgba8_data[i*4+2]-last_value[i*4+2],3,5)<<11);
  ...
  *image = sg_make_image(&desc);
  ...
  lcd_params_t lcd_params = {
      ...
      .display_mode =
          gui_state.test_runner_mode ? 0 : gui_state.settings.screen_shader,
      ...
      .red_color = {lcd_info.red_color[0], lcd_info.red_color[1],
                    lcd_info.red_color[2]},
      .green_color = {lcd_info.green_color[0], lcd_info.green_color[1],
                      lcd_info.green_color[2]},
      .blue_color = {lcd_info.blue_color[0], lcd_info.blue_color[1],
                     lcd_info.blue_color[2]},
      .color_correction_strength =
          gui_state.test_runner_mode ? 0 : gui_state.settings.color_correction};
  ...
  if (is_touch) {
    ...
    if (gui_state.mouse_button[0] && rx >= 0 && rx <= 1.0 && ry >= 0. &&
        ry <= 1.0)
      emu_state.joy.inputs[SE_KEY_PEN_DOWN] = true;

    for (int i = 0; i < SAPP_MAX_TOUCHPOINTS; ++i) {
      if (gui_state.touch_points[i].active == false)
        continue;
  ...
  sg_bindings bind = {.vertex_buffers[0] = gui_state.quad_vb,
                      .fs_images[0] = *image};
...
void se_draw_image(uint8_t *data, int im_width, int im_height, int x, int y,
                   int render_width, int render_height, bool has_alpha) {
  se_draw_image_opacity(data, im_width, im_height, x, y, render_width,
                        render_height, has_alpha, 1.0);
}
bool se_draw_image_button(uint8_t *data, int im_width, int im_height, int x,
                          int y, int render_width, int render_height,
                          bool has_alpha) {
  float dpi_scale = se_dpi_scale();
  ...
  if (igIsItemActive())
    opacity = 0.6;
  else if (igIsItemHovered(ImGuiHoveredFlags_None))
    opacity = 0.8;
  se_draw_image_opacity(data, im_width, im_height, x, y, render_width,
                        render_height, has_alpha, opacity);
...
bool se_handle_keybind_settings(int keybind_type, se_keybind_state_t *state) {
  double rebind_timer =
      SE_REBIND_TIMER_LENGTH - (se_time() - state->rebind_start_time);
  ...
  for (int k = 0; k < num_keybinds; ++k) {
    ...
    if (state->bound_id[k] != -1) {
      switch (keybind_type) {
      case SE_BIND_KEYBOARD:
        button_label = se_keycode_to_string(state->bound_id[k]);
        break;
      ...
      case SE_BIND_KEY: {
        ...
        if (is_hat) {
          ...
          const char *dir = "";
          if (hat_val == SDL_HAT_UP)
            dir = "UP";
          if (hat_val == SDL_HAT_DOWN)
            dir = "DOWN";
          if (hat_val == SDL_HAT_LEFT)
            dir = "LEFT";
          if (hat_val == SDL_HAT_RIGHT)
            dir = "RIGHT";
          ...
          snprintf(buff, sizeof(buff), se_localize("Analog %d %s"), joy_id,
                   dir);
        } else {
#ifdef PLATFORM_ANDROID
...
#endif
          snprintf(buff, sizeof(buff), se_localize("Key %d"),
                   state->bound_id[k]);
          button_label = buff;
        }
      }
        ...
        break;
      case SE_BIND_ANALOG:
        snprintf(buff, sizeof(buff), se_localize("Analog %d (%0.2f)"),
                 state->bound_id[k], state->value[k]);
        button_label = buff;
        button_label = buff;
        break;
#endif
...
void sb_draw_onscreen_controller(sb_emu_state_t *state, int controller_h,
                                 int controller_y_pad, bool preview) {
  if (state->run_mode != SB_MODE_RUN && preview == false)
    return;
  ...
  if (controller_h * 1.4 < win_w)
    size_scalar = controller_h * 1.4;
  size_scalar *= 1.15;
  if (gui_state.settings.touch_controls_scale)
    size_scalar *= sqrtf(gui_state.settings.touch_controls_scale);
  ...
  if (opacity > 1 || preview)
    opacity = 1;
  if (!gui_state.settings.auto_hide_touch_controls)
    opacity = 1;
  if (opacity <= 0) {
    opacity = 0;
  }
  ...
  float a_pos[2] = {win_w - button_r * 1.5,
                    face_button_h * 0.48 + face_button_y};
  float b_pos[2] = {win_w - button_r * 3.8,
                    face_button_h * 0.54 + face_button_y};

  // Only have the x and y buttons on screen if the emulated system uses them.
  float x_pos[2] = {10e9, 10e9};
  float y_pos[2] = {10e9, 10e9};
  float dpad_pos[2] = {dpad_sz1 + button_padding * 2,
                       face_button_h * 0.5 + face_button_y};
  if (emu_state.system == SYSTEM_GB) {
    dpad_pos[1] *= 0.8;
    a_pos[1] *= 0.8;
    b_pos[1] *= 0.8;
  ...
  button_row_t bottom_row[] = {
      {"Select", button_x_off, button_w * 0.67 - button_padding,
       SE_REGION_KEY_SELECT},
      {"Hold", button_x_off + button_w * 0.67, button_w * 0.33,
       SE_REGION_KEY_HOLD},
      {"Turbo", b_pos[0] + win_x - button_r, button_w * 0.33,
       SE_REGION_KEY_TURBO},
      {"Start", b_pos[0] + win_x - button_r + button_w * 0.33 + button_padding,
       button_w * 0.67 - button_padding, SE_REGION_KEY_START},
  };
  ...
  bool a = false, b = false, x = false, y = false, up = false, down = false,
  ...
  for (int i = 0; i < p; ++i) {
    ...
    if (dx >= -dpad_sz1 * 1.15 && dx <= dpad_sz1 * 1.15 &&
        dy >= -dpad_sz1 * 1.15 && dy <= dpad_sz1 * 1.15) {
      if (dy > dpad_sz0)
        down = true;
      if (dy < -dpad_sz0)
        up = true;
    ...
    if (sb_distance(points[i], a_pos, 2) < button_r * 1.6)
      a = true;
    if (sb_distance(points[i], b_pos, 2) < button_r * 1.6)
      b = true;
    if (sb_distance(points[i], x_pos, 2) < button_r * 1.6)
      x = true;
    if (sb_distance(points[i], y_pos, 2) < button_r * 1.6)
      y = true;
  }
  int scale = 1;
  int button_press = 0;
  ...
  if (state->prev_frame_joy.inputs[SE_KEY_A])
    prev_pressed |= 1 << 0;
  if (state->prev_frame_joy.inputs[SE_KEY_B])
    prev_pressed |= 1 << 1;
  if (state->prev_frame_joy.inputs[SE_KEY_X])
    prev_pressed |= 1 << 2;
  if (state->prev_frame_joy.inputs[SE_KEY_Y])
    prev_pressed |= 1 << 3;

  if (state->prev_frame_joy.inputs[SE_KEY_L])
    prev_pressed |= 1 << 4;
  if (state->prev_frame_joy.inputs[SE_KEY_R])
    prev_pressed |= 1 << 5;
  if (state->prev_frame_joy.inputs[SE_KEY_SELECT])
    prev_pressed |= 1 << 6;
  if (state->prev_frame_joy.inputs[SE_KEY_START])
    prev_pressed |= 1 << 9;

  float *key_pos[] = {a_pos, b_pos, x_pos, y_pos};

  ImDrawList *dl = igGetWindowDrawList();
  for (int i = 0; i < 4; ++i) {
    ImU32 col = SB_BFE(gui_state.touch_controls.hold_toggle, i, 1) ? hold_color
                : SB_BFE(gui_state.touch_controls.turbo_toggle, i, 1)
                    ? turbo_color
                    : line_color;
    bool pressed = SB_BFE(button_press | prev_pressed, i, 1);
    float *pos = key_pos[i];

    if (se_draw_theme_region_tint(
            SE_REGION_KEY_A + i * 2 + (pressed ? 1 : 0),
            pos[0] - button_r * themed_scale, pos[1] - button_r * themed_scale,
            button_r * 2 * themed_scale, button_r * 2 * themed_scale, col))
      ;
    else if (se_draw_theme_region_tint(
                 SE_REGION_KEY_A + i * 2, pos[0] - button_r * themed_scale,
                 pos[1] - button_r * themed_scale, button_r * 2 * themed_scale,
                 button_r * 2 * themed_scale, col)) {
      if (pressed)
        ImDrawList_AddCircleFilled(dl, (ImVec2){pos[0], pos[1]}, button_r,
                                   sel_color, 128);
      ...
      ;
                 ...
                 SE_REGION_KEY_BLANK, pos[0] - button_r * themed_scale,
                 pos[1] - button_r * themed_scale, button_r * 2 * themed_scale,
                 button_r * 2 * themed_scale, col)) {
      if (pressed)
        ImDrawList_AddCircleFilled(dl, (ImVec2){pos[0], pos[1]}, button_r,
      ...
      if (pressed)
        ImDrawList_AddCircleFilled(dl, (ImVec2){pos[0], pos[1]}, button_r,
                                   sel_color, 128);
      ImDrawList_AddCircle(dl, (ImVec2){pos[0], pos[1]}, button_r, line_color2,
                           128, line_w1);
      ImDrawList_AddCircle(dl, (ImVec2){pos[0], pos[1]}, button_r, col, 128,
                           line_w0);
    }
  }
  ...
  if (dpad_code == 4) {
    dpad_code = state->prev_frame_joy.inputs[SE_KEY_UP] > 0.2     ? 0
                : state->prev_frame_joy.inputs[SE_KEY_DOWN] > 0.2 ? 6
                                                                  : 3;
    dpad_code += state->prev_frame_joy.inputs[SE_KEY_LEFT] > 0.2    ? 0
                 : state->prev_frame_joy.inputs[SE_KEY_RIGHT] > 0.2 ? 2
                                                                    : 1;
  }
  bool draw_dpad = !se_draw_theme_region_tint(
      SE_REGION_DPAD_UL + dpad_code, dpad_pos[0] - dpad_sz1 * themed_scale,
      dpad_pos[1] - dpad_sz1 * themed_scale, dpad_sz1 * 2 * themed_scale,
      dpad_sz1 * 2 * themed_scale, line_color);
  if (draw_dpad) {
    if (!se_draw_theme_region_tint(
            SE_REGION_DPAD_UL + 4, dpad_pos[0] - dpad_sz1 * themed_scale,
            dpad_pos[1] - dpad_sz1 * themed_scale, dpad_sz1 * 2 * themed_scale,
            dpad_sz1 * 2 * themed_scale, line_color)) {
    ...
    if (dpad_code >= 6)
      ImDrawList_AddRectFilled(
          dl, (ImVec2){dpad_pos[0] - dpad_sz0, dpad_pos[1] + dpad_sz0},
          (ImVec2){dpad_pos[0] + dpad_sz0, dpad_pos[1] + dpad_sz1}, sel_color,
          0, ImDrawCornerFlags_None);
    if (dpad_code < 3)
      ImDrawList_AddRectFilled(
          dl, (ImVec2){dpad_pos[0] - dpad_sz0, dpad_pos[1] - dpad_sz1},
          (ImVec2){dpad_pos[0] + dpad_sz0, dpad_pos[1] - dpad_sz0}, sel_color,
          0, ImDrawCornerFlags_None);

    if ((dpad_code % 3) == 0)
      ImDrawList_AddRectFilled(
          dl, (ImVec2){dpad_pos[0] - dpad_sz1, dpad_pos[1] - dpad_sz0},
          (ImVec2){dpad_pos[0] - dpad_sz0, dpad_pos[1] + dpad_sz0}, sel_color,
          0, ImDrawCornerFlags_None);
    if ((dpad_code % 3) == 2)
      ImDrawList_AddRectFilled(
          dl, (ImVec2){dpad_pos[0] + dpad_sz0, dpad_pos[1] - dpad_sz0},
          (ImVec2){dpad_pos[0] + dpad_sz1, dpad_pos[1] + dpad_sz0}, sel_color,
          0, ImDrawCornerFlags_None);
  }
  ...
  if (gui_state.settings.touch_controls_show_turbo == false) {
    ...
    gui_state.touch_controls.hold_toggle =
        gui_state.touch_controls.turbo_toggle = 0;
    gui_state.touch_controls.last_hold_toggle_presses =
        gui_state.touch_controls.last_turbo_toggle_presses = 0;
  }
  ...
  for (int b = 0; b < sizeof(bottom_row) / sizeof(bottom_row[0]); ++b) {
    ...
    for (int i = 0; i < p; ++i) {
      ...
      if (dx >= -(x_max - x_min) * 0.05 && dx <= (x_max - x_min) * 1.05 &&
          dy >= 0 && dy <= button_h) {
        button_press |= 1 << (b + 6);
        pressed = true;
        region += 1;
    ...
    if (b == turbo_button && (pressed || gui_state.touch_controls.turbo_toggle))
      col = turbo_color;
    if (b == hold_button && (pressed || gui_state.touch_controls.hold_toggle))
      col = hold_color;
    if (SB_BFE(gui_state.touch_controls.hold_toggle, b + 6, 1))
      col = hold_color;
    if (SB_BFE(gui_state.touch_controls.turbo_toggle, b + 6, 1))
      col = turbo_color;
    if (!se_draw_theme_region_tint(region + pressed, x_min, button_y,
                                   x_max - x_min, button_h, col)) {
      if (!se_draw_theme_region_tint(region, x_min, button_y, x_max - x_min,
                                     button_h, col)) {
        ImDrawList_AddRect(dl, (ImVec2){x_min, button_y},
                           (ImVec2){x_max, button_y + button_h}, line_color2, 0,
                           ImDrawCornerFlags_None, line_w1);
        ImDrawList_AddRect(dl, (ImVec2){x_min, button_y},
                           (ImVec2){x_max, button_y + button_h}, col, 0,
                           ImDrawCornerFlags_None, line_w0);
      }
      if (pressed) {
        ImDrawList_AddRectFilled(dl, (ImVec2){x_min, button_y},
                                 (ImVec2){x_max, button_y + button_h},
                                 sel_color, 0, ImDrawCornerFlags_None);
      }
    }
  }
  ...
  if (emu_state.system != SYSTEM_GB) {
    for (int b = 0; b < sizeof(top_row) / sizeof(top_row[0]); ++b) {
      ...
      if (SB_BFE(gui_state.touch_controls.hold_toggle, b + 4, 1))
        col = hold_color;
      if (SB_BFE(gui_state.touch_controls.turbo_toggle, b + 4, 1))
        col = turbo_color;
      if (!se_draw_theme_region_tint(region + pressed, x_min, button_y,
                                     x_max - x_min, button_h, col)) {
        if (!se_draw_theme_region_tint(region, x_min, button_y, x_max - x_min,
                                       button_h, col)) {
          ImDrawList_AddRect(dl, (ImVec2){x_min, button_y},
                             (ImVec2){x_max, button_y + button_h}, line_color2,
                             0, ImDrawCornerFlags_None, line_w1);
          ImDrawList_AddRect(dl, (ImVec2){x_min, button_y},
                             (ImVec2){x_max, button_y + button_h}, col, 0,
                             ImDrawCornerFlags_None, line_w0);
        }
  ...
  gui_state.touch_controls.hold_toggle ^=
      hold_mask & ~gui_state.touch_controls.last_hold_toggle_presses;
  gui_state.touch_controls.turbo_toggle ^=
      turbo_mask & ~gui_state.touch_controls.last_turbo_toggle_presses;
  gui_state.touch_controls.turbo_toggle &=
      ~(hold_mask & ~gui_state.touch_controls.last_hold_toggle_presses);
  gui_state.touch_controls.hold_toggle &=
      ~(turbo_mask & ~gui_state.touch_controls.last_turbo_toggle_presses);
  ...
  if (turbo_t > 0.5)
    button_press |= gui_state.touch_controls.turbo_toggle;
...
void se_update_solar_sensor(sb_emu_state_t *state) {
  ...
  if (state->joy.solar_sensor > 1.0)
    state->joy.solar_sensor = 1.0;
  if (state->joy.solar_sensor < 0.0)
    state->joy.solar_sensor = 0.0;
  last_t = se_time();
}
void se_text_centered_in_box(ImVec2 p, ImVec2 size, const char *text) {
  ...
  ImU32 color =
      igColorConvertFloat4ToU32(igGetStyle()->Colors[ImGuiCol_ButtonActive]);
  ImDrawList_AddRectFilled(
      igGetWindowDrawList(), curr_cursor_screen,
      (ImVec2){curr_cursor_screen.x + size.x, curr_cursor_screen.y + size.y},
      color, 0, ImDrawCornerFlags_None);
...
// CPU: 73%->48
bool se_selectable_with_box(const char *first_label, const char *second_label,
                            const char *box, bool force_hover,
                            int reduce_width) {
  ImVec2 win_min, win_sz, win_max;
  win_min.x = 0;
  win_min.y =
      0; // content boundaries min (roughly (0,0)-Scroll), in window coordinates
...
#ifdef UNICODE_GUI
  first_label =
      (const char *)utf8proc_NFC((const utf8proc_uint8_t *)first_label);
  second_label =
      (const char *)utf8proc_NFC((const utf8proc_uint8_t *)second_label);
#endif
  ...
  curr_pos.y += padding;
  curr_pos.x += padding;
  if (igSelectableBool("", force_hover, ImGuiSelectableFlags_None,
                       (ImVec2){igGetWindowContentRegionWidth() - reduce_width,
                                item_height}))
    clicked = true;
  ImVec2 next_pos;
  ...
  igBeginChildFrame(
      igGetIDStr(first_label),
      (ImVec2){igGetWindowContentRegionWidth() - box_w - padding - reduce_width,
               item_height},
...
void se_android_open_file_picker() {
  ...
  if (nResult != JNI_ERR) {
    ...
    jclass ClassNativeActivity =
        (*pJNIEnv)->GetObjectClass(pJNIEnv, nativeActivity);
    jmethodID MethodShowKeyboard = (*pJNIEnv)->GetMethodID(
        pJNIEnv, ClassNativeActivity, "openFile", "()V");
...
float se_android_get_display_dpi_scale() {
  ...
  if (nResult != JNI_ERR) {
    ...
    jclass ClassNativeActivity =
        (*pJNIEnv)->GetObjectClass(pJNIEnv, nativeActivity);
    jmethodID MethodDPI = (*pJNIEnv)->GetMethodID(pJNIEnv, ClassNativeActivity,
                                                  "getDPIScale", "()F");
...
void se_android_get_visible_rect(float *top, float *bottom) {
  ...
  if (nResult != JNI_ERR) {
    ...
    jclass ClassNativeActivity =
        (*pJNIEnv)->GetObjectClass(pJNIEnv, nativeActivity);
    jmethodID MethodBottom = (*pJNIEnv)->GetMethodID(
        pJNIEnv, ClassNativeActivity, "getVisibleBottom", "()F");
    *bottom =
        (*pJNIEnv)->CallFloatMethod(pJNIEnv, nativeActivity, MethodBottom) /
        se_dpi_scale();
    jmethodID MethodTop = (*pJNIEnv)->GetMethodID(pJNIEnv, ClassNativeActivity,
                                                  "getVisibleTop", "()F");
    *top = (*pJNIEnv)->CallFloatMethod(pJNIEnv, nativeActivity, MethodTop) /
           se_dpi_scale();
    // Finished with the JVM.
    (*pJavaVM)->DetachCurrentThread(pJavaVM);
  }
...
void se_android_get_language(char *language_buffer, size_t buffer_size) {
  ...
  if (nResult != JNI_ERR) {
    ...
    jclass ClassNativeActivity =
        (*pJNIEnv)->GetObjectClass(pJNIEnv, nativeActivity);
    jmethodID getLanguageMethod = (*pJNIEnv)->GetMethodID(
        pJNIEnv, ClassNativeActivity, "getLanguage", "()Ljava/lang/String;");
    if (getLanguageMethod) {
      jstring joStringPropVal = (jstring)(*pJNIEnv)->CallStaticObjectMethod(
          pJNIEnv, ClassNativeActivity, getLanguageMethod);
      const jchar *jcVal =
          (*pJNIEnv)->GetStringUTFChars(pJNIEnv, joStringPropVal, JNI_FALSE);
      ...
      LOGE("Failed to find getLanguage() method in JNIEnv");
    // Finished with the JVM.
    (*pJavaVM)->DetachCurrentThread(pJavaVM);
  }
}
...
void se_android_poll_events(bool visible) {
  ...
  if (visible != last_visible) {
    ...
    if (nResult != JNI_ERR) {
      ...
      if (visible) {
        jmethodID MethodShowKeyboard = (*pJNIEnv)->GetMethodID(
            pJNIEnv, ClassNativeActivity, "showKeyboard", "()V");
        (*pJNIEnv)->CallVoidMethod(pJNIEnv, nativeActivity, MethodShowKeyboard);
      } else {
        jmethodID MethodShowKeyboard = (*pJNIEnv)->GetMethodID(
            pJNIEnv, ClassNativeActivity, "hideKeyboard", "()V");
        (*pJNIEnv)->CallVoidMethod(pJNIEnv, nativeActivity, MethodShowKeyboard);
      }
      // Finished with the JVM.
      (*pJavaVM)->DetachCurrentThread(pJavaVM);
  ...
  if (nResult != JNI_ERR) {
    ...
    jclass ClassNativeActivity =
        (*pJNIEnv)->GetObjectClass(pJNIEnv, nativeActivity);
    jmethodID getEvent = (*pJNIEnv)->GetMethodID(pJNIEnv, ClassNativeActivity,
                                                 "getEvent", "()I");
    jmethodID pollKeyboard = (*pJNIEnv)->GetMethodID(
        pJNIEnv, ClassNativeActivity, "pollKeyboard", "()V");
    ...
    while (true) {
      int32_t event =
          (*pJNIEnv)->CallIntMethod(pJNIEnv, nativeActivity, getEvent);
      if (event == -1)
        break;
      if (!(event & 0xF0000000))
        ...
        if (fv < 0.2 && fv > -0.2 &&
            (int)axis < sizeof(cont->axis_last_zero_time) /
                            sizeof(cont->axis_last_zero_time[0])) {
          cont->axis_last_zero_time[axis] = se_time();
        }
        if ((fv > 0.3) || (fv < -0.3 && fv > -0.6))
          cont->analog.last_bind_activitiy = axis;
        double delta = se_time() - cont->axis_last_zero_time[axis];

        if (fv > 0.4 && delta < 2)
          cont->key.last_bind_activitiy = axis | SE_JOY_POS_MASK;
        if (fv < -0.4 && delta < 2)
          cont->key.last_bind_activitiy = axis | SE_JOY_NEG_MASK;
  ...
  for (int i = 0; i < SE_NUM_KEYBINDS; ++i)
    emu_state.joy.inputs[i] += cont->key.value[i] > 0.5;

  emu_state.joy.inputs[SE_KEY_LEFT] +=
      cont->analog.value[SE_ANALOG_LEFT_RIGHT] < -0.3;
  emu_state.joy.inputs[SE_KEY_RIGHT] +=
      cont->analog.value[SE_ANALOG_LEFT_RIGHT] > 0.3;
  emu_state.joy.inputs[SE_KEY_UP] +=
      cont->analog.value[SE_ANALOG_UP_DOWN] < -0.3;
  emu_state.joy.inputs[SE_KEY_DOWN] +=
      cont->analog.value[SE_ANALOG_UP_DOWN] > 0.3;
...
void se_android_request_permissions() {
  ...
  if (nResult != JNI_ERR) {
    ...
    jclass ClassNativeActivity =
        (*pJNIEnv)->GetObjectClass(pJNIEnv, nativeActivity);
    jmethodID MethodShowKeyboard = (*pJNIEnv)->GetMethodID(
        pJNIEnv, ClassNativeActivity, "requestPermissions", "()V");
    (*pJNIEnv)->CallVoidMethod(pJNIEnv, nativeActivity, MethodShowKeyboard);
...
void se_bring_text_field_into_view() {
  if (igGetIO()->WantTextInput) {
    ...
    if (igGetCurrentContext()->ActiveIdWindow)
      window = igGetCurrentContext()->ActiveIdWindow;
...
static void se_file_picker_click_region(int x, int y, int w, int h,
                                        void (*accept_func)(const char *)) {
  ...
  char *new_path = (char *)EM_ASM_INT(
      {
        ...
        if (input.value != '') {
          ...
          function print_file(e) {
            ...
            if (FS.analyzePath(out_file)["exists"])
              FS.unlink(out_file);
      ...
      gui_state.current_click_region_id, x * delta_dpi_scale,
      y * delta_dpi_scale, w * delta_dpi_scale, h * delta_dpi_scale);
...
// Opens a file picker when clicked is true or a user clicks in the click region
// defined by x,y,w,h in ImGUI coordinates File pickers only open for the click
// region on web platforms due to web security precautions. On Desktop/Native
// platforms they only open if clicked is set to true.
void se_open_file_browser(bool clicked, float x, float y, float w, float h,
                          void (*file_open_fn)(const char *dir),
                          const char **file_types, char *output_path) {
#ifndef EMSCRIPTEN
  ...
  while (file_types[num_file_types]) {
    if (!strcmp(file_types[num_file_types], "$DIR$"))
...
#endif
#ifdef USE_TINY_FILE_DIALOGS
  if (tinyfd_openFileDialog("tinyfd_query", "", num_file_types, file_types,
                            NULL, 0)) {
    gui_state.file_browser.state = SE_FILE_BROWSER_CLOSED;
  }
  char *outPath = NULL;
  if (allow_directory)
    outPath = tinyfd_selectFolderDialog(se_localize_and_cache("Select Folder"),
                                        output_path ? output_path : "");
  else
    outPath = tinyfd_openFileDialog(se_localize_and_cache("Open ROM"), "",
                                    num_file_types, file_types, NULL, 0);
  if (outPath) {
...
void se_convert_cheat_code(char *text_code, int cheat_index) {
  ...
  for (int i = 0; i < SE_MAX_CHEAT_CODE_SIZE * 8; ++i) {
    if (text_code[i] == '\0')
      break;
    else if ((text_code[i] >= '0' && text_code[i] <= '9') ||
             (text_code[i] >= 'A' && text_code[i] <= 'F') ||
             (text_code[i] >= 'a' && text_code[i] <= 'f')) {
  ...
  if (cheat->size >= SE_MAX_CHEAT_CODE_SIZE)
    cheat->size = SE_MAX_CHEAT_CODE_SIZE;
...
bool se_process_file_browser() {
  ...
  if (gui_state.file_browser.state == SE_FILE_BROWSER_CLOSED)
    return false;
  if (gui_state.file_browser.current_path[0] == '\0')
  ...
  igBegin(se_localize_and_cache(ICON_FK_FILE_O " File Browser"),
          &file_browser_open,
          ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
  if (!file_browser_open)
    gui_state.file_browser.state = SE_FILE_BROWSER_CLOSED;
  if (se_selectable_with_box("Exit File Browser",
                             "Go back to recently loaded games", ICON_FK_BAN,
                             false, 0)) {
    gui_state.file_browser.state = SE_FILE_BROWSER_CLOSED;
  }
  if (file_browse->allow_directory) {
    if (se_selectable_with_box("Select Folder",
                               gui_state.file_browser.current_path,
                               ICON_FK_CHECK, false, 0)) {
      size_t len =
          strnlen(gui_state.file_browser.current_path, SB_FILE_PATH_SIZE);
      if (len < SB_FILE_PATH_SIZE - 2) {
        if (gui_state.file_browser.current_path[len - 1] != '\\' &&
            gui_state.file_browser.current_path[len - 1] != '/') {
  ...
  if (se_selectable_with_box("Go to home directory", home_dir, ICON_FK_HOME,
                             false, 0)) {
    strncpy(gui_state.file_browser.current_path, home_dir, SB_FILE_PATH_SIZE);
  }

  const char *parent_dir = sb_parent_path(gui_state.file_browser.current_path);
  if (se_selectable_with_box("Go to parent directory", parent_dir,
                             ICON_FK_ARROW_UP, false, 0)) {
  ...
  igSetNextWindowPos((ImVec2){w_pos.x, w_pos.y + list_y_off}, ImGuiCond_Always,
                     (ImVec2){0, 0});
  igSetNextWindowSize((ImVec2){w_size.x, w_size.y - list_y_off},
                      ImGuiCond_Always);

  igBegin(se_localize_and_cache(ICON_FK_FOLDER_OPEN " Open File From Disk"),
          NULL, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
  bool update_cache =
      file_browse->has_cache == false ||
      file_browse->cached_time + 5. < se_time() ||
      strncmp(file_browse->cached_path, file_browse->current_path,
              SB_FILE_PATH_SIZE) != 0;
  if (update_cache) {
    strncpy(file_browse->cached_path, file_browse->current_path,
            SB_FILE_PATH_SIZE);
    ...
    if (tinydir_open(&file_browse->cached_dir,
                     gui_state.file_browser.current_path) == -1) {
      printf("Error opening %s\n", gui_state.file_browser.current_path);
    } else {
      int max_files = 4096;
      file_browse->cached_files =
          (tinydir_file *)malloc(sizeof(tinydir_file) * max_files);
      int f = 0;
      while (file_browse->cached_dir.has_next && f < max_files) {
        tinydir_readfile(&file_browse->cached_dir,
                         &file_browse->cached_files[f]);
        ...
        if (!file_browse->cached_files[f].is_dir) {
          ...
          for (int i = 0; i < file_browse->num_file_types; ++i) {
            if (sb_path_has_file_ext(file_browse->cached_files[f].path,
                                     file_browse->file_types[i])) {
              show_item = true;
              break;
            }
          }
          if (file_browse->num_file_types == 0)
            show_item = true;
        } else {
  ...
  for (int f = 0; f < file_browse->num_cached_files; ++f) {
    const char *ext = file_browse->cached_files[f].is_link
    ...
    if (se_selectable_with_box(file_browse->cached_files[f].name,
                               file_browse->cached_files[f].path, ext, false,
                               0)) {
      if (file_browse->cached_files[f].is_dir)
        strncpy(gui_state.file_browser.current_path,
                file_browse->cached_files[f].path, SB_FILE_PATH_SIZE);
      else {
...
bool se_string_contains_string_case_insensitive(char *canidate, char *search) {
  ...
  for (int i = 0; i < len1 - len2; i++) {
    int j = 0;
    for (j = 0; j < len2; j++) {
      if (tolower(canidate[i + j]) != tolower(search[j]))
        break;
    }
    if (j == len2)
      return true;
...
void se_load_rom_overlay(bool visible) {
  ...
  w_size.y /= se_dpi_scale();
  igSetNextWindowSize((ImVec2){w_size.x, 0}, ImGuiCond_Always);
  igSetNextWindowPos((ImVec2){w_pos.x, w_pos.y}, ImGuiCond_Always,
                     (ImVec2){0, 0});
  igSetNextWindowBgAlpha(
      gui_state.settings.hardcore_mode ? 1.0 : SE_TRANSPARENT_BG_ALPHA);
  igBegin(se_localize_and_cache(ICON_FK_FILE_O " Load Game"),
          gui_state.settings.hardcore_mode ? NULL : &gui_state.overlay_open,
          ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
  ...
  if (gui_state.ui_type == SE_UI_ANDROID || gui_state.ui_type == SE_UI_IOS) {
    ...
    prompt1 = "Load ROM(.gb, .gbc, .gba, .zip), save(.sav), or GBA bios "
              "(gba_bios.bin) from file";
    prompt2 = "You can also drag & drop a ROM/save file to load it";
  }
  bool clicked =
      se_selectable_with_box(prompt1, prompt2, ICON_FK_FOLDER_OPEN, false, 0);
  se_open_file_browser(clicked, x, y, w, h, se_load_rom, valid_rom_file_types,
                       NULL);
  ...
  igSetNextWindowPos((ImVec2){(w_pos.x), list_y_off + w_pos.y},
                     ImGuiCond_Always, (ImVec2){0, 0});
  igSetNextWindowBgAlpha(
      gui_state.settings.hardcore_mode ? 1.0 : SE_TRANSPARENT_BG_ALPHA);
  igBegin(se_localize_and_cache(ICON_FK_CLOCK_O " Load Recently Played Game"),
          NULL, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
  ...
  igSameLine(0, 5);
  igPushItemWidth(-55);
  igInputText("##search", gui_state.search_buffer,
              sizeof(gui_state.search_buffer), ImGuiInputTextFlags_None, NULL,
              NULL);
  igPopItemWidth();
  igSameLine(0, 5);
  ...
  case SE_NO_SORT:
    icon = ICON_FK_LONG_ARROW_DOWN ICON_FK_CLOCK_O;
    break;
  case SE_SORT_ALPHA_ASC:
    icon = ICON_FK_SORT_ALPHA_ASC;
    break;
  case SE_SORT_ALPHA_DESC:
    icon = ICON_FK_SORT_ALPHA_DESC;
    break;
  default:
  ...
  if (se_button(icon, (ImVec2){50, 0})) {
    ...
    if (gui_state.recent_games_sort_type > SE_SORT_ALPHA_DESC)
      gui_state.recent_games_sort_type = SE_NO_SORT;
    se_sort_recent_games_list();
  }
  igSeparator();
  int num_entries = 0;
  for (int i = 0; i < SE_NUM_RECENT_PATHS; ++i) {
    if (gui_state.sorted_recently_loaded_games[i] == -1)
      break;
    se_game_info_t *info = gui_state.recently_loaded_games +
                           gui_state.sorted_recently_loaded_games[i];
    ...
    if (save_exists)
      reduce_width = 85;
#endif
    if (!se_string_contains_string_case_insensitive(info->path,
                                                    gui_state.search_buffer))
      continue;
    if (se_selectable_with_box(file_name, se_replace_fake_path(info->path),
                               ext_upper, false, reduce_width)) {
      se_load_rom(info->path);
    }
#ifdef EMSCRIPTEN
    if (save_exists) {
      igSameLine(0, 4);
      if (se_button(ICON_FK_DOWNLOAD " Export Save",
                    (ImVec2){reduce_width - 4, 40}))
        se_download_emscripten_file(save_file_path);
    }
#endif
    igSeparator();
    num_entries++;
  ...
  if (num_entries == 0)
    se_text("No recently played games");
  igEnd();
  return;
}
#ifdef USE_SDL
static void se_poll_sdl() {
  ...
  while (SDL_PollEvent(&sdlEvent)) {
    ...
    case SDL_JOYDEVICEREMOVED:
      if (cont->sdl_joystick ==
          SDL_JoystickFromInstanceID(sdlEvent.jdevice.which)) {
    ...
    case SDL_JOYBUTTONDOWN:
      if (SDL_JoystickFromInstanceID(sdlEvent.jbutton.which) ==
          cont->sdl_joystick)
    ...
    case SDL_JOYAXISMOTION:
      if (SDL_JoystickFromInstanceID(sdlEvent.jaxis.which) ==
          cont->sdl_joystick) {
        float v = sdlEvent.jaxis.value / 32768.f;
        if (v < 0.2 && v > -0.2 &&
            (int)sdlEvent.jaxis.axis <
                sizeof(cont->axis_last_zero_time) /
                    sizeof(cont->axis_last_zero_time[0])) {
        ...
        double delta =
            se_time() - cont->axis_last_zero_time[sdlEvent.jaxis.axis];
        if (v > 0.4 && delta < 2)
          cont->key.last_bind_activitiy = sdlEvent.jaxis.axis | SE_JOY_POS_MASK;
        if (v < -0.4 && delta < 2)
          cont->key.last_bind_activitiy = sdlEvent.jaxis.axis | SE_JOY_NEG_MASK;
      }
      break;
    }
  }
    ...
    for (int k = 0; k < SE_NUM_KEYBINDS; ++k) {
      ...
      if (is_hat) {
        ...
        if (hat_id < SDL_JoystickNumHats(cont->sdl_joystick)) {
          ...
          if (match_value == SDL_HAT_UP)
            val |= (hat_value == SDL_HAT_UP) | (hat_value == SDL_HAT_RIGHTUP) |
                   (hat_value == SDL_HAT_LEFTUP);
          if (match_value == SDL_HAT_DOWN)
            val |= (hat_value == SDL_HAT_DOWN) |
                   (hat_value == SDL_HAT_RIGHTDOWN) |
                   (hat_value == SDL_HAT_LEFTDOWN);
          if (match_value == SDL_HAT_LEFT)
            val |= (hat_value == SDL_HAT_LEFT) |
                   (hat_value == SDL_HAT_LEFTDOWN) |
                   (hat_value == SDL_HAT_LEFTUP);
          if (match_value == SDL_HAT_RIGHT)
            val |= (hat_value == SDL_HAT_RIGHT) |
                   (hat_value == SDL_HAT_RIGHTDOWN) |
                   (hat_value == SDL_HAT_RIGHTUP);
        }
      } else if (is_joy) {
        int joy_id = SB_BFE(key, 0, 16);
        if (joy_id < SDL_JoystickNumAxes(cont->sdl_joystick)) {
          ...
          val = false;
          if (key & SE_JOY_NEG_MASK)
            val |= v < -0.3;
          if (key & SE_JOY_POS_MASK)
            val |= v > 0.3;
        }
      } else {
...
void se_update_frame() {
#ifdef ENABLE_HTTP_CONTROL_SERVER
  hcs_update(gui_state.settings.http_control_server_enable,
             gui_state.settings.http_control_server_port, se_hcs_callback);
  if (gui_state.settings.http_control_server_enable) {
    for (int i = 0; i < SE_NUM_KEYBINDS; ++i)
      emu_state.joy.inputs[i] += gui_state.hcs_joypad.inputs[i];
  }
  ...
  if (emu_state.run_mode == SB_MODE_RUN || emu_state.run_mode == SB_MODE_STEP ||
    ...
    while (max_frames_per_tick--) {
      ...
      if (emu_state.run_mode == SB_MODE_STEP) {
        ...
        if (unlocked_mode) {
          if (simulation_time < curr_time && emu_state.frame) {
            break;
          }
        } else {
          if (emu_state.frame == 0 && simulation_time > curr_time)
            break;
          if (emu_state.frame &&
              curr_time - simulation_time < sim_time_increment * 0.8) {
            break;
          }
        }
      }
  ...
  if (emu_state.run_mode == SB_MODE_PAUSE)
    emu_state.frame = 0;
  if (emu_state.run_mode == SB_MODE_REWIND)
    emu_state.frame = -emu_state.frame * frames_per_rewind_state;
...
void se_imgui_theme() {
  ...
  // 0.86f, 1.00f}; colors[ImGuiCol_DockingEmptyBg]         = (ImVec4){1.00f,
  // 0.00f, 0.00f, 1.00f};
  ...
  if (gui_state.settings.theme == SE_THEME_CUSTOM) {
    ...
    if (palette[1 * 4 + 3]) {
      ...
      colors[ImGuiCol_PlotLinesHovered] =
          colors[ImGuiCol_PlotHistogramHovered] = colors[ImGuiCol_Text] =
              (ImVec4){r, g, b, a};
      colors[ImGuiCol_TextDisabled] = (ImVec4){r, g, b, a * 0.4f};
      colors[ImGuiCol_ScrollbarGrabHovered] = (ImVec4){r, g, b, a * 0.6f};
      colors[ImGuiCol_SliderGrabActive] = colors[ImGuiCol_ScrollbarGrabActive] =
          (ImVec4){r, g, b, a * 0.8f};
    }
    // Second Color
    if (palette[2 * 4 + 3]) {
    ...
    if (palette[3 * 4 + 3]) {
      ...
      colors[ImGuiCol_TitleBg] = colors[ImGuiCol_TitleBgActive] =
          colors[ImGuiCol_TitleBgCollapsed] = colors[ImGuiCol_TableHeaderBg] =
              colors[ImGuiCol_TableBorderStrong] = (ImVec4){r, g, b, a};

      colors[ImGuiCol_SliderGrab] = colors[ImGuiCol_ScrollbarGrab] =
          (ImVec4){r, g, b, a};
    ...
    if (palette[4 * 4 + 3]) {
      ...
      colors[ImGuiCol_PlotLines] = colors[ImGuiCol_PlotHistogram] =
...
void se_set_default_controller_binds(se_controller_state_t *cont) {
  ...
  /*
  cont->key.bound_id[SE_KEY_EMU_PAUSE] =
  se_get_sdl_key_bind(gc,SDL_CONTROLLER_BUTTON_GUIDE,SE_JOY_POS_MASK);
  cont->key.bound_id[SE_KEY_EMU_REWIND] =
  se_get_sdl_key_bind(gc,SDL_CONTROLLER_BUTTON_PADDLE1,SE_JOY_POS_MASK);
  cont->key.bound_id[SE_KEY_EMU_FF_2X] =
  se_get_sdl_key_bind(gc,SDL_CONTROLLER_BUTTON_PADDLE2,SE_JOY_POS_MASK);
  cont->key.bound_id[SE_KEY_EMU_FF_MAX] =
  se_get_sdl_key_bind(gc,SDL_CONTROLLER_BUTTON_PADDLE3,SE_JOY_POS_MASK);
  */
...
bool se_load_controller_settings(se_controller_state_t *cont) {
  ...
  char settings_path[SB_FILE_PATH_SIZE];
  snprintf(settings_path, SB_FILE_PATH_SIZE, "%s%s-bindings.bin",
           se_get_pref_path(), cont->name);
  bool load_old_settings = sb_load_file_data_into_buffer(
      settings_path, (uint8_t *)bind_map, sizeof(bind_map));
...
int se_get_sdl_key_bind(SDL_GameController *gc, int button,
  ...
  if (bind.bindType == SDL_CONTROLLER_BINDTYPE_HAT) {
    ...
    int mask = 0;
    if (hat_mask & SDL_HAT_UP)
      mask = SDL_HAT_UP;
    if (hat_mask & SDL_HAT_DOWN)
      mask = SDL_HAT_DOWN;
    if (hat_mask & SDL_HAT_LEFT)
      mask = SDL_HAT_LEFT;
    if (hat_mask & SDL_HAT_RIGHT)
      mask = SDL_HAT_RIGHT;
    if (!mask)
      return -1;
...
int se_get_sdl_axis_bind(SDL_GameController *gc, int button) {
  SDL_GameControllerButtonBind bind =
      SDL_GameControllerGetBindForAxis(gc, button);
  if (bind.bindType != SDL_CONTROLLER_BINDTYPE_AXIS)
    return -1;
  else
...
void se_set_default_controller_binds(se_controller_state_t *cont) {
  ...
  for (int i = 0; i < SE_NUM_KEYBINDS; ++i)
    cont->key.bound_id[i] = -1;
  cont->key.bound_id[SE_KEY_A] =
      se_get_sdl_key_bind(gc, SDL_CONTROLLER_BUTTON_A, SE_JOY_POS_MASK);
  cont->key.bound_id[SE_KEY_B] =
      se_get_sdl_key_bind(gc, SDL_CONTROLLER_BUTTON_B, SE_JOY_POS_MASK);
  cont->key.bound_id[SE_KEY_X] =
      se_get_sdl_key_bind(gc, SDL_CONTROLLER_BUTTON_X, SE_JOY_POS_MASK);
  cont->key.bound_id[SE_KEY_Y] =
      se_get_sdl_key_bind(gc, SDL_CONTROLLER_BUTTON_Y, SE_JOY_POS_MASK);
  cont->key.bound_id[SE_KEY_L] = se_get_sdl_key_bind(
      gc, SDL_CONTROLLER_BUTTON_LEFTSHOULDER, SE_JOY_POS_MASK);
  cont->key.bound_id[SE_KEY_R] = se_get_sdl_key_bind(
      gc, SDL_CONTROLLER_BUTTON_RIGHTSHOULDER, SE_JOY_POS_MASK);
  cont->key.bound_id[SE_KEY_UP] =
      se_get_sdl_key_bind(gc, SDL_CONTROLLER_BUTTON_DPAD_UP, SE_JOY_POS_MASK);
  cont->key.bound_id[SE_KEY_DOWN] =
      se_get_sdl_key_bind(gc, SDL_CONTROLLER_BUTTON_DPAD_DOWN, SE_JOY_NEG_MASK);
  cont->key.bound_id[SE_KEY_LEFT] =
      se_get_sdl_key_bind(gc, SDL_CONTROLLER_BUTTON_DPAD_LEFT, SE_JOY_NEG_MASK);
  cont->key.bound_id[SE_KEY_RIGHT] = se_get_sdl_key_bind(
      gc, SDL_CONTROLLER_BUTTON_DPAD_RIGHT, SE_JOY_POS_MASK);
  cont->key.bound_id[SE_KEY_START] =
      se_get_sdl_key_bind(gc, SDL_CONTROLLER_BUTTON_START, SE_JOY_POS_MASK);
  cont->key.bound_id[SE_KEY_SELECT] =
      se_get_sdl_key_bind(gc, SDL_CONTROLLER_BUTTON_BACK, SE_JOY_POS_MASK);
  ...
  cont->analog.bound_id[SE_ANALOG_UP_DOWN] =
      se_get_sdl_axis_bind(gc, SDL_CONTROLLER_AXIS_LEFTY);
  cont->analog.bound_id[SE_ANALOG_LEFT_RIGHT] =
      se_get_sdl_axis_bind(gc, SDL_CONTROLLER_AXIS_LEFTX);
  cont->analog.bound_id[SE_ANALOG_L] =
      se_get_sdl_axis_bind(gc, SDL_CONTROLLER_AXIS_TRIGGERLEFT);
  cont->analog.bound_id[SE_ANALOG_R] =
      se_get_sdl_axis_bind(gc, SDL_CONTROLLER_AXIS_TRIGGERRIGHT);
}
void se_set_new_controller(se_controller_state_t *cont, int index) {
  SDL_Joystick *joy = SDL_JoystickOpen(index);
  if (joy == cont->sdl_joystick)
    return;
  if (cont->sdl_joystick)
    SDL_JoystickClose(cont->sdl_joystick);
  if (cont->sdl_gc)
...
void se_draw_controller_config(gui_state_t *gui) {
  ...
  if (igBeginCombo(se_localize_and_cache("Controller"),
                   se_localize_and_cache(cont_name), ImGuiComboFlags_None)) {
    {
      bool is_selected = cont->sdl_joystick == NULL;
      if (igSelectableBool(se_localize_and_cache("No Controller"), is_selected,
    ...
    for (int j = 0; j < SDL_NumJoysticks(); ++j) {
      ...
      if (igSelectableBool(jname, is_selected, ImGuiSelectableFlags_None,
                           (ImVec2){0, 0})) {
  ...
  if (modified) {
    ...
    char settings_path[SB_FILE_PATH_SIZE];
    snprintf(settings_path, SB_FILE_PATH_SIZE, "%s%s-bindings.bin",
             se_get_pref_path(), cont_name);
    sb_save_file_data(settings_path, (uint8_t *)bind_map, sizeof(bind_map));
    se_emscripten_flush_fs();
  }
#ifdef USE_SDL
  if (SDL_JoystickHasRumble(cont->sdl_joystick)) {
    se_text("Rumble Supported");
  } else
    se_text("Rumble Not Supported");
#endif
}

void se_reset_default_gb_palette() {
  uint8_t palette[4 * 3] = {0x81, 0x8F, 0x38, 0x64, 0x7D, 0x43,
                            0x56, 0x6D, 0x3F, 0x31, 0x4A, 0x2D};
  for (int i = 0; i < 4; ++i) {
    gui_state.settings.gb_palette[i] =
        palette[i * 3] | (palette[i * 3 + 1] << 8) | (palette[i * 3 + 2] << 16);
  }
}
...
void se_draw_touch_controls_settings() {
  ...
  float scale = (igGetWindowContentRegionWidth() - 2) /
                (aspect_ratio + 1.0 / aspect_ratio);

  igDummy(
      (ImVec2){0, (igGetWindowContentRegionWidth() * 0.5 - 2 - scale) * 0.5});
  if (igBeginChildFrame(1, (ImVec2){scale * aspect_ratio, scale},
                        ImGuiWindowFlags_None)) {
  ...
  if (igBeginChildFrame(2, (ImVec2){scale / aspect_ratio, scale},
                        ImGuiWindowFlags_None)) {
    se_draw_emulated_system_screen(true);
  }
  igEndChildFrame();
  ...
  igSameLine(SE_FIELD_INDENT, 0);
  igPushItemWidth(-1);
  se_slider_float("##TouchControlsScale",
                  &gui_state.settings.touch_controls_scale, 0.3, 1.0,
                  "Scale: %.2f");

  se_text("Opacity");
  igSameLine(SE_FIELD_INDENT, 0);
  igPushItemWidth(-1);
  se_slider_float("##TouchControlsOpacity",
                  &gui_state.settings.touch_controls_opacity, 0, 1.0,
                  "Opacity: %.2f");
  igPopItemWidth();
...
void se_draw_menu_panel() {
  ...
  if (gui_state.settings.hardcore_mode)
    ...
    for (int i = 0; i < SE_NUM_SAVE_STATES; ++i) {
      ...
      igBeginChildFrame(i + 100, (ImVec2){slot_w, slot_h},
                        ImGuiWindowFlags_None);
      ImVec2 screen_p;
      ...
      if (se_button(se_localize_and_cache("Capture"), (ImVec2){button_w, 0}))
        se_capture_state_slot(i);
      if (!save_states[i].valid)
        se_push_disabled();
      if (se_button(se_localize_and_cache("Restore"), (ImVec2){button_w, 0}))
      ...
      if (save_states[i].valid) {
        float w_scale = 1.0;
        float h_scale = 1.0;
        if (save_states[i].screenshot_width >
            save_states[i].screenshot_height) {
          h_scale = (float)save_states[i].screenshot_height /
                    (float)save_states[i].screenshot_width;
        } else {
          w_scale = (float)save_states[i].screenshot_width /
                    (float)save_states[i].screenshot_height;
        }
        screen_w *= w_scale;
        screen_h *= h_scale;
        ...
        se_draw_image(
            save_states[i].screenshot, save_states[i].screenshot_width,
            save_states[i].screenshot_height, screen_x * se_dpi_scale(),
            screen_y * se_dpi_scale(), screen_w * se_dpi_scale(),
            screen_h * se_dpi_scale(), true);
        if (save_states[i].valid == 2) {
          igSetCursorScreenPos((ImVec2){screen_x + screen_w * 0.5 - 15,
                                        screen_y + screen_h * 0.5 - 15});
          se_button(ICON_FK_EXCLAMATION_TRIANGLE, (ImVec2){30, 30});
          se_tooltip("This save state came from an incompatible build. SkyEmu "
                     "has attempted to recover it, but there may be issues");
        }
      } else {
        screen_h *= 0.85;
        screen_x += button_w + (slot_w - screen_w - button_w) * 0.5;
        screen_y += (slot_h - screen_h) * 0.5 - style->FramePadding.y;
        ImU32 color =
            igColorConvertFloat4ToU32(style->Colors[ImGuiCol_MenuBarBg]);
        ImDrawList_AddRectFilled(
            igGetWindowDrawList(), (ImVec2){screen_x, screen_y},
            (ImVec2){screen_x + screen_w, screen_y + screen_h}, color, 0,
            ImDrawCornerFlags_None);
        ImVec2 anchor;
        igSetCursorScreenPos((ImVec2){screen_x + screen_w * 0.5 - 5,
                                      screen_y + screen_h * 0.5 - 5});
        se_text(ICON_FK_BAN);
      }
      igEndChildFrame();
    }
    ...
    if (gui_state.settings.hardcore_mode)
      ...
      for (int i = 0; i < SE_NUM_CHEATS; i++) {
        ...
        if (gui_state.editing_cheat_index == i) {
          ...
          igInputText("##Name", cheat->name, SE_MAX_CHEAT_NAME_SIZE - 1,
        ...
        if (gui_state.editing_cheat_index == i) {
          ...
          for (int i = 0; i < cheat->size; i += 1) {
            off += snprintf(code_buffer + off, sizeof(code_buffer) - off,
                            "%08X", cheat->buffer[i]);
            if (i % 2)
              off +=
                  snprintf(code_buffer + off, sizeof(code_buffer) - off, "\n");
            else
              off +=
                  snprintf(code_buffer + off, sizeof(code_buffer) - off, " ");
          }
          igSetNextItemWidth(win_w);
          // Not setting ImGuiInputTextFlags_CharsHexadecimal as it doesn't
          // allow whitespace
          if (igInputTextMultiline("##CheatCode", code_buffer,
                                   sizeof(code_buffer), (ImVec2){0, 300},
                                   ImGuiInputTextFlags_CharsUppercase, NULL,
                                   NULL)) {
    ...
    if (emu_state.rom_loaded) {
      ...
      for (int i = 0; i < sizeof(info->name) / sizeof(info->name[0]); ++i) {
        if (info->name[i][0]) {
          ...
          se_input_file_callback(info->name[i], info->path[i], bios_types,
                                 se_bios_file_open_fn,
                                 ImGuiInputTextFlags_None);
        }
      }
      if (missing_bios) {
  ...
  igPushItemWidth(-1);
  se_text("Screen Shader");
  igSameLine(SE_FIELD_INDENT, 0);
  se_combo_str(
      "##Screen Shader", &v,
      "Pixelate\0Bilinear\0LCD\0LCD & Subpixels\0Smooth Upscale (xBRZ)\0", 0);
  gui_state.settings.screen_shader = v;
  v = gui_state.settings.screen_rotation;
  se_text("Screen Rotation");
  igSameLine(SE_FIELD_INDENT, 0);
  se_combo_str("##Screen Rotation", &v,
               "0 degrees\00090 degrees\000180 degrees\000270 degrees\0", 0);
  gui_state.settings.screen_rotation = v;
  se_text("Color Correction");
  igSameLine(SE_FIELD_INDENT, 0);
  se_slider_float("##Color Correction", &gui_state.settings.color_correction, 0,
                  1.0, "Strength: %.2f");
  int color_correct = gui_state.settings.gba_color_correction_mode;
  se_text("GBA Color Correction Type");
  igSameLine(180, 0);
  ...
  if (se_button("Reset Palette to Defaults", (ImVec2){0, 0}))
  ...
  if (gui_state.ui_type == SE_UI_ANDROID || gui_state.ui_type == SE_UI_IOS) {
    ...
    bool value = true;
    bool modified =
        se_handle_keybind_settings(SE_BIND_KEYBOARD, &gui_state.key);
    ...
    if (modified) {
      char settings_path[SB_FILE_PATH_SIZE];
      snprintf(settings_path, SB_FILE_PATH_SIZE, "%skeyboard-bindings.bin",
               se_get_pref_path());
      sb_save_file_data(settings_path, (uint8_t *)gui_state.key.bound_id,
                        sizeof(gui_state.key.bound_id));
      se_emscripten_flush_fs();
    }
  }
#if defined(USE_SDL) || defined(PLATFORM_ANDROID)
  ...
  igSameLine(SE_FIELD_INDENT, 0);
  igPushItemWidth(-1);
  if (igBeginCombo("##Language",
                   se_language_string(gui_state.settings.language),
    ...
    for (int lang_id = 0; lang_id < SE_MAX_LANG_VALUE; ++lang_id) {
      ...
      if (lang[0]) {
        if (igSelectableBool(lang, false, ImGuiSelectableFlags_None,
                             (ImVec2){0, 0}))
          gui_state.settings.language = lang_id;
      }
    }
    igEndCombo();
  }
  ...
  igPushItemWidth(-1);
  bool load =
      se_combo_str("##Theme", &theme, "Dark\0Light\0Black\0Custom\0", 0);
  igPopItemWidth();
  if (gui_state.settings.theme == SE_THEME_CUSTOM) {
    const char *types[] = {"*.png", NULL};
    load |= se_input_file("Theme Path", gui_state.paths.theme, types,
                          ImGuiInputTextFlags_None);
    load |= strncmp(gui_state.loaded_theme_path, gui_state.paths.theme,
                    SB_FILE_PATH_SIZE) != 0;
    if (load) {
    ...
    load |= se_input_file("Custom Font", gui_state.paths.custom_font,
                          font_types, ImGuiInputTextFlags_None);
    load |= strncmp(gui_state.loaded_custom_font_path,
                    gui_state.paths.custom_font, SB_FILE_PATH_SIZE) != 0;
    if (load) {
      gui_state.update_font_atlas = true;
      strncpy(gui_state.loaded_custom_font_path, gui_state.paths.custom_font,
              SB_FILE_PATH_SIZE);
    }

    float old_scale = gui_state.settings.custom_font_scale;
    se_slider_float("##FontScale", &gui_state.settings.custom_font_scale, 0.5,
                    1.5, "Font Scale: %0.2fx");
    if (old_scale != gui_state.settings.custom_font_scale)
      gui_state.update_font_atlas = true;

    se_custom_theme_t *theme = &gui_state.theme;
    float w = igGetWindowContentRegionWidth();
    float h =
        w * theme->regions[SE_REGION_NAME].h / theme->regions[SE_REGION_NAME].w;
    ImVec2 p, v;
  ...
  if (gui_state.ui_type == SE_UI_DESKTOP) {
    ...
    igSeparator();
    se_input_path("Save File/State Path", gui_state.paths.save,
                  ImGuiInputTextFlags_None);
    se_input_path("BIOS/Firmware Path", gui_state.paths.bios,
                  ImGuiInputTextFlags_None);
    se_input_path("Cheat Code Path", gui_state.paths.cheat_codes,
                  ImGuiInputTextFlags_None);
    ...
    if (memcmp(&gui_state.last_saved_paths, &gui_state.paths,
               sizeof(gui_state.paths))) {
      se_save_search_paths();
  ...
  igSeparator();
  se_text("Solar Sensor");
  igSameLine(SE_FIELD_INDENT, 0);
  igPushItemWidth(-1);
  se_slider_float("##Solar Sensor", &emu_state.joy.solar_sensor, 0., 1.,
                  "Brightness: %.2f");
...
static void se_reset_audio_ring() {
  // Reset the audio ring to 50% full with empty samples to avoid crackles while
  // the buffer fills back up.
  emu_state.audio_ring_buff.read_ptr = 0;
  emu_state.audio_ring_buff.write_ptr = 4096;
  for (int i = 0; i < SB_AUDIO_RING_BUFFER_SIZE; ++i)
    emu_state.audio_ring_buff.data[i] = 0;
}
static void se_init_audio() {
  saudio_setup(&(saudio_desc){.sample_rate = SE_AUDIO_SAMPLE_RATE,
                              .num_channels = 2,
                              .num_packets = 4,
...
bool se_begin_menu_bar() {
  ...
  se_draw_theme_region(SE_REGION_MENUBAR, 0, y_off, menu_bar_size.x,
                       menu_bar_size.y);
  return true; //-V1020
}

void se_end_menu_bar() { igEnd(); }
  ...
  if (strcmp(cmd, "/ping") == 0)
    ...
    while (*params) {
      if (strcmp(params[0], "ui_type") == 0) {
        if (strcmp(params[1], "DESKTOP") == 0)
          gui_state.ui_type = SE_UI_DESKTOP;
        if (strcmp(params[1], "ANDROID") == 0)
          gui_state.ui_type = SE_UI_ANDROID;
        if (strcmp(params[1], "IOS") == 0)
          gui_state.ui_type = SE_UI_IOS;
        if (strcmp(params[1], "WEB") == 0)
          gui_state.ui_type = SE_UI_WEB;
      } else if (strcmp(params[0], "menu") == 0)
        gui_state.sidebar_open = atoi(params[1]);
      else if (strcmp(params[0], "dpi") == 0)
        gui_state.dpi_override = atof(params[1]);
      else if (strcmp(params[0], "touch_controls_scale") == 0)
        gui_state.settings.touch_controls_scale = atof(params[1]);
      else if (strcmp(params[0], "language") == 0)
        gui_state.settings.language = se_convert_locale_to_enum(params[1]);
      else if (strcmp(params[0], "shader") == 0)
        gui_state.settings.screen_shader = atof(params[1]);
      else if (strcmp(params[0], "load_slot") == 0)
        se_restore_state_slot(atoi(params[1]));
      else if (strcmp(params[0], "capture_slot") == 0)
        se_capture_state_slot(atoi(params[1]));
      else if (strcmp(params[0], "edit_cheat_index") == 0)
        gui_state.editing_cheat_index = atoi(params[1]);
      else if (strcmp(params[0], "debug_tools") == 0)
        gui_state.settings.draw_debug_menu = atoi(params[1]);
      else if (strcmp(params[0], "fake_paths") == 0)
        gui_state.fake_paths = atoi(params[1]);
      else if (strcmp(params[0], "theme") == 0)
        gui_state.settings.theme = atoi(params[1]);
    ...
    while (*params) {
      if (strcmp(params[0], "frames") == 0)
        step_frames = atoi(params[1]);
      params += 2;
    }
    ...
    if (emu_state.rom_loaded) {
      ...
      int format = 0;
      while (*params) {
        if (strcmp(params[0], "embed_state") == 0)
          embed_state = atoi(params[1]) != 0;
        if (strcmp(params[0], "format") == 0) {
          if (strcmp(params[1], "BMP") == 0 || strcmp(params[1], "bmp") == 0)
            format = 1;
          if (strcmp(params[1], "JPG") == 0 || strcmp(params[1], "jpg") == 0)
            format = 2;
        }
        params += 2;
      }
      ...
      uint32_t width = 0, height = 0;
      if (embed_state) {
        se_save_state_t *save_state =
            (se_save_state_t *)malloc(sizeof(se_save_state_t));
      ...
      if (format == 0) {
        ...
        stbi_write_png_to_func(se_png_write_mem, &cont, width, height, 4,
                               imdata, 0);
        free(imdata);
        ...
        *mime_type = "image/png";
        ...
        se_png_write_context_t cont = {0};
        stbi_write_bmp_to_func(se_png_write_mem, &cont, width, height, 4,
                               imdata);
        free(imdata);
        *result_size = cont.size;
        *mime_type = "image/bmp";
        ...
        se_png_write_context_t cont = {0};
        stbi_write_jpg_to_func(se_png_write_mem, &cont, width, height, 4,
                               imdata, 95);
        free(imdata);
        *result_size = cont.size;
        *mime_type = "image/jpg";
    ...
    while (*params) {
      if (strcmp(params[0], "map") == 0)
        ...
        se_append_char_to_string(&response, &response_size,
                                 map[SB_BFE(byte, 4, 4)]);
        se_append_char_to_string(&response, &response_size,
                                 map[SB_BFE(byte, 0, 4)]);
      }
    ...
    *mime_type = "application/json";
    ...
    case SB_MODE_PAUSE:
      off += snprintf(buffer + off, sizeof(buffer) - off, "\"PAUSE\",\n");
      break;
    case SB_MODE_RUN:
      off += snprintf(buffer + off, sizeof(buffer) - off, "\"RUN\",\n");
      break;
    case SB_MODE_STEP:
      off += snprintf(buffer + off, sizeof(buffer) - off, "\"STEP\",\n");
      break;
    case SB_MODE_RESET:
      off += snprintf(buffer + off, sizeof(buffer) - off, "\"RESET\",\n");
      break;
    case SB_MODE_REWIND:
      off += snprintf(buffer + off, sizeof(buffer) - off, "\"REWIND\",\n");
      break;
    }
    off +=
        snprintf(buffer + off, sizeof(buffer) - off, "  \"rom-loaded\" : %s,\n",
    ...
    off +=
        snprintf(buffer + off, sizeof(buffer) - off, "  \"rewind-info\" : {\n");
    off += snprintf(buffer + off, sizeof(buffer) - off,
                    "    \"entries-used\" : %d,\n", rewind_buffer.size);
    off += snprintf(buffer + off, sizeof(buffer) - off,
    ...
    *mime_type = "text/plain";
    ...
    if (cheat_count == 0) {
      ...
      size_t max_size = SE_MAX_CHEAT_CODE_SIZE * cheat_count +
                        SE_MAX_CHEAT_NAME_SIZE * cheat_count + 64 * cheat_count;
      char *max_buffer = (char *)calloc(max_size, sizeof(char));
      int off = 0;

      for (int i = 0; i < SE_NUM_CHEATS; ++i) {
        if (cheats[i].state != -1) {
          off += snprintf(max_buffer + off, max_size - off, "%d - %s:", i,
                          cheats[i].name);
          for (int j = 0; j < cheats[i].size; ++j) {
            off += snprintf(max_buffer + off, max_size - off, " %08x",
                            cheats[i].buffer[j]);
          }
          if (cheats[i].state == 0) {
...
static void frame(void) {
  ...
  if (emu_state.joy.inputs[SE_KEY_TOGGLE_FULLSCREEN] &&
      last_toggle_fullscreen == false)
    sapp_toggle_fullscreen();
  ...
  if (!last_back_press && gui_state.button_state[SAPP_KEYCODE_BACK]) {
    ...
    else if (emu_state.run_mode != SB_MODE_PAUSE)
      emu_state.run_mode = SB_MODE_PAUSE;
    else if (emu_state.rom_loaded && !gui_state.ran_from_launcher)
      emu_state.run_mode = SB_MODE_RUN;
    else
      sapp_quit();
  }
  ...
  const double delta_time = stm_sec(
      stm_round_to_common_refresh_rate(stm_laptime(&gui_state.laptime)));
  ...
  if (gui_state.test_runner_mode == false && se_begin_menu_bar()) {
    ...
    if (gui_state.sidebar_open) {
      igPushStyleColorVec4(ImGuiCol_Button,
                           style->Colors[ImGuiCol_ButtonActive]);
      if (se_button_themed(
              SE_REGION_MENU + 2, ICON_FK_TIMES,
              (ImVec2){SE_MENU_BAR_BUTTON_WIDTH, SE_MENU_BAR_HEIGHT}, false)) {
        gui_state.sidebar_open = !gui_state.sidebar_open;
      }
      igPopStyleColor(1);
    } else {
      if (se_button_themed(
              SE_REGION_MENU, ICON_FK_BARS,
              (ImVec2){SE_MENU_BAR_BUTTON_WIDTH, SE_MENU_BAR_HEIGHT}, false)) {
        gui_state.sidebar_open = !gui_state.sidebar_open;
      }
    }
    ...
    if (gui_state.ui_type != SE_UI_ANDROID && gui_state.ui_type != SE_UI_IOS) {
      ...
      if (vol_x < toggle_x + (sel_width + 1) * num_toggles)
        vol_x = toggle_x + (sel_width + 1) * num_toggles;
  ...
  if (!active) {
    ...
    bool draw_sidebars_over_screen =
        scaled_screen_width - sidebar_w * num_sidebars_open < sidebar_w * 0.5;
    ...
    if (gui_state.sidebar_open) {
      igSetNextWindowPos((ImVec2){screen_x, menu_height}, ImGuiCond_Always,
                         (ImVec2){0, 0});
      igSetNextWindowSize((ImVec2){sidebar_w, (gui_state.screen_height -
                                               menu_height * se_dpi_scale()) /
                                                  se_dpi_scale()},
                          ImGuiCond_Always);
      igBegin(se_localize_and_cache("Menu"), &gui_state.sidebar_open,
              ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
      se_draw_menu_panel();
      igEnd();
      screen_x += sidebar_w;
    ...
    if (gui_state.settings.draw_debug_menu) {
      ...
      screen_x = se_draw_debug_panels(screen_x, sidebar_w, menu_height,
                                      (height - menu_height * se_dpi_scale()) /
                                          se_dpi_scale());
    ...
    igSetNextWindowPos((ImVec2){screen_x, menu_height}, ImGuiCond_Always,
                       (ImVec2){0, 0});
    igSetNextWindowSize(
        (ImVec2){screen_width, height - menu_height * se_dpi_scale()},
        ImGuiCond_Always);
    ...
    for (int i = 0; i < SAPP_MAX_TOUCHPOINTS; ++i) {
      if (gui_state.touch_points[i].active)
    ...
    igPopStyleColor(1);
    igEnd();
    bool draw_click_region = emu_state.run_mode != SB_MODE_RUN &&
                             emu_state.run_mode != SB_MODE_REWIND &&
                             !draw_sidebars_over_screen &&
                             (gui_state.overlay_open || !emu_state.rom_loaded);
    if (draw_click_region) {
      ...
      igSetNextWindowSize(
          (ImVec2){screen_width, height - menu_height * se_dpi_scale()},
          ImGuiCond_Always);
      igBegin("##ClickRegion", &gui_state.overlay_open,
  ...
  if (gui_state.update_font_atlas) {
    ...
    if (!font) {
      ...
      const uint8_t *karla_compressed_data =
          se_get_resource(SE_KARLA, &karla_compressed_size);
      font = ImFontAtlas_AddFontFromMemoryCompressedTTF(
          atlas, karla_compressed_data, karla_compressed_size,
          13 * se_dpi_scale() * font_scale, NULL, NULL);
    }

    uint64_t forkawesome_compressed_size;
    const uint8_t *forkawesome_compressed_data =
        se_get_resource(SE_FORKAWESOME, &forkawesome_compressed_size);

    static const ImWchar icons_ranges[] = {
        ICON_MIN_FK, ICON_MAX_FK,
        0}; // Will not be copied by AddFont* so keep in scope.
    ImFontConfig *config = ImFontConfig_ImFontConfig();
    config->MergeMode = true;
    config->GlyphMinAdvanceX = 13.0f;
    ImFont *font2 = ImFontAtlas_AddFontFromMemoryCompressedTTF(
        atlas, forkawesome_compressed_data, forkawesome_compressed_size,
        13 * se_dpi_scale() * font_scale, config, icons_ranges);
    ImFontConfig_destroy(config);
    igGetIO()->FontDefault = font2;

#ifdef UNICODE_GUI
    uint64_t notosans_cjksc_compressed_size;
    const uint8_t *notosans_cjksc_compressed_data =
        se_get_resource(SE_NOTO, &notosans_cjksc_compressed_size);
    ...
    static ImWchar
        ranges[((SE_MAX_UNICODE_CODE_POINT + 1) / SE_FONT_CACHE_PAGE_SIZE) * 2 +
               1] = {0};
    int index = 0;
    for (int i = 0;
         i < ((SE_MAX_UNICODE_CODE_POINT + 1) / SE_FONT_CACHE_PAGE_SIZE); ++i) {
      if (gui_state.font_cache_page_valid[i] == 0x1) {
        ranges[index * 2] = i * SE_FONT_CACHE_PAGE_SIZE;
        if (ranges[index * 2] == 0)
          ranges[index * 2] = 1;
        ranges[index * 2 + 1] =
            i * SE_FONT_CACHE_PAGE_SIZE + SE_FONT_CACHE_PAGE_SIZE;
        index++;
      }
    }
    ImFont *font3 = ImFontAtlas_AddFontFromMemoryCompressedTTF(
        atlas, notosans_cjksc_compressed_data, notosans_cjksc_compressed_size,
        14 * se_dpi_scale() * font_scale, config3, ranges);
    uint64_t noto_armenian_size;
    const uint8_t *noto_armenian =
        se_get_resource(SE_NOTO_ARMENIAN, &noto_armenian_size);
    ImFont *font4 = ImFontAtlas_AddFontFromMemoryCompressedTTF(
        atlas, noto_armenian, noto_armenian_size,
        14 * se_dpi_scale() * font_scale, config3, ranges);
    uint64_t noto_sans_size = 0;
    const uint8_t *noto_sans = se_get_resource(SE_NOTO_SANS, &noto_sans_size);
    ImFont *font5 = ImFontAtlas_AddFontFromMemoryCompressedTTF(
        atlas, noto_sans, noto_sans_size, 14 * se_dpi_scale() * font_scale,
        config3, ranges);
...

    {
      uint64_t karla_compressed_size;
      const uint8_t *karla_compressed_data =
          se_get_resource(SE_SV_BASIC_MANUAL, &karla_compressed_size);
    ...
    int bytes_per_pixel;
    ImFontAtlas_GetTexDataAsRGBA32(atlas, &font_pixels, &font_width,
    ...
    if (has_atlas_image)
      sg_destroy_image(gui_state.font_atlas_image);
  ...
  se_free_all_images();
  if (memcmp(&gui_state.last_saved_settings, &gui_state.settings,
             sizeof(gui_state.settings))) {
    char settings_path[SB_FILE_PATH_SIZE];
    snprintf(settings_path, SB_FILE_PATH_SIZE, "%suser_settings.bin",
             se_get_pref_path());
    sb_save_file_data(settings_path, (uint8_t *)&gui_state.settings,
                      sizeof(gui_state.settings));
    se_emscripten_flush_fs();
...
void se_load_settings() {
...

  {
    ...
    snprintf(keybind_path, SB_FILE_PATH_SIZE, "%skeyboard-bindings.bin",
             se_get_pref_path());
    if (!sb_load_file_data_into_buffer(keybind_path,
                                       (uint8_t *)gui_state.key.bound_id,
                                       sizeof(gui_state.key.bound_id))) {
      se_set_default_keybind(&gui_state);
    }
  }
...
#endif
  {
    ...
    snprintf(settings_path, SB_FILE_PATH_SIZE, "%suser_settings.bin",
             se_get_pref_path());
    if (!sb_load_file_data_into_buffer(settings_path,
                                       (void *)&gui_state.settings,
                                       sizeof(gui_state.settings))) {
      gui_state.settings.settings_file_version = -1;
    }
    int max_settings_version_supported = 3;
    if (gui_state.settings.settings_file_version >
        max_settings_version_supported) {
    ...
    if (gui_state.settings.screen_shader > 4)
      gui_state.settings.screen_shader = 4;
    ...
    if (gui_state.settings.custom_font_scale < 0.5)
      gui_state.settings.custom_font_scale = 1.0;
    if (gui_state.settings.custom_font_scale > 2.0)
      gui_state.settings.custom_font_scale = 1.0;
    if (gui_state.settings.touch_controls_scale < 0.1)
      gui_state.settings.touch_controls_scale = 1.0;
    if (gui_state.settings.touch_controls_opacity < 0 ||
        gui_state.settings.touch_controls_opacity > 1.0)
      gui_state.settings.touch_controls_opacity = 0.5;
    if (gui_state.settings.gba_color_correction_mode > GBA_HIGAN_CORRECTION)
      gui_state.settings.gba_color_correction_mode = GBA_SKYEMU_CORRECTION;
    gui_state.last_saved_settings = gui_state.settings;
    if (gui_state.settings.theme == SE_THEME_CUSTOM)
      se_load_theme_from_file(gui_state.paths.theme);
  }
}
static void se_compute_draw_lcd_rect(float *lcd_render_w, float *lcd_render_h,
                                     bool *hybrid_nds) {
  *hybrid_nds = false;
  float rotation = gui_state.settings.screen_rotation * 0.5 * 3.14159;
  if (!gui_state.settings.stretch_to_fit) {
    ...
    *lcd_render_h = native_h * render_scale;
    ...
    if (gui_state.settings.integer_scaling) {
      ...
      float old_h = *lcd_render_h;
      float dpi_scale = se_dpi_scale();
      *lcd_render_h = ((int)((*lcd_render_h) / (native_h / dpi_scale))) *
                      native_h / dpi_scale;
      *lcd_render_w = ((int)((*lcd_render_w) / (native_w / dpi_scale))) *
                      native_w / dpi_scale;
      if (*lcd_render_w == 0)
        *lcd_render_w = old_w;
      if (*lcd_render_h == 0)
...
static void se_draw_lcd_in_rect(float lcd_render_x, float lcd_render_y,
                                float lcd_render_w, float lcd_render_h,
                                bool hybrid_nds) {
  float dpi_scale = se_dpi_scale();
  ...
  if (emu_state.system == SYSTEM_GBA) {
    se_draw_lcd_defer(core.gba.framebuffer, GBA_LCD_W, GBA_LCD_H, lx, ly, lw,
                      lh, rotation, false);
  } else if (emu_state.system == SYSTEM_NDS) {
    if (hybrid_nds) {
      float p[6] = {
          0.3333 * lw, -lh * 0.25, 0.3333 * lw, lh * 0.25, -0.1666 * lw, 0,
      };
      for (int i = 0; i < 3; ++i) {
        float x = p[i * 2 + 0];
        float y = p[i * 2 + 1];
      ...
      se_draw_lcd_defer(core.nds.framebuffer_top, NDS_LCD_W, NDS_LCD_H,
                        lx + p[0], ly + p[1], lw / 3, lh * 0.5, rotation,
                        false);
      se_draw_lcd_defer(core.nds.framebuffer_bottom, NDS_LCD_W, NDS_LCD_H,
                        lx + p[2], ly + p[3], lw / 3, lh * 0.5, rotation, true);
      se_draw_lcd_defer(core.nds.framebuffer_top, NDS_LCD_W, NDS_LCD_H,
                        lx + p[4], ly + p[5], lw * 2 / 3, lh, rotation, false);
    } else {
      float p[4] = {0, -lh * 0.25, 0, lh * 0.25};
      for (int i = 0; i < 2; ++i) {
        float x = p[i * 2 + 0];
        float y = p[i * 2 + 1];
        p[i * 2 + 0] = x * cos(-rotation) + y * sin(-rotation);
      ...
      se_draw_lcd_defer(core.nds.framebuffer_top, NDS_LCD_W, NDS_LCD_H,
                        lx + p[0], ly + p[1], lw, lh * 0.5, rotation, false);
      se_draw_lcd_defer(core.nds.framebuffer_bottom, NDS_LCD_W, NDS_LCD_H,
                        lx + p[2], ly + p[3], lw, lh * 0.5, rotation, true);
    }
  } else if (emu_state.system == SYSTEM_GB) {
    se_draw_lcd_defer(core.gb.lcd.framebuffer, SB_LCD_W, SB_LCD_H, lx, ly, lw,
                      lh, rotation, false);
  }
}
static bool se_draw_theme_region_tint_partial(int region, float x, float y,
  ...
  if (!r->active)
    return false;
  if (gui_state.theme.image.id == SG_INVALID_ID)
    return false;
  if (w == 0 || h == 0)
    return false;
  ...
  for (int axis = 0; axis < 2; ++axis)
    for (int i = 0; i < SE_MAX_CONTROL_POINTS; ++i) {
      se_control_point_t *cp =
          axis ? r->control_points_y + i : r->control_points_x + i;
      bool fixed = cp->resize_control == SE_RESIZE_FIXED;
      bool screen = cp->screen_control == SE_SCREEN_BOTH;
      if (fixed && cp->screen_control == 0)
        fixed_pixels[axis] += cp->end_pixel - cp->start_pixel;
      if (screen)
        screen_pixels[axis] += cp->end_pixel - cp->start_pixel;
      if (fixed && screen)
        fixed_screen_pixels[axis] += cp->end_pixel - cp->start_pixel;
    }
  ...
  SE_RPT2 lcd_non_fixed_scale[r] = (lcd_dims[r] - fixed_screen_pixels[r]) /
                                   (screen_pixels[r] - fixed_screen_pixels[r]);

  float non_fixed_pixels[2];
  SE_RPT2 non_fixed_pixels[r] = dims[r] - fixed_pixels[r] - lcd_dims[r];
  float non_fixed_pixels_scale[2];
  int rdims[2] = {r->w, r->h};
  SE_RPT2 non_fixed_pixels_scale[r] =
      (non_fixed_pixels[r]) /
      (rdims[r] * uniform_scale_factor - fixed_pixels[r] - screen_pixels[r]);
  ...
  for (int yc = 0; yc < SE_MAX_CONTROL_POINTS; ++yc) {
    ...
    if (ycp->start_pixel >= ycp->end_pixel)
      continue;
    pmax.x = pmin.x = x;
    ...
    for (int xc = 0; xc < SE_MAX_CONTROL_POINTS; ++xc) {
      ...
      if (xcp->start_pixel >= xcp->end_pixel)
        continue;
      ImVec2 uv0 = {(xcp->start_pixel + 1) / tex_w,
                    (ycp->start_pixel + 1) / tex_h};
      ...
      pmax.x += rw;
      if (pmin.x > x_clamp || pmin.y > y_clamp)
        continue;
      if (pmax.x > x_clamp) {
        uv1.x =
            uv0.x + (uv1.x - uv0.x) * (x_clamp - pmin.x) / (pmax.x - pmin.x);
        pmax.x = x_clamp;
      }
      if (pmax.y > y_clamp) {
        uv1.y =
            uv0.y + (uv1.y - uv0.y) * (y_clamp - pmin.y) / (pmax.y - pmin.y);
        pmax.y = y_clamp;
      }
      if (xcp->screen_control && ycp->screen_control && first_screen) {
        ...
        float dpi_scale = se_dpi_scale();
        float lcd_pos[2] = {
            ceil((pmin.x + lcd_dims[0] * 0.5) * dpi_scale) / dpi_scale,
            ceil((pmin.y + lcd_dims[1] * 0.5) * dpi_scale) / dpi_scale};
        se_draw_lcd_in_rect(lcd_pos[0], lcd_pos[1], lcd_dims[0], lcd_dims[1],
                            hybrid_nds);
      }

      int t = 0xff000000;
      ImDrawList_AddImage(igGetWindowDrawList(),
                          (ImTextureID)(uintptr_t)gui_state.theme.image.id,
                          pmin, pmax, uv0, uv1, tint);
      // ImDrawList_AddRect(igGetWindowDrawList(),pmin,pmax,t,0,ImDrawCornerFlags_None,
      // 2);
      pmin.x = pmax.x;
    }
    pmin.y = pmax.y;
...
static void se_compute_lcd_render_dims(float *available_dims,
                                       float *render_dims) {}
static bool se_load_theme_from_image(uint8_t *im, uint32_t im_w,
                                     uint32_t im_h) {
  if (!im) {
    return false;
  }
  ...
  for (int x = 0; x < 2; ++x) {
    for (int y = 0; y < 2; ++y) {
      se_theme_region_t *region =
          &theme->regions[SE_REGION_VOL_EMPTY + y + x * 2];
  ...
  for (int key = 0; key < 7; ++key) {
    ...
    se_theme_region_t *key_down =
        &theme->regions[key * 2 + SE_REGION_KEY_A_PRESSED];
    key_up->x = 4194;
  ...
  for (int x = 0; x < 2; ++x) {
    for (int y = 0; y < 2; ++y) {
      se_theme_region_t *region =
          &theme->regions[SE_REGION_KEY_START + y + x * 2];
    ...
    if (region->active) {
      for (int dir = 0; dir < 2; ++dir) {
        ...
        while (curr_x < end_x && curr_y < end_y) {
          ...
          int screen = im[p + 1];
          int gamepad = im[p + 2];
          if (resize != cp->resize_control || screen != cp->screen_control ||
              gamepad != cp->gamepad_control) {
            ++current_point;
            if (current_point >= SE_MAX_CONTROL_POINTS) {
              printf("Error: Theme requires more control points than the %d "
                     "limit\n",
...
static void se_init() {
  ...
  if (emu_state.cmd_line_arg_count > 3 &&
      strcmp("http_server", emu_state.cmd_line_args[1]) == 0) {
    gui_state.test_runner_mode = true;
    gui_state.settings.http_control_server_port =
        atoi(emu_state.cmd_line_args[2]);
    ...
    // HTTP Server mode only has frame stepping which is not allowed in hardcore
    // mode.
...
static void init(void) {
  ...
  sg_buffer_desc vb_desc = {.usage = SG_USAGE_IMMUTABLE,
...
static void emsc_load_callback(const sapp_html5_fetch_response *response) {
  if (response->succeeded) {
    sb_save_file_data((char *)response->user_data,
                      (uint8_t *)response->buffer_ptr, response->fetched_size);
...
static void event(const sapp_event *ev) {
  ...
  if (ev->type == SAPP_EVENTTYPE_FILES_DROPPED) {
    ...
    if (num_dropped_files) {
      ...
      for (int i = 0; i < num_dropped_files; ++i) {
        ...
        sapp_html5_fetch_dropped_file(
            &(sapp_html5_fetch_request){.dropped_file_index = i,
    ...
    for (int i = 0; i < SAPP_MAX_TOUCHPOINTS; ++i) {
      ...
      if (ev->touches[i].pos_y < gui_state.screen_height * 0.05 &&
          gui_state.touch_points[i].active)
        gui_state.menubar_hide_timer = se_time();
    }
  } else if (ev->type == SAPP_EVENTTYPE_MOUSE_MOVE) {
    gui_state.mouse_pos[0] = ev->mouse_x;
    gui_state.mouse_pos[1] = ev->mouse_y;
    if (gui_state.mouse_pos[1] < gui_state.screen_height * 0.1)
      gui_state.menubar_hide_timer = se_time();
  } else if (ev->type == SAPP_EVENTTYPE_MOUSE_UP ||
             ev->type == SAPP_EVENTTYPE_MOUSE_DOWN) {
    int b = ev->mouse_button;
    if (b < 3)
      gui_state.mouse_button[0] = ev->type == SAPP_EVENTTYPE_MOUSE_DOWN;
  }
}
bool se_run_ar_cheat(const uint32_t *buffer, uint32_t size) {
  if (emu_state.system == SYSTEM_GBA)
    return gba_run_ar_cheat(&core.gba, buffer, size);
  if (emu_state.system == SYSTEM_GB)
    return sb_run_ar_cheat(&core.gb, buffer, size);
  if (emu_state.system == SYSTEM_NDS)
    return nds_run_ar_cheat(&core.nds, buffer, size);
...
#ifdef PLATFORM_ANDROID
void Java_com_sky_SkyEmu_EnhancedNativeActivity_se_1android_1load_1rom(
...
void Java_com_sky_SkyEmu_EnhancedNativeActivity_se_1android_1load_1file(
    JNIEnv *env, jobject thiz, jstring filePath) {
  const char *nativeFilePath = (*env)->GetStringUTFChars(env, filePath, 0);
...
sapp_desc sokol_main(int argc, char *argv[]) {
  ...
  return (sapp_desc) {
    ...
    .event_cb = event, .window_title = "SkyEmu", .width = width,
    .height = height, .enable_dragndrop = true, .enable_clipboard = true,
    .high_dpi = true, .max_dropped_file_path_length = 8192,
#if defined(EMSCRIPTEN)
    .max_dropped_files = 32,
#endif
    .swap_interval = 0, .ios_keyboard_resizes_canvas = true
  };
}
