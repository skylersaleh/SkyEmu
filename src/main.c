...
bool se_load_state_from_disk(se_save_state_t* save_state, const char* filename) {
  ...
  int downscale = 1;
  while((im_w / downscale) * (im_h / downscale) * 4 >= sizeof(save_state->screenshot))
    downscale++;
...
static void se_sort_recent_games_list() {
  ...
  if(gui_state.recent_games_sort_type == SE_SORT_ALPHA_ASC)
    qsort(gui_state.sorted_recently_loaded_games, size, sizeof(int), se_game_info_alpha_comparator);
...
void se_draw_arm_state(const char* label, arm7_t* arm, emu_byte_read_t read) {
  ...
  for(int i = 0; i < ARM_DEBUG_SWI_RING_SIZE && i < arm->debug_swi_ring_offset; ++i) {
    ...
    if(arm->debug_swi_ring_times[ind] >= 2)
      se_text("SWI 0x%02x (%dx)", arm->debug_swi_ring[ind], arm->debug_swi_ring_times[ind]);
    else
...
void gb_cpu_debugger() {
  ...
  int register_values_8b[] = {
    ...
    SB_U16_LO(cpu_state->af),
    SB_U16_HI(cpu_state->bc),
    SB_U16_LO(cpu_state->bc),
    SB_U16_HI(cpu_state->de),
    SB_U16_LO(cpu_state->de),
    SB_U16_HI(cpu_state->hl),
    ...
    if(igTreeNodeStrStr(mmios[i].name, lab)) {
      for(int f = 0; f < sizeof(mmios[i].bits) / sizeof(mmios[i].bits[0]); ++f) {
        ...
        if(size) {
          ...
          igSameLine(0, 2);
          if(size > 1)
            se_text("%s (Bits [%d:%d])", mmios[i].bits[f].name, start, start + size - 1);
          else
...
void sb_draw_onscreen_controller(sb_emu_state_t* state, int controller_h, int controller_y_pad, bool preview) {
  ...
  int dpad_code = up ? 0 : down ? 6
                                : 3;
  dpad_code += left ? 0 : right ? 2
                                : 1;
  if(dpad_code == 4) {
    dpad_code = state->prev_frame_joy.inputs[SE_KEY_UP] > 0.2 ? 0 : state->prev_frame_joy.inputs[SE_KEY_DOWN] > 0.2 ? 6
                                                                                                                    : 3;
    dpad_code += state->prev_frame_joy.inputs[SE_KEY_LEFT] > 0.2 ? 0 : state->prev_frame_joy.inputs[SE_KEY_RIGHT] > 0.2 ? 2
                                                                                                                        : 1;
  }
...
void se_open_file_browser(bool clicked, float x, float y, float w, float h, void (*file_open_fn)(const char* dir), const char** file_types, char* output_path) {
  ...
  if(allow_directory)
    ...
    outPath = tinyfd_openFileDialog(se_localize_and_cache("Open ROM"), "", num_file_types, file_types, NULL, 0);
  if(outPath) {
      ...
      while(file_browse->cached_dir.has_next && f < max_files) {
        ...
        if(!file_browse->cached_files[f].is_dir) {
          ...
          for(int i = 0; i < file_browse->num_file_types; ++i) {
            if(sb_path_has_file_ext(file_browse->cached_files[f].path, file_browse->file_types[i])) {
              show_item = true;
              break;
            }
          }
